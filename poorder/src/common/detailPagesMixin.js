    ...
    resetJieshuanWagedom(row, mawbinfo) { //发起驳回的时候，如果wagedom是结算,需要重置为航线或者客服
      ...
      if (row.wagedom == '结算' && row.status == -1) {
        if (mawbinfo.czlx == '代操作' || mawbinfo.islocal == 2) {
          ...
          if (row.servicecode == 'OA0010' && ['航司运费', '航司中转费', '航司战争附加费', '航司燃油附加费', '锂电池操作费','更改费'].includes(row.items)) {
            wagedom = '航线'
          } else {
...
export const myCommonTable = { //费用驳回的弹窗
  ...
  computed: {
    ...
    wagedom() {
      if (this.initiator == '35' || this.initiator == '30' || !this.initiator) {
        return '结算'
      } else if (this.initiator == '10') {
        return '客服'
      } else if (this.initiator == '20') {
        return '航线'
      } else if (this.initiator == '40') {
        return '经理'
      } else if (this.initiator == '50') {
        return '结算对账时'
      } else if (this.initiator == '60') {
        return '调账'
      } else if (this.initiator == '70') {
        return 'D/N制作'
      } else if (this.initiator == '75') {
        return 'D/N审核'
      }
    },
    ordertype(){
  ...
  methods: {
    ...
    // 获取费用对象状态 ---- 同意按钮
    getAffirmCostObject(row) {
      if (this.initiator == '10') {
        // 客服
        return row.receiver === 10 && (row.status === 100 || row.status === 110 || row.status === 200 || row.status === 210) ? true : false
      } else if (this.initiator == '20') {
        // 航线
        return row.receiver === 20 && (row.status === 100 || row.status === 110 || row.status === 200 || row.status === 210) ? true : false
      } else if (this.initiator == '35' || this.initiator == '30' || !this.initiator) {
     
        return (this.mawbinfo.area != '上海' && this.ordertype === '2'
          || this.ordertype === '4' && row.receiver === 35 && row.status === 210)
          || (row.receiver === 30 && row.status === 200)
          || (this.mawbinfo.area != '上海' && row.receiver === 30 && row.status === 100)
          || (this.mawbinfo.area != '上海' && this.ordertype != '4' && row.receiver === 35 && row.status === 110) ? true : false
      } else if (this.initiator == '40') {
        // 经理
        return row.receiver === 40 && row.status === 200 ? true : false
      } else if (this.initiator == '50') {
        // 结算对账时
        return row.receiver === 50 && row.status === 200 ? true : false
      } else if (this.initiator == '60') {
        // 调账
        return row.receiver === 60 && row.status === 200 ? true : false
      } else if (this.initiator == '70') {
        // D/N制作
        return row.receiver === 70 && row.status === 200 ? true : false
      } else if (this.initiator == '75') {
        // D/N审核
        return row.receiver === 75 && row.status === 200 ? true : false
      }
    },
    // 获取驳回对象状态 -----  驳回按钮
    getRejectCostObject(row) {
      if (this.initiator == '10') {
        // 客服
        return row.receiver === 10 && (row.status === 100 || row.status === 110 || row.status === 200 || row.status === 210) ? true : false
      } else if (this.initiator == '20') {
    ...
    getDeleteObject(row) {
      if (this.initiator == '10') {
        ...
        // 订单
        return (this.mawbinfo.area === '上海' && row.status === 100) || (this.mawbinfo.area != '上海' && this.ordertype === '2' && (row.receiver === 10 || row.receiver === 20) && row.status === 110) || this.mawbinfo.area === '上海' && this.ordertype === '4' && (row.receiver === 10 || row.receiver === 20 || row.receiver === 35) && row.status === 110 ? true : false
      } else if (this.initiator == '40') {
        // 经理 
        return row.status === 100 ? true : false
      } else if (this.initiator == '50') {
        // 结算对账时
        return row.status === 100 ? true : false
      } else if (this.initiator == '60') {
  ...
  // props.data.index == (tableData.length - 1) && (props.data.row.status == 100 || props.data.row.status == 110) && isSponsor && initiator != 35  删除按钮
  // ||(wagedom=='结算'&&props.data.row.wageinout=='1'&&mawbinfo.orderguid=='-1'&&props.data.row.receiver!='35')//操作
  template: `
  <div>
<commonTable class="commonTable" :head="tableHead" :firstTdShow="false" :table-data="tableData"  :tableIndex="tableIndex"    :wagetype="wagetype" :putIndex="putIndex">
    ...
    <template slot="operate" slot-scope="props">
         <i class="el-icon-delete" @click="operateFunc(4,props.data.index,props.data.row)" v-show="props.data.index == (tableData.length - 1) && getDeleteObject(props.data.row)"></i>
        
         <span v-show="props.data.index==(tableData.length-1)&&(props.data.row.status==500||props.data.row.status==510?isSponsor:(wagedom!='结算'||props.data.row.wageinout=='2'&&props.data.row.receiver==initiator)||wagedom=='结算')">
              <i class="el-icon-success" style="color:#00CD00;" title="同意" @click="agreeBtn(3,props.data.index)" v-show="props.data.index == (tableData.length - 1) &&  getAffirmCostObject(props.data.row)"></i>
              <i class="el-icon-error" style="color:red;" title="驳回" @click="agreeBtn(2,props.data.index)" v-show="props.data.index == (tableData.length - 1) &&  getRejectCostObject(props.data.row)"></i>
         </span>
  </template>
