...
export default {
        ...
        if (nav) {
          ...
          this.$axios({
            ...
            setTimeout(() => {
              ...
              if (gotoNav) {
                let data = JSON.parse(sessionStorage.nav).filter(
                  (i) => i.auturl == gotoNav + ".vue"
                )[0];
            ...
            window.addEventListener("beforeunload", (event) => {
              window.opener.queueObj.boState["isOpen"] = false;
            });
            window.opener.queueObj.subscribe(
              queueKey.gotoNav,
              "app",
              function (d) {
                let data = JSON.parse(sessionStorage.nav).filter(
  ...
  methods: {
    ...
    createNav(getNav, kzdata) {
      //创建路由
      console.log(getNav);
      if (
        this.system == "wffmanagement" &&
        window.opener &&
        window.opener.resultdicWffmanagement
      ) {
        this.workUnFinishResult =
          window.opener && window.opener.resultdicWffmanagement; //经理审核在登录页控制
      } else {
        ...
        if (item.auturl !== "admin/frmPoSearch.aspx") {
          ...
          if (kzdata.resultstatus > 0 && item.module != "基础数据层") {
            ...
            if (kzdata.resultid == 34) {
              if (
                !(temp == "asignSearchMonitor" || item.autname == "航线操作")
              ) {
                continue;
              }
            }
        ...
        this.$confirm(kzdata.resultmessage, "提示", {
          ...
          if (
            ...
            for (let i of newrouter) {
              ...
              if (
                i.temp &&
                ((kzdata.resultid == 15 && i.temp === "gnSearch") ||
                  (kzdata.resultid == 4 && i.temp === "costConfirmation") ||
                  (kzdata.resultid == 5 && i.temp === "costConfirmation") ||
                  (kzdata.resultid == 16 && i.temp === "gnCostConfirm"))
              ) {
        ...
        setTimeout(() => {
          if (this.system == "wffmanagement") {
            let resultdic =
              window.opener &&
              window.opener.resultdicWffmanagement &&
              window.opener.resultdicWffmanagement.resultdic;
