...
class priceFreightEditView extends priceFreightView {
  ...
  async loadData(p) {
    ...
    await this.request("get", url, { json: JSON.stringify(json) }).then(d => {
      ...
      this.approvalArr = this.feeMap[p.feeid]['approvalArr'] || [];
  ...
  updateView(map) {
    ...
    this.hbhArr = map.hbhArr ? map.hbhArr.map(m => {
      return {
        hbh: m.hbh,
        schedule: m.schedule,
        remark: m.remark,
  ...
  clickNext({data={}, disabledStatus=false}) {
     if (this._currentPageMode == pageMode.pageOne || disabledStatus) {
      if (disabledStatus) { //禁用
        this.disabled = disabledStatus
        this.openEditDialog(data, disabledStatus)
      } else { // 编辑
  ...
  async savePrice() {
    ...
    let data = {
      ...
      volRatioArr: this.convertDiffArr(this.volArr).filter(f => { return f.isSetValue }),
 ...
 createFixedPriceKey(v, w) {
  ...
  openRelationDialog(title) {
    ...
    if (title == this.relationMap.cus.title) {  // 客户参数
