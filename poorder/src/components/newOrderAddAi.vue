  ...
  <div>
     <!-- <commonTabs :list="infoTableFirst" style="margin-top: 12px;" :showExpanionBtn="false" :checkedIndex.sync="goodsinfoTabsChecked" :showTooltip="false" titlefield="title"> -->
       ...
       <div>
         ...
         <div class="detail-c" v-show="goodsinfoTabsChecked==0">
             ...
             <newFormCmpt :system="initInfo.system" :view-data.sync="inputModelData.orderdom=='总单'?inputViewData:allViewData"  :model-data.sync="initInfo" :pagetype="2" :area="inputModelData.area" id="mawbMark">
               <hwxzComptNew slot="exForm1" showGuageString :inputModelData="initInfo" v-if="initInfo.orderdom=='直单'"></hwxzComptNew>
              </newFormCmpt> 
          </div>
       ...
       <div v-show="initInfo.orderdom=='直单'">
         <!-- 总单服务项目 -->
        <div class="detail">
            ...
            <div class="detail-c" v-show="showService">
              ...
              <serviceList  :pagetype="initInfo.guid>0?2:1"
                  ...
                  :system="initInfo.system" :czlx="initInfo.czlx">
                </serviceList>
            </div>
        </div>
        <!-- 总单货物信息 -->
        <div class="detail" v-show="Object.values(newService).filter(i=>i.servicecode=='AB0420'&&!i.model).length&&initInfo.ybpiece">
          <div class="detail-title">
                实际货物信息
          </div>
          <div class="detail-c" style="overflow:auto">
               <commonTable :head="cargoTableHead" :tableData="initInfo.ybstoreList" class="commonTable" style="width:1500px;">
                       ...
                       <template slot="operate" slot-scope="props">
                           <i class="el-icon-delete" v-show="props.data.index!=initInfo.ybstoreList.length-1"
     ...
     <div v-show="initInfo.orderdom=='总单'">
       ...
       <div class="detail">
        ...
        <!-- 分单货物信息 -->
        <div class="detail" v-show="Object.values(newService).filter(i=>i.servicecode=='AB0420'&&!i.model).length&&initInfo.hawb_ybpiece">
          <div class="detail-title">
                实际货物信息
          </div>
          <div class="detail-c" style="overflow:auto">
     ...
     <!-- </commonTabs>   -->
        ...
        <div style="float:right" v-if="pagetype=='1'">
          ...
          <el-button type="primary" @click="saveConfirm(true)">保存并新增</el-button>    
        </div> 

        <div style="float:right" v-else>
          <el-button type="danger" @click="putMawbInfo" v-if="!$store.state.mainSearch">保存</el-button>
          <el-button type="primary" @click="$parent.$emit('update:visible',false)" v-if="!$store.state.mainSearch">返回</el-button>    
        </div>
      ...
      <el-dialog :visible.sync="assignShow" :close-on-click-modal="false" v-if="assignShow" append-to-body
        ...
        <fenpei :fenpeiguid="mawbguid" :information="serviceList.filter(i=>i.isdel==1)" @back="backFenpei"
          ref="fenpei" :system="initInfo.system" :new-add="1" origin="客服">
...
<script>
  ...
  export default {
    ...
    components: {
     tempConfig,serviceList,revSedMawb,revSedHawb,revSed,fenpei
    },
    ...
    methods: {
         initInfoData(){//初始数据
          if(this.pagetype=='1'){
            return {
             ...
             hawb_englishpm: "CONSOLIDATION CARGO AS PER ATTACHED MANIFEST",//英文品名
          ...
          let hawbField=['yjStoredatetype','hwplace','hasdjh','hascdjh','hastzcx','hasjmyq','ysfs','jsfs']

          let hawbFieldList= ['hawb_sfg','hawb_mdg','hawb_ybpiece','hawb_ybweight','hawb_jfweight','hawb_smallpiece','hawb_englishpm','hawb_sizeremark','hawb_tradeterm','hawb_jsfs','hawb_yjStoredatetype','hawb_hwplace','hawb_hasdjh','hawb_hascdjh','hawb_hastzcx','hawb_hasjmyq']
          
          hawbField.forEach(i=>{
            mawbInfo[i]=String(mawbInfo[i])
          })

          if(mawbInfo.orderdom=='直单'){//直单直接取总单上的信息
          ['hawb_sizeremark','hawb_englishpm','hawb_tradeterm','hawb_jsfs','hawb_yjStoredatetype','hawb_hwplace','hawb_hasdjh','hawb_hascdjh','hawb_hastzcx','hawb_hasjmyq'].forEach(i=>{
         ...
         isOver(mawbpiece,hawbpiece){//件数超量  总单件数、分单件数
         //新增只需
         var allPiece=Number(this.initInfo[mawbpiece])-Number(this.initInfo['goodsybpiece'])
          ...
          if(Number(this.initInfo[hawbpiece])<this.initInfo.ybstoreList.reduce((a, b) => { return a += Number(b.piece) }, 0)){
            this.initInfo[hawbpiece]=''
            return this.$message.error('分单件数小于货物总件数')
          }
          this.isOverJfweight('jfweight','hawb_jfweight')
         },
         isOverJfweight(jfweight,hawbJfweight){//计费重量、分单计费重量
             if(this.initInfo[hawbJfweight]){
               var allJfweight=Number(this.initInfo[jfweight])-Number(this.initInfo['goodsjfweight'])
               var allPiece=Number(this.initInfo['ybpiece'])-Number(this.initInfo['goodsybpiece'])
              if(Number(this.initInfo[hawbJfweight])<Number(allJfweight)&&Number(this.initInfo['hawb_ybpiece'])==Number(allPiece)){
         ...
         reSearch(){//输入总运单号重新查询
            if(this.initInfo.mawb){
              ...
              params: {mawb:this.initInfo.mawb,area:this.initInfo.area,orderdom:this.initInfo.orderdom},
              loading: true,
              tip: false
            })
            .then(({data})=>{
              if(data.resultstatus==0){
                ...
                if(data.resultdic.impMawbInfo){
                    Object.keys(this.inputViewData).forEach(i=>{
                      if(['ybpiece','ybweight','jfweight'].includes(i)){
                        if(data.resultdic.impMawbInfo[i]){
                           this.inputViewData[i]['itemStyle']={ width: '200px', 'margin-right': '6px' }
                           this.inputViewData['jfweight']['occupyRestSpace']=false
                        }
                      }
                      if(['goodsybpiece','goodsybweight','goodsjfweight'].includes(i)){
                        if(data.resultdic.impMawbInfo[i]){
                    ...
                    Object.keys(data.resultdic.impMawbInfo).forEach(i=>{
                      ...
                      if(['jsfs','yjStoredatetype','hwplace','hasdjh','hascdjh','hastzcx','hasjmyq'].includes(i)){
        ...
        //  },
         inputChange(field,field2,field3){//新增一条货物信息 进仓编号、重量、件数
           ...
           let allpiece=this.initInfo.ybstoreList.reduce((a, b) => { return a += Number(b.piece) }, 0);

            if(this.initInfo.orderdom=='总单'&&allpiece>Number(this.initInfo.hawb_ybpiece)||this.initInfo.orderdom=='直单'&&allpiece>Number(this.initInfo.ybpiece)){
              data[field3]=''
              return this.$message.error('件数超出限制')
            }else{
         ...
         if(this.initInfo.ybstoreList.length==1){
           this.initInfo.ybstoreList=[this.initCargoInfo]
         }else{
      ...
      confirmAdd(){//必填判断
        ...
        for(var i in Object.keys(this.allViewData)){
          ...
          if(!item.hidden&&item.required&&!this.initInfo[field]){//必填项，非隐藏表单
            status=false
        ...
        if(Object.values(this.newService).filter(i=>i.servicecode=='AB0420'&&!i.model).length){
          ...
          if(!this.initInfo['ybstoreList'].filter(i=>i.khjcno).length){
            return  this.$message.error('请补充完整货物信息')
          }
          var storeIndex=this.initInfo['ybstoreList'].length-2
          if(!this.initInfo['ybstoreList'][storeIndex]['khjcno']||!this.initInfo['ybstoreList'][storeIndex]['piece']||!this.initInfo['ybstoreList'][storeIndex]['weight']){
            return  this.$message.error('请补充完整货物信息')
          }else{
            status=true
          }
        ...
        if(status){
           ...
           let publicField=['hawb_sfg','hawb_mdg','hawb_ybpiece','hawb_smallpiece','hawb_ybweight','hawb_jfweight']
           if(this.inputModelData.orderdom=='总单'){

           Object.keys(this.hawbViewData).forEach(i=>{//分单、总单基本信息赋值  分单与总单相同字段标识'hawb_mdg'
             if(i.includes('_')||i=='hawb'){
               ...
               if(!publicField.includes(i)){
                  this.$set(mawbInfo,i.includes('_')?i.split('_')[1]:'hawb',this.initInfo[i])
               }
             }
           })
           ...
           Object.keys(mawbInfo).forEach(i=>{
             if(i.split('_')[0]=='hawb'&&!publicField.includes(i)){
               this.$set(mawbInfo,i.split('_')[1],this.initInfo[i])
               //delete mawbInfo.i
             } 
           })
           }
           mawbInfo.ybstoreList=mawbInfo.ybstoreList.filter(i=>i.piece&&i.weight&&i.khjcno)
           return mawbInfo
        }else{
          return false
        }
      ...
      saveConfirm(type){//确认订舱  type:保存并新增
       if(this.confirmAdd()){
            ...
            this.$confirm(
              ...
              if(this.saveStatus){
                //  初始化部分分单信息
                 ...
                 var list1=['hawb_hwplace','hawb_hasdjh','hawb_hascdjh','hawb_hastzcx','hawb_hasjmyq'].forEach(i=>{
                    this.initInfo[i]='2'
                 })
                 var list2=['hawb_ybpiece','hawb_ybweight','hawb_jfweight','hawb'].forEach(i=>{
                   console.log(i)
                   this.initInfo[i]=''
                  })
                  this.reSearch()
    ...
    watch:{
    'inputModelData.orderdom':{//切换订单类型
      handler(val){
      this.goodsinfoTabsChecked=0
      //直单只显示总单信息(hawbViewData部分表单和inputViewData合并)，总单则是分开
      Object.keys(this.hawbViewData).forEach(i=>{
        if(!['hawb_sizeremark','hawb_englishpm','hawb_hascdjh','hawb_hasdjh','hawb_hasjmyq','hawb_hastzcx','hawb_hwplace','hawb_jsfs','hawb_tradeterm','hawb_yjStoredatetype'].includes(i)){
    ...
    'initInfo.mdg':{//货物来源根据到达港改变
      handler(val){
        if(val){
           this.inputViewData['jjd']['options']=JSON.parse(localStorage.jjd).filter(i=>i.sfg==val).map(i=>{
    ...
    created(){
      this.packageNameList=JSON.parse(localStorage.groupType).filter(i=>i.groupid=='31').map(i=>i.typename)
     this.$watch(()=>{
      ...
      this.$watch(()=>{
        return ['opersystem','opersystemdom','area','ordertype','fid','gid','wtkhname','ispay_pre','system'].map(i=>this.inputModelData[i])
      },(val)=>{
        ['opersystem','opersystemdom','area','ordertype','fid','gid','wtkhname','ispay_pre','system','customerRelList'].forEach(i=>{
            ...
            if (this.newService[item.servicecode]) {
              if(this.newService[item.servicecode].disabled==false){
                ...
                item.servicecode == 'OA0010' ? this.initInfo.system != '国内服务' && this.initInfo.czlx == '自货' : item.isdel == 1;
                 ...
                 if(item.servicecode=='AB0420'&&!this.newService[item.servicecode].model){
                   this.initInfo.ysfs='外仓货'
                 }
              }
            }
