  ...
  <div>
    ...
    <div v-else> -->
      ...
      <el-button-group style="margin-top: 20px;margin-bottom: 15px;" size="small" class="buttonTabs"
        v-if="system=='bo'&&!prevUpload&&!peidanF">
        <el-button v-for="(item,index) in firstLevelTab" :key="index" style="height: 32px;"
          :style="{border:index==firstLevelIndex?'1px solid #0F649B':'',borderRadius:0+'px'}" :type="index==firstLevelIndex?'primary':''"
      ...
      <div class="orderListContainer" v-if="firstLevelIndex == 0 && orderList && orderList.length > 0">
        ...
        <div class="item" v-for="(item,index) in orderList" :key="index" @click="getClickObj(item)" >
          {{item.pono}}
        </div>
      </div>
        ...
        <div class="main">
          <div class="main-detail">
            ...
            <div class="docdetail" style="display:flex;flex-wrap:wrap">
              ...
              <commonTable :head="tableHead" :tableData="handleRow == 'all' ? docList : docList.filter(item=> item.jobno == handleRow.pono)" draggable style="width:100%" isMultiSelect
              ...
              <el-upload drag class="upload-demo" ref="upload" v-show="isediting&&editIndex==-1" multiple
                :action="$store.state.imageWebApi + (tagStatus !== '' ?  'api/RankDocMultiple':  'api/RankDoc')" :data="newUpData" :show-file-list="true"
      ...
      <div v-if="!prevUpload">
        <slot name="imgdocTable" v-if="firstLevelIndex == 0"></slot>
...
<script>
  export default {
          ...
          this.$watch(
            () => {
              ...
              if (!this.docupData.fid || !this.docupData.gid) {
                ...
                this.$axios({
                  method: "get",
                  params: { sid: (this.tagStatus == "情况补充说明" || this.tagStatus ==  "更改结算对象") ? this.wdkhId : this.docupData.gid , system: '空出', timestamp: 0, relarea: this.orderArea, area: this.orderArea },
      ...
      */
      ...
      submitUpload() {
        ...
        // let data = new FormData();
          ...
          this.$axios({
            method: "put",
            url: this.$store.state.imageWebApi + (this.tagStatus == "情况补充说明" || this.tagStatus ==  "更改结算对象" && this.firstLevelIndex == 1 ?"api/RankDocMultiple" :  "api/RankDoc" ),
            data: this.docupData,
            loading: true,
            tip: false,
          }).then(response => {
      ...
      getDocList() {
        ...
        setTimeout(() => {
          ...
          this.getRankDoc(json).then(data => {
            ...
            if(this.peidanF&&data.length){
               this.$emit('peidanFc',docList.map(i=>i.id).join(','))
            }
    ...
    watch: {
      ...
      'firstLevelIndex':{
        ...
        handler: function(val){
          if(val === 1 && (this.tagStatus == "上传情况说明" || this.tagStatus ==  "更改结算对象") ) {
            this.docviewData.khjcno.hidden =  this.tagStatus == "" ? false :  (this.tagStatus == "上传情况说明" || this.tagStatus ==  "更改结算对象")  ? true : false
            this.docviewData.kdnumber.hidden =  this.tagStatus == "" ? false :  (this.tagStatus == "上传情况说明" || this.tagStatus ==  "更改结算对象")  ? true : false
            this.docviewData.sjr.hidden =  this.tagStatus == "" ? false :  (this.tagStatus == "上传情况说明" || this.tagStatus ==  "更改结算对象" ) ? true : false
          }  else if(val === 0){
            this.docviewData.khjcno.hidden =  false
            this.docviewData.kdnumber.hidden =  false
            this.docviewData.sjr.hidden =  false
          } 
          // else if(val === 0 && (this.tagStatus == "上传情况说明" || this.tagStatus ==  "更改结算对象")) {
          //   this.docviewData.khjcno.hidden =  false
          //   this.docviewData.kdnumber.hidden =  false
          //   this.docviewData.sjr.hidden =  false
          // }
    ...
    computed: {
      ...
      dialogTitle() {
        if(this.tagStatus == "上传情况说明") {
          return "上传情况说明"
        } else if(this.tagStatus== "更改结算对象") {
          return "更改结算对象"
        } else {
           return this.prevUpload&&!this.peidanF? `${this.modelData.shipperno}` : this.peidanF?`${this.bgNumber}`:"文档操作"
        }
          ...
          if(this.tagStatus == "上传情况说明") {
             ...
             console.log(this.orderList)
              this.orderList.map(item => {
                resultData.push({mawb:item.mawb, pono:item.pono,boguid:item.boguid})
            })
            console.log(resultData)
            obj.joblist = JSON.stringify(resultData)
            ...
            delete obj.gid
              ...
              console.log(this.orderList)
              this.orderList.map(item => {
                 arr.push({mawb:item.mawb, pono:item.pono,boguid:item.boguid})
              })
              obj.joblist = JSON.stringify(arr)
              obj.items = this.itemList
              obj.gid = this.newObjectId
              console.log(obj)
          } else {
            delete obj.items
            delete obj.joblist
    ...
    mounted(){
      if(this.tagStatus == "上传情况说明" || this.tagStatus ==  "更改结算对象"){
              this.$set(this.docupData, 'ready01', "主营");
              this.$set(this.docupData, 'typename', "情况说明");
      }
    }
