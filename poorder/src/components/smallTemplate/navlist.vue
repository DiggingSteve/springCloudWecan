        ...
        <div class="list-item" v-for="(item, index) in list" :key="index" v-if="!item.hidden">
            <div class="item-name">
                <div :level="item.level" :style="{color:getSystemBgColor(item.autname,item.level)}" :id="item.level==1?item.autname:''"
                    ...
                    <div class="titlediv">
                        <span v-if="item.level>1" style="display:inline-block;width:14px;position: relative;"
                        ...
                        <span class="title">
                            ...
                            <router-link v-else tag="span" :to="{name:item.temp,params: {id: item.id}}" :key="item.id">
                                {{item.autname}}
                            </router-link>
