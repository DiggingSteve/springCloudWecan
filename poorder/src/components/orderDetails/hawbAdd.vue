<template>
  <div>
    <div
      style="text-align: right;height: 0px;transform: translate(-112px,-52px);z-index: 600;position: relative"
    >
    ...
    <div v-show="mawbdetail.hawbList.length > 0">
      <div v-for="(i, hawbIndex) in mawbdetail.hawbList" :key="hawbIndex">
        <div class="detail" v-show="hawbIndex == hawblistChecked">
          <div class="detail" v-show="mawbdetail.hawbList.length > 0">
            ...
            <div class="detail-c">
              <tempConfig
                ...
                :type="80"
                :jsondata.sync="mawbdetail.hawbList[hawbIndex].serviceList"
                v-if="tempConfigShow"
              >
              </tempConfig>
              ...
              <serviceList
                ...
                :serviceList="
                  initInfo.guid > 0 && mawbdetail.hawbList.length > 0
                    ? mawbdetail.hawbList[hawbIndex].serviceList
                    : []
                "
                :newService="
                  mawbdetail.hawbList.length > 0
                    ? mawbdetail.hawbList[hawbIndex].newService
                    : {}
                "
          ...
          <div
            class="detail"
            v-show="
              Object.values(mawbdetail.hawbList[hawbIndex].newService).filter(
          ...
          >
            ...
            <div class="detail-c" style="overflow:auto">
              <commonTable
                ...
                :tableData="
                  mawbdetail.hawbList.length > 0
                    ? mawbdetail.hawbList[hawbIndex].cargoInfoList
                    : []
                "
                class="commonTable"
                style="width:1500px;"
...
export default {
  ...
  watch: {
    'initInfo.ysfs':{
      ...
      handler(val){
        ...
        if(this.mawbdetail.hawbList.length > 0 ) {
          if(this.mawbdetail.hawbList[this.hawblistChecked].newService['AB0420']){
            if(val!='外仓货'){
              this.mawbdetail.hawbList[this.hawblistChecked].newService['AB0420']['model']=true
            }else{
              this.mawbdetail.hawbList[this.hawblistChecked].newService['AB0420']['model']=false
            }
          }
        }
  ...
  methods: {
    addHawb(data = {}) {
      ...
      // this.handleTabIndex = this.hawblistChecked
    },
    fetch_hawb_data(hawb = {}) {
      ...
      let sumJfweight = 0
      if(this.mawbdetail.hawbList.length > 0) {
          this.mawbdetail.hawbList.forEach(item => {
      ...
      var obj = {
        ...
        sfg: "",
        mdg: "",
        ybpiece: String(Number(this.initInfo.ybpiece) - sumYbpiece),
        ybweight: String(Number(this.initInfo.ybpiece) - sumYbweight) ,
        jfweight: String(Number(this.initInfo.ybpiece) - sumJfweight),
        ybvolume:"",
        smallpiece: "",
        hwxz: "",
        hwlx: "",
      ...
      if (
        ...
        this.mawbdetail.hawbList.length > 0
      ) {
        obj.fid = this.mawbdetail.hawbList[0].fid;
        obj.hawb = this.mawbdetail.hawbList[0].hawb;
        obj.sfg = this.mawbdetail.hawbList[0].sfg;
        obj.hwplace = this.mawbdetail.hawbList[0].hwplace;
        obj.tradeterm = this.mawbdetail.hawbList[0].tradeterm;
        obj.jsfs = this.mawbdetail.hawbList[0].jsfs;
        obj.hasdjh = this.mawbdetail.hawbList[0].hasdjh;
        obj.hascdjh = this.mawbdetail.hawbList[0].hascdjh;
        obj.hastzcx = this.mawbdetail.hawbList[0].hastzcx;
        obj.hasjmyq = this.mawbdetail.hawbList[0].hasjmyq;
        obj.englishpm = this.mawbdetail.hawbList[0].englishpm;
        obj.sizeremark = this.mawbdetail.hawbList[0].sizeremark;
      } else {
    ...
    inputChange(field, field2, field3) {
      ...
      let data = this.mawbdetail.hawbList[this.handleTabIndex].cargoInfoList[
        this.mawbdetail.hawbList[this.hawblistChecked].cargoInfoList.length - 1
      ]; //最后一条货物信息是否有数据

      if (data[field] && data[field2] && data[field3]) {
        this.mawbdetail.hawbList[this.hawblistChecked].cargoInfoList.push({
          ...this.initCargoInfo
        });
      }
    },
    delKhjcno(index) {
      //删除一条货物信息
      this.mawbdetail.hawbList[this.hawblistChecked].cargoInfoList.splice(
        index,
        1
      );
    },
