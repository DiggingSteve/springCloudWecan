  ...
  <div class="invoice" style="width:1200px;margin:0 auto">
      ...
      <div class="detail-c">
        ...
        <p>
          <el-row>
            <el-col :span="6">
              <span>发票号:</span>
              <el-input style="width:140px;" v-model="invoiceObject.invoicecode"
                :disabled="invoicecodeDisable || invoiceType == '2'"></el-input>
            </el-col>
            <el-col :span="6">
              <span>开票人:</span>
            ...
            <el-col :span="6">
              <span>开票日期:</span>
              <el-date-picker v-model="invoiceObject.invoicedate" type="date" value-format="yyyy-MM-dd"
            ...
            <el-col :span="6">
              ...
              <el-select placeholder="请选择" style="width:140px;" v-model="invoiceObject.invoicedom">
                <el-option v-for="(selitem, index) in fptype" :key="index" :label="selitem" :value="selitem">
                </el-option>
        ...
        <p>
          <el-row>
            <el-col :span="12">
              <span>收付对象:</span>
              <page-select :modelval.sync="invoiceObject.paymentgid" :pagetype="100" @change="getData"
                style="width:73%;display:inline-block" width="100%"></page-select>
      ...
      <el-select v-model="invoiceObject.address_thr.areade" placeholder="请选择"  :area="areadeData" style="width:103px;">
          ...
          <el-row>
            ...
            <el-col :span="6" style="margin-left:26px;">
              <customerRel
                ...
                customerRelType="财务" :needPhone="false" @changeRelData="customerRelList = $event">
              </customerRel>
            </el-col>
          </el-row>
        </p>
        ...
        <p>
          <el-row>
            ...
            <el-col :span="6">
              <span>航班(到港)日期:</span>
              <el-date-picker v-model="invoiceObject.hbrq" type="date" value-format="yyyy-MM-dd" placeholder="选择日期"
                style="width:140px">
              </el-date-picker>
            </el-col>
        ...
        <p>
          <el-row>
            <el-col :span="24">
              <span>开票备注:</span>
              <el-input type="textarea" style="width:86%;vertical-align:middle" v-model="invoiceObject.remark">
              </el-input>
            </el-col>
        ...
        <p v-if="invoiceType == '1'">
          <el-row>
            <ElCol :span="24">
              <customerRel style="margin-left: 123px;"
                :fid="invoiceObject.paymentgid || invoiceObject.settgid || (information && (information.accountcomgid || information.settgid)) || ''"
                customerRelType="财务" @changeRelData="customerRelList = $event">
              </customerRel>
            </ElCol>
          </el-row>
        </p>
    ...
    <div class="detail mawbCharge">
          ...
          <commonTable :head="invoiceHead" :table-data="invoiceCostData">
            ...
            <template slot="productName" slot-scope="props">
              <el-select v-model="props.data.row.productName" @change="changeTaxCode($event, props.data.index)">
                <el-option v-for="i in invoiceTableSelectList" :key="i.TaxName" :label="i.TaxName" :value="i.TaxName">
                </el-option>
              </el-select>
            </template>
      ...
      <div class="costTotalMoney" v-if="invoiceType == '1'">
        <p style="color:orange" v-for="(item, index) in invoiceCostAll" :key="index">本票开票合计{{ item.currency }}金额：<span
            style="margin-right:10px;">{{ item.total }}</span>元<span style="margin-left:10px;text-align:left">{{
                item.ctotal
            }}</span></p>
      ...
      <div v-else class="costTotalMoney">
        <p style="color:orange">剩余开票金额：<span style="margin-right:10px;margin-left:28px;">{{ leastAmount.toFixed(2)
        }}</span>元<span style="margin-left:10px;text-align:left">{{ cleastAmount }}</span></p>

        <p style="color:orange">本票开票合计金额：<span style="margin-right:10px;">{{
            Number(invoiceObject.sumTotalAmount).toFixed(2)
        }}</span>元
          <span style="margin-left:10px;text-align:left">{{ csumTotalAmount }}</span>
        </p>

        <p style="color:orange">本票开票合计税额：<span style="margin-right:10px;">{{
            Number(invoiceObject.sumTaxAmount).toFixed(2)
        }}</span>元
          <span style="margin-left:10px;text-align:left">{{ csumTaxAmount }}</span>
        </p>

        <p style="color:orange">本票开票价税合计：<span style="margin-right:10px;">{{ Number(invoiceObject.total).toFixed(2)
        }}</span>元
          <span style="margin-left:10px;text-align:left">{{ ctotal }}</span>
        </p>
...
export default {
  ...
  created() {
    ...
    if (this.information) {
      ...
      this.invoiceObject.settgid = this.invoiceObject.paymentgid;
  ...
  computed: {
    ...
    //是否个人抬头
    isPersonal() {
      let titleName = this.invoiceObject.titlename;
      return titleName == "个人"
    }
  },
  watch: {
    ...
    'invoiceObject.paymentgid': {
      handler(val) {
        if (this.isPersonal) {
          this.invoiceObject.customerAddressTel='';
        }
      }
    },
    'invoiceObject.customerAddressTel': {
      handler(val) {
        if(this.isPersonal)return;
        let isLegalTel = this.checkTel(val);
        if (!isLegalTel) {
          this.$message("购方地址电话号要求格式 区号-号码或者11位手机号")
        }
        let tel = this.getTel(val);
        this.invoiceObject.buyerTel = tel;
        this.invoiceObject.buyerAddress = val.replace(tel, '');
      }
    },
    'invoiceCostData': {
      handler(val) {
  ...
  methods: {
    ...
    checkTel(str) {
      var reg = /(\d{3}-\d{7,}|\d{4}-\d{7,})|\d{11}|\d{3}-\d{3}-\d{4}/g;
    ...
    pushInvoiceCostData(isfirst) {//添加服务项目  //是否第一次push
      ...
      this.invoiceCostData.push({
        'unitPrice': isfirst ? Number(this.totalMoney).toFixed(2) : '',
