...
<!-- transmitMsg 接收子组件点击分单详细加号的索引  用来查询子表格数据-->
 ...
 <template slot="operate" slot-scope="props">
   <span  class="el-icon-edit" style="color:green;cursor:pointer" title="制作单证" @click="openDialog(props.data.index,2)"></span> &nbsp;&nbsp;&nbsp;&nbsp;
   <!-- <span  class="el-icon-chat-dot-square"  title="订单通讯" @click="openChatRoome(props.data.row)"></span> -->
   <el-badge is-dot class="item"      :hidden="$store.state.readMessageList.indexOf(props.data.row.guid) > -1 ? false : true" style="vertical-align:none"   >
    <i class="el-icon-chat-dot-square" style="color:green;cursor:pointer"  @click="openChatRoome(props.data.row)"></i>
</el-badge>
   </template>
 </tableCompt>
...
<!-- v-if="dialogShow"  -->
   ...
   <!-- <hawbtabsoper
             v-if="mawbinfo.guid"
                ...
                ></hawbtabsoper> -->
      <confirmDetail :mawbdetail="tableDataRes[selectTableIndex]" :visible.sync="dialogShow" pagetype=3 fromPage='billmake'></confirmDetail>
       ...
       components: {
        ...
        search(){
           ...
           if(this.$store.state.ifLimitSearch&&Object.keys(this.searchData).length==0){
              ...
              this.tableDataRes=[];
              this.ziTableData=[];
              this.searchData.modifystatus=1
                  var jsonArr2={
                  ...
                  var json={'json':JSON.stringify(jsonArr2)}
                   ...
                   this.$axios({method:'get',url:this.$store.state.webApi+'api/ExAxplineHawbDoc',params:json,loading:true,tip:false}).then(results=>{
                       //console.log(results)
