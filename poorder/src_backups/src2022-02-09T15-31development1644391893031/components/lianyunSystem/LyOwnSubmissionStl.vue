<template>
  <div class="LyOwnSubmissionStl">
    ...
    <el-dialog
      ...
      <LyCostDetail :visible.sync="OrderDetailVisible" @back="OrderDetailVisible = false" :id="cur_order_id" :guid="cur_guid"></LyCostDetail>
...
<script>
import LyCostDetail from './LyCostDetail'

export default {
  name: 'LyOwnSubmissionStl',
  components: {
    LyCostDetail
  },
  data () {
  ...
  methods: {
    ...
    search () {
      ...
      this.$axios({method:'get',url:this.$store.state.webApi+'api/ExTransport/SubmitTransport',params:json,loading:true,tip:false})
      .then(results=>{
