<template>
 ...
 <div  v-if="pagetype==1">
     <infoList :inputModelData="allmaindata" v-if="allmaindata.guid"></infoList>

          <div style="height:32px;line-height:32px;margin-bottom:10px;margin-top:10px;">
        <p>

         <span style="float:left">
         <el-button type="primary" size="mini"  style="height:28px;line-height:13px;" :disabled="mawbCharge.length==0" @click="showAccountShow" v-if="isFromFob">
         <span v-if="isFromFob">D/N预览</span>
         </el-button>

        <el-button type="primary" v-if="orderguid>0&&applyCost&&((profitmode.indexOf('利润')!='-1'&&!allyfUconfirm)||(profitmode.indexOf('Load')!='-1'&&!allysUconfirm)||mawbInformation.wageconfirmstatus>=700)&&!applyModel" @click="applyStart" :disabled="commitstatus=='100'||lineshu">费用申请修改</el-button>

         <el-button type="primary" v-if="orderguid>0&&applyCost&&((profitmode.indexOf('利润')!='-1'&&!allyfUconfirm)||(profitmode.indexOf('Load')!='-1'&&!allysUconfirm)||mawbInformation.wageconfirmstatus>=700)&&applyModel" @click="cancelApply" :disabled="commitstatus=='100'||lineshu">取消申请</el-button>

         <el-button type="primary" v-if="orderguid>0&&applyCost&&((profitmode.indexOf('利润')!='-1'&&!allyfUconfirm)||(profitmode.indexOf('Load')!='-1'&&!allysUconfirm)||mawbInformation.wageconfirmstatus>=700)" @click="finishApply" :disabled="!applyModel||commitstatus=='100'">申请完成</el-button>
...
<span>
 ...
 inactive-color="#009900"
  ...
  >
   ...
   <el-button type="primary" v-if="orderguid>0&&applyCost&&((profitmode.indexOf('利润')!='-1'&&!allyfUconfirm)||(profitmode.indexOf('Load')!='-1'&&!allysUconfirm)||mawbInformation.wageconfirmstatus>=700)&&!applyModel" @click="applyStart" :disabled="commitstatus=='100'||lineshu">
            费用申请修改
        </el-button>

        <el-button type="primary" v-if="orderguid>0&&applyCost&&((profitmode.indexOf('利润')!='-1'&&!allyfUconfirm)||(profitmode.indexOf('Load')!='-1'&&!allysUconfirm)||mawbInformation.wageconfirmstatus>=700)&&applyModel" @click="cancelApply" :disabled="commitstatus=='100'||lineshu">
            取消申请
        </el-button>

         <el-button type="primary" v-if="orderguid>0&&applyCost&&((profitmode.indexOf('利润')!='-1'&&!allyfUconfirm)||(profitmode.indexOf('Load')!='-1'&&!allysUconfirm)||mawbInformation.wageconfirmstatus>=700)" @click="finishApply" :disabled="commitstatus=='100'||!applyModel">申请完成</el-button>
  ...
  <div style="float:left;margin-right:16px;letter-spacing:0.1em">
    ...
    <div class="detail mawbCharge detailPaneYoung">
      ...
      <el-col :span="3" >
        ...
        <el-col :span="yingshouStr.length>0?15:15" style="font-size:12px;font-weight:bold;letter-spacing:0.1em;text-indent:0">
        ...
        <el-button type="primary" size="mini" :disabled="!((!allysConfirm)&&(allysUconfirm)&&!applyModel)" @click="confirmButton('1','700','all')" :style="{'background':!(!allysConfirm&&allysUconfirm&&!applyModel)?'#ccc':'#00cc00','height':'32px','letter-spacing':'0.1em','font-weight':'bold','border-radius':'3px'}" >应收总确认</el-button>
        <el-button type="primary" size="mini" :disabled="!(!allysUconfirm&&(wageguids.length>0||wageCostAllYs)&&!applyModel)" @click="confirmButton('1','1','all')" :style="{'background':!(!allysUconfirm&&(wageguids.length>0||wageCostAllYs)&&!applyModel)?'#ccc':'#00cc00','height':'32px','letter-spacing':'0.1em','font-weight':'bold','border-radius':'3px'}">应收总取消</el-button>
     </span>
      ...
      <div class="detail-c">
       <el-row style="line-height:30px;">
        ...
        <commonTable class="commonTable" :head="mawb_table_model" :table-data="!applyModel?mawbCharge:otherWageinList" style="position:relative;margin-top:26px;" :tableIndex="0" v-if="mawbCharge" :colorIndex="modCostIndex" :className="modCostIndex" :wagetype="wagetype">
           ...
           <el-button type="primary" size="mini" :disabled="yingShouDA()" @click="delAllYsCost" style="display:none">应收费用批量删除</el-button>
            ...
            <td>
              <page-select :modelval.sync="curObjectKyYs.items" :pagetype="5" :area="mawbInformation.area" v-if="mawbInformation.area"></page-select>
...
<i class="el-icon-delete" @click="delCost(props.data.index,1,1)" :class="[showUconfirmF('1',props.data.row)?'disabled':'']" title="删除费用"></i>
  ...
  <div style="float:left;margin-right:16px;letter-spacing:0.1em">
    ...
    <div style="float:right;color:#0f5a8c;font-size:14px;font-weight:bold;margin-right:8px;cursor:pointer" @click="wageOutCarryIn">
             <span>应付带应收</span>
             <span><img src="../../../boStatic/images/jiantou.png"></span>
    </div>
      ...
      <el-row class="detail-title">
        ...
        <el-col :span="yingfuStr.length>0?15:15" style="font-size:12px;font-weight:bold;letter-spacing:0.1em;text-indent:0">
            <div style="width:2%;display:inline-block;vertical-align:top;font-size:18px;color:#999" class="arrow">
...
<span  style="float:right;margin-top:-1px;">
        <el-button type="primary" size="mini" :disabled="!(!allyfConfirm&&allyfUconfirm&&!applyModel)"  @click="confirmButton('2','700','all')" :style="{'background':!(!allyfConfirm&&allyfUconfirm&&!applyModel)?'#ccc':'red','height':'32px','letter-spacing':'0.1em','font-weight':'bold','border-raduis':'3px'}">应付总确认</el-button>
        <el-button type="primary" size="mini" :disabled="!(!allyfUconfirm&&(wageguidf.length>0||wageCostAllYf)&&!applyModel)" @click="confirmButton('2','1','all')" :style="{'background':!(!allyfUconfirm&&(wageguidf.length>0||wageCostAllYf)&&!applyModel)?'#ccc':'red','height':'32px','letter-spacing':'0.1em','font-weight':'bold','border-raduis':'3px'}">应付总取消</el-button>
     </span>
      ...
      <div class="detail-c">
        ...
        <commonTable class="commonTable" :head="mawb_table_model" :table-data="!applyModel?mawbPayment:otherWageoutList" style="margin-top:26px;position:relative" :tableIndex="1" v-if="mawbPayment" :colorIndex="modCostIndex" :wagetype="wagetype">
  ...
  <template slot="content">
         ...
         <template slot="firstTd" slot-scope="props">
            ...
            <td>
                 <page-select :modelval.sync="curObjectKyYf.items" :pagetype="5" :area="mawbInformation.area" v-if="mawbInformation.area"></page-select>
            </td>
            <td>
   ...
   <el-row class="detail-title">
      ...
      <div class="tableContent">
       ...
       <table :style="{'border-collapse':'collapse','table-layout':'fixed','width':'100%','margin-top':'12px'}" >
         ...
         <tr v-for="(item,index) in mawbPayment" :style="{'backgroundColor':index==modCostIndex&&wagetype=='2'?'#F56C6C':item.isfree=='是'?'#e2fdde':'','height':'32px','color':index==modCostIndex&&wagetype=='2'?'#fff':''}">
           <td width='80'>
...
<el-row class="detail-title">
  ...
  <commonTable class="commonTable" :head="mawb_table_model" style="margin-top:20px;" :tableIndex="0" v-if="yingfuMod">
            ...
            <td>
                 ...
                 <page-select :modelval.sync="curObjectKyYf.items" :pagetype="5" :area="mawbInformation.area"></page-select>
            </td>
            <td>
             <el-select v-model="curObjectKyYf.wagetype" style="width:130px;">
...
if(type=="1"&&dom=="1"){
      ...
      .then(()=>{
        if((this.mawbInformation.area!=this.conditionalAreaForBuild)||(this.mawbInformation.area==this.conditionalAreaForBuild&&this.childguid!=-1)){
                  var data={}
         if(!model){
          if(type=="1"&&dom=="1"){
    ...
    addCost(type,dom,data){
      ...
      if(type=="1"&&dom=="1"){
        ...
        if(this.mawbInformation.area==this.conditionalAreaForBuild&&this.childguid==-1){
         this.showContact=true
...
return settname
    ...
    wageOutCarryIn(){
       this.$axios({method:'POST',url:this.$store.state.webApi+'api/ExBoBaseCopyWage',data:{boguid:this.boguid},loading:false,tip:false,noarea:'1'})
       .then(results=>{
           if(results.data.resultstatus==0){
            this.$message.success(results.data.resultmessage)
            this.mawbCost()
