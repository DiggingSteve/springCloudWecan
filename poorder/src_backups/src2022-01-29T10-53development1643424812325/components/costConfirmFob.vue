...
<!-- 总单费用制作非合并 -->
 <el-dialog
        ...
        center>
<!--         @close="uncostMaking" -->
        <costmaking :mawbguid="mawbguid" @unvisible="uncostMaking" :boguid="boguid" ref="costDialog" :isFromFob="Fob" :area="area" :gid="wtgid" :initiator="70" :visible.sync="costMaking"></costmaking>
        <!-- <span slot="footer">
...
if(this.monitor=='1'){
  search.overseasacc={'in':'-1,20'}
}else{
  search.overseasacc={'in':'1'}
}

//search.confirmstatus_in={'in':'1'}
//search.hawbzdpiece={'begin':'1'}
            var jsonArr2={
              where:search,
            ...
            this.$axios({method:'get',url:this.$store.state.webApi+'api/ExHpoboAxplineOverseas1',params:json,loading:true,tip:true}).then(results=>{
              //console.log(results)
