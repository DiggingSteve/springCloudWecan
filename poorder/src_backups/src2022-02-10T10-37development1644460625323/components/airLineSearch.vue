    ...
    <div>
        <div class="page-container-box">
            <newSearchComp :name="name" :view-data.sync="inputViewData" :model-data.sync="inputModelData"
                :search-data.sync="searchData" @reset="reset" :pageshow.sync='pageshow' :chinese-where.sync="chineseWhere"
                :pagetype="1">
            </newSearchComp>
...
{{dataGrouping}} -->
        ...
        <tableCompt :name="name" :table-data-res="tableDataRes" :nestedTableType="2" :ziTableData="ziTableData"
            :chinese-where="chineseWhere" showZiConfig :showTotal="showTotal" :dataGrouping="dataGrouping" isGroup
            :isPaging="false" showMargeColumn ref="tableComponet" v-on:rightF="rightF" :explainInfoArr="explainInfoArr"
            @leftF="leftF" style="margin-bottom:40px;" :yellowMargeFixed="false" :isSort="false" @transSelIndex="openDialog">
            ...
            <template slot="mawb" slot-scope="props">
                ...
                <i v-if="props.data.row.hawblist&&props.data.row.hawblist.length>0"
                    ...
                    @click="openZiTable(props.data.row.index,props.data.row.hawblist)">
                </i>

                <!--  <span>{{props.data.value}}<span style="color:red" v-if="props.data.row.commbillmodifystatus==2">(待)</span></span> -->
            </template>

            <!-- <template slot="pono" slot-scope="props">

                <span class="poczColor" :class="{underlineRred:props.data.index==selectTableIndex}"
            ...
            <template slot="dzstatus" slot-scope="props">
                <span
                    :style="{'color':props.data.value=='签单申请'?'red':props.data.value=='签单确认'?'green':'#1a7dbf','cursor':'pointer'}"
                    @click="sizeRowData(props.data.row.boguid)">{{props.data.value}}</span>
            </template>

        </tableCompt>

        <!-- 主菜单右键 -->
        <el-card class="box-card" v-if="rightM&&opentype=='1'"
            :style="{'position':'fixed','left':position.left,'top':position.top,'width':'150px','zIndex':'999999999'}">
            <p @click="inPc(3)"
                :class="[rowData.pcstatus>=100||!rowData.hbh||!rowData.hbrq||!rowData.mawb?'disabled':'']"><i
                    class="el-icon-download"></i>配舱</p>
            <p @click="inPc(3,'out')" :class="[rowData.pcstatus!=100?'disabled':'']"><i class="el-icon-upload2"></i>返舱
            </p>
            <p @click="addShipPcCw"
                :class="[rowData.pcstatus>=100||!rowData.hbh||!rowData.hbrq||!rowData.mawb?'disabled':'']">
                <i class="el-icon-folder-add"></i>新增并配舱
            </p>
            <p @click="addShipPc('2')" :class="[rowData.pcstatus!='100'?'disabled':'']"><i
                    class="el-icon-edit"></i>舱位信息修改</p>
            <p @click="allLock('1')" :class="[rowData.pcstatus!='100'?'disabled':'']"><i class="el-icon-lock"></i>舱位锁定
            </p>
            <p :class="[rowData.pcstatus!='200'?'disabled':'']" @click="jjSet"><i class="el-icon-setting"></i>设置交接</p>
            <p :class="[rowData.pcstatus!='400'?'disabled':'']" @click="unSet"><i
                    class="el-icon-document-remove"></i>取消交接</p>
            <p :class="[rowData.pcstatus<'100'?'disabled':'']" @click="showCwPc"><i
                    class="el-icon-picture-outline"></i>舱位照片</p>
        ...
        <!--  子菜单右键-->
        <el-card class="box-card" v-if="rightM&&opentype=='2'"
            :style="{'position':'fixed','left':position.left,'top':position.top,'width':'150px','zIndex':'999999999'}">
            <p @click="modInfo"><i class="el-icon-edit"></i>上榜信息修改</p>
            <!-- 	<p  @click="inPc(2)"><i class="el-icon-download"></i>配舱</p> -->
            <p @click="addShipPc('3')"
                :class="[getStatusNum('配舱状态',rowData.pcstatus)>=100||!rowData.hbh||!rowData.hbrq||!rowData.mawb||(addShipPcList&&shipaceDataList.length)?'disabled':'']">
                <i class="el-icon-bottom"></i>新增并配舱</p>
            <p @click="inPc(2,'out')"
                :class="[getStatusNum('配舱状态',rowData.pcstatus)!='100'&&getStatusNum('配舱状态',rowData.pcstatus)!='300'?'disabled':'']">
                <i class="el-icon-upload2"></i>单票返舱</p>
            <!-- <p  @click="outPc(2)"><i class="el-icon-upload2"></i>返舱</p> -->
            <!-- 	<p :class="[(getStatusNum('单证状态',rowData.dzstatus)>=300||rowData.mawb)&&(!rowData.mawb&&rowData.commbillmodifystatus=='1')?'disabled':'']" @click="installMawb"><i class="el-icon-folder-checked"></i>配置总运单</p> -->
            <p :class="[(rowData.mawb)||(getStatusNum('单证状态',rowData.dzstatus)>=300&&rowData.commbillmodifystatus!='2')?'disabled':'']"
                @click="installMawb"><i class="el-icon-folder-checked"></i>配置总运单</p>

            <p :class="[(rowData.commbillmodifystatus!='2'&&(getStatusNum('单证状态',rowData.dzstatus)>=300||!rowData.mawb))?'disabled':'']"
                @click="unstallMawb"><i class="el-icon-folder-delete"></i>解除总运单</p>
            <p @click="xnlhShow" :class="[getStatusNum('配舱状态',rowData.pcstatus)!=200?'disabled':'']"><i
                    class="el-icon-arrow-down"></i>虚拟拉货</p>
            <p :class="[getStatusNum('配舱状态',rowData.pcstatus)!=300?'disabled':'']" @click="lhRestore"><i
                    class="el-icon-refresh"></i>拉货恢复</p>
            ...
            <p @click="airDetail"
                :class="[!rowData.mawb||!rowData.hbh||!rowData.hbrq||rowData.czlx=='代操作'||rowData.czlx=='唯凯代操作'?'disabled':'']">
                <i class="el-icon-document-checked"></i>航线签单</p>
            <p :class="{'disabled':!rowData.mawb}" @click="getErChen(rowData.mawb)">
                <i class="el-icon-search"></i>二程查询
            </p>
        </el-card>
        ...
        <el-dialog title="航线综合查询" v-if="dialogShow" center :visible.sync="dialogShow" class="dialogPage"
            :close-on-click-modal="false" :close-on-press-escape="false" width="100%" top="0px" :modal="false">
            <mawbaddput :visible.sync="dialogShow" :pagetype='15' :mawbguid="mawbguid" :boguid="boguid" :area="area"
                :orderdom="orderdom"></mawbaddput>
        </el-dialog>

        <!-- 舱位查询 -->
        <el-dialog title="舱位查询" :visible.sync="cwDialog" class="addPcDialog" :close-on-click-modal="false"
            v-if="cwDialog" width="1300px">
        ...
        <el-dialog title="总运单管理" :visible.sync="mawbManageDialog" class="addPcDialog" :close-on-click-modal="false"
            v-if="mawbManageDialog" width="1300px">
            <mawbmanage :visible.sync="mawbManageDialog"></mawbmanage>

        </el-dialog>

        <!-- 配舱 -->
        <el-dialog :visible.sync="inPcDialog" class="addPcDialog" :close-on-click-modal="false" v-if="inPcDialog"
            width="1100px">
        ...
        <el-dialog :visible.sync="newCW" v-if="newCW" width="1100px" append-to-body>
            ...
            <addNewCw :visible.sync="newCW" :operationType="operationType" :shipaceData="shipaceData"
                @reSearch="upDateRow" :shipaceDataList="shipaceDataList"></addNewCw>
        </el-dialog>

        <!-- 上榜信息修改 -->
        <el-dialog :visible.sync="modInfoDialog" v-if="modInfoDialog" width="600px" :title="'上榜信息修改'+infoData.pono"
            append-to-body>
            <!-- <h4 style="margin-bottom:20px;">
           	上榜信息修改
           </h4> -->
        ...
        <!-- 虚拟拉货 -->
        <el-dialog :visible.sync="xnlhDialog" width="600px" append-to-body
            @close="xnlhData={truckreason:'',reasontype:'虚拟拉货',truckman:'',truckmawb:'',truckremark:'',truckdate:'',modifyman:'','modifydate':''}">
            <h4 style="margin-bottom:20px;">
                虚拟拉货 总运单号:{{rowData.mawb}}
            </h4>
            <newFormCmpt :view-data.sync="xnlhViewModel" :model-data.sync="xnlhData" style="margin-left:-30px;">
            </newFormCmpt>
        ...
        <!-- 配置总运单弹框 -->
        <el-dialog :visible.sync="dialogMawbConfig" v-if="dialogMawbConfig" width="1100px" append-to-body title="配置运单" custom-class="mawbDialog">
                    ...
                    <table id="ybcc">
                        <tr v-for="(i,index) in sizeData.ybList">
                            <td v-for="(i2,index2) in i">
                                ({{2*index+index2+1}}){{Number(i2.vol_longs_yb)}}*{{Number(i2.vol_widths_yb)}}*{{Number(i2.vol_heights_yb)}}*{{Number(i2.vol_piece_yb)}}={{StringNum(Number((i2.vol_longs_yb*i2.vol_widths_yb*i2.vol_heights_yb))*Number(i2.vol_piece_yb))}}{{i2.packagetypename_yb}}
                            </td>
                        </tr>
                ...
                <div class="info">
                    <table id="sjcc">
                        <tr v-for="(i,index) in sizeData.sjList">
                            <td v-for="(i2,index2) in i">
                                ({{2*index+index2+1}}){{Number(i2.vol_longs)}}*{{Number(i2.vol_widths)}}*{{Number(i2.vol_heights)}}*{{Number(i2.vol_piece)}}={{StringNum(Number((i2.vol_longs*i2.vol_widths*i2.vol_heights))*Number(i2.vol_piece))}}{{i2.packagetypename}}
                            </td>
                        </tr>
      ...
      <span style="width:300px;margin-right:100px;margin-bottom:8px;display:inline-block">
        ...
        <!-- 舱位照片 -->
        <el-dialog :visible.sync="dialogImg" class="dialogImgDoc" :close-on-click-modal="false" v-if="dialogImg"
            width="980px" append-to-body top="0">
            <iframe :src="imgUrl" frameborder="0" allowtransparency="true"
                style="width: 100%; height: 100%; border: 0px none;" v-if="rowData.shipaceguid"></iframe>
        </el-dialog>
        <!-- 交接弹框 -->
        <el-dialog :visible.sync="jjDialog" v-if="jjDialog" width="600px" append-to-body title="设置交接并带入总运单号">
            ...
            <div style="margin-bottom:12px;" class="input-required">
                ...
                <span style="display:inline-block">
                    <el-date-picker v-model="jjData" type="date" placeholder="选择日期" style="width:400px;">
                    </el-date-picker>
                </span>
            </div>
    ...
    <el-form>
      <el-form-item label="驳回类型：">
        <my-select :groupid="135" class="input-required" v-model="orderCancelForm.rollbacktype"></my-select>
      </el-form-item>
      ...
      <el-form-item style="text-align:right;margin-top:12px;">
        <el-button btnnum="540" @click="orderCancel=false" type="primary" plain>取消</el-button>
        <el-button btnnum="550" type="primary" plain @click="cancelOrderFunc">确定</el-button>
      </el-form-item>
    </el-form>
  </el-dialog> -->

        <cancelOper :orderCancel.sync="orderCancel" v-if="orderCancel" dom=2
            :inputModelData="tableDataRes.find(i=>i.checked)" @success="newCancelOrderFunc"></cancelOper>
...
<script>
    ...
    import airPlaneDeatil from './orderDetails/airPlaneDeatil'
    ...
    export default {
        ...
        getErChen(mawb){
                ...
                let data={"data":{"TypeStr":"TRACK","UserLanguage":"zh-cn","MawbNo":mawb}}
                  this.$axios({
                      method:'POST',
                      url: "/ercheng/Webservice/WSMawbSystem.asmx/WSTrackTrace",
                      data:data,
                      loading: true,
                      tip: false,
                      emulateJSON:true
                  ...
                  .then(results=>{
                      if(results.data.d){
                          window.open(`http://www.mawb.cn/${results.data.d}`)
                      }
                  })
        },
            openZiTable(index, data) {
                ...
                if (this.ziTableIndex == index) {
                    ...
                    this.$axios({
                        ...
                        params: {
                            hpoidHawbList: this.tableDataRes[index].guid,
                            boguid: this.tableDataRes[index].boguid
                        },
                        loading: true,
                        tip: false
                    }).then(results => {
            ...
            //航线签单详细
            airDetail() {
                if (!this.rowData.mawb || !this.rowData.hbh || !this.rowData.hbrq || this.rowData.czlx == '代操作' || this.rowData.czlx == '唯凯代操作') {
                    return false
                }
            ...
            cancelOrderFunc() {
                ...
                json.boguid = this.tableDataRes.filter(i => {
                    return i.checked
                }).map(i => {
                    return i.boguid
                }).join(',')
                json.sid = this.tableDataRes.filter(i => {
                    return i.checked
                }).map(i => {
                    return i.guid
                }).join(',')
                json.pono = this.tableDataRes.filter(i => {
                    return i.checked
                }).map(i => {
                    return i.pono
                }).join(',')
                status = this.tableDataRes.filter(i => {
                    return i.checked
                }).map(i => {
                    ...
                    .then(results => {
                        ...
                        this.sizeData.storeList.forEach(i => {
                            if (sjList.indexOf(i.vol_guid) == '-1' && i.vol_guid != 0) {
                                // i.vol_heights=Number(i.vol_heights).toFixed(2)
                                // i.vol_longs=Number(i.vol_longs).toFixed(2)
                                // i.vol_widths=Number(i.vol_widths).toFixed(2)
                        ...
                        this.sizeData.storeYbList.forEach(i => {
                            if (ybList.indexOf(i.vol_guid_yb) == '-1' && i.vol_guid_yb != 0) {
                                // i.vol_heights_yb=Number(i.vol_heights_yb).toFixed(2)
                                // i.vol_longs_yb=Number(i.vol_longs_yb).toFixed(2)
                                // i.vol_widths_yb=Number(i.vol_widths_yb).toFixed(2)
            ...
            modInfo() {
                ...
                this.infoData.outwageallinprice=this.infoData.outwageallinprice=='666666'?'':this.infoData.outwageallinprice
            ...
            xnlhShow() {
                ...
                if (this.getStatusNum('配舱状态', this.rowData.pcstatus) != '200') {
            ...
            installMawb() {
                ...
                if ((this.rowData.mawb) || (this.getStatusNum('单证状态', this.rowData.dzstatus) >= 300 && this.rowData.commbillmodifystatus != '2')) {
            ...
            unstallMawb() {
                ...
                if (this.rowData.commbillmodifystatus != '2' && (this.getStatusNum('单证状态', this.rowData.dzstatus) >= 300 || !this.rowData.mawb)) {
            ...
            search(type) {
                ...
                this.$axios({
                    method: 'get',
                    url: this.$store.state.webApi + 'api/ExHpoAxplineMateShipspace',
                    params: json,
                    loading: true,
                    tip: false
                }).then(results => {
                    ...
                    //console.log(this.searchDataBack)
                    ...
                    //data=this.dataSort(data,['hbrq','qfsj','hbh'])
                    console.log(data)
