...
export const createPage = function (name) {
  ...
  if (["listConfirmOut", "orderTaking", "orderTakingOverDate", 'cabinUnconfirmed', 'listConfirmOverDate'].includes(name)) {
    return {
      name: name,
      mixins: [acceptPage],
      data() {
  ...
  if (Object.keys(cassifyObj).includes(name)) {
    ...
    return {
      ...
      <el-button-group  class="buttonTabs" >
          ...
          <el-button @click.native="mng=false" :class="[!mng?'active':'']" >
      ...
      <keep-alive>
        ...
        <pagesecond class="cre" pagetype="2" :monitor="${!second.nomonitor}?'2':''" ${second.propsStr}  v-if="!mng" ref="sec" @getTableData="handleData"></pagesecond>
      </keep-alive>
