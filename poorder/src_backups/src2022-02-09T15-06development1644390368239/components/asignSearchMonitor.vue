...
<div>
    <div class="page-container-box">
      ...
      {{searchData}} -->
        ...
        <div class="searchForm">
            ...
            <!-- <el-button @click="cwDialog=true" disabled>舱位查询</el-button>
            <el-button @click="inPc(1)" disabled>批量配舱</el-button>
            <el-button @click="inPc(1,'out')" disabled>批量返舱</el-button>
            <el-button  @click="backKfFunc" disabled>撤单/驳回/退关</el-button>
            <el-button @click="mawbManageDialog=true" disabled>总运单管理</el-button> -->
        </div>
    ...
    <el-card class="box-card" v-if="rightM&&opentype=='2'" :style="{'position':'fixed','left':position.left,'top':position.top,'width':'150px','zIndex':'999999999'}">
        <p class="disabled"  @click="modInfo">
            <i class="el-icon-edit"></i>上榜信息修改
        </p>
        <!--  <p  @click="inPc(2)"><i class="el-icon-download"></i>配舱</p> -->
        <p @click="addShipPc('3')" class="disabled" :class="[getStatusNum('配舱状态',rowData.pcstatus)>=100||!rowData.hbh||!rowData.hbrq||!rowData.mawb||(addShipPcList&&shipaceDataList.length)?'disabled':'']">
            <i class="el-icon-bottom"></i>新增并配舱
        </p>
        <p @click="inPc(2,'out')" class="disabled" :class="[getStatusNum('配舱状态',rowData.pcstatus)!='100'&&getStatusNum('配舱状态',rowData.pcstatus)!='300'?'disabled':'']">
        ...
        <p class="disabled" :class="[(rowData.mawb)||(getStatusNum('单证状态',rowData.dzstatus)>=300&&rowData.commbillmodifystatus!='2')?'disabled':'']" @click="installMawb">
            <i class="el-icon-folder-checked"></i>配置总运单
        </p>
        <p  class="disabled" :class="[(rowData.commbillmodifystatus!='2'&&(getStatusNum('单证状态',rowData.dzstatus)>=300||!rowData.mawb))?'disabled':'']" @click="unstallMawb">
            <i class="el-icon-folder-delete"></i>解除总运单
        </p>
        <p class="disabled" @click="xnlhShow" :class="[getStatusNum('配舱状态',rowData.pcstatus)!=200?'disabled':'']">
            <i class="el-icon-arrow-down"></i>虚拟拉货
        </p>
        <p  class="disabled" :class="[getStatusNum('配舱状态',rowData.pcstatus)!=300?'disabled':'']" @click="lhRestore">
    ...
    <el-dialog title="航线综合查询" v-if="dialogShow3" center :visible.sync="dialogShow3" class="dialogPage" :close-on-click-modal="false" :close-on-press-escape="false" width="100%" top="0px" :modal="false">
        <airPlaneDeatil :boguid="boguid" :visible.sync="dialogShow3" monitor="2"></airPlaneDeatil>
    </el-dialog>
    ...
    <!-- 配置总运单弹框 -->
    <el-dialog :visible.sync="dialogMawbConfig" v-if="dialogMawbConfig" width="1100px" top="4%" append-to-body title="配置总单">
        <div class="detail">
    ...
    <!-- 总运单维护弹框 -->
    <el-dialog :visible.sync="mawbDialog" width="1300px" top="4%" v-if="mawbDialog" :close-on-press-escape="false" append-to-body>
...
export default {
    name: "asignSearchMonitor",
    mixins: [systemCheck, mawbConfigMixin(), searchCmptMixins],
    components: {
        airPlaneDeatil,
        cwSearch,
        addNewCw,
    ...
    methods: {
        ...
        search(type) {
            ...
            this.$axios({
                    method: "get",
                    url: this.$store.state.webApi + "api/ExHpoAxplineMateShipspaceMonitor",
                    params: json,
                    loading: true,
                    tip: false
                })
    ...
    created() {
        ...
        // } else {
        //     this.$set(this.inputModelData, 'dzstatus', 'AO0030')
        // }
