<template>
  <div class="costDetail" >
    <!-- <infoList :inputModelData="allmaindata" v-if="allmaindata.guid"  :ifopendetail="false"></infoList> -->
    <!-- 11111111
{{initiator}} -->
    <div class="pagetype1" style="margin-top:12px;text-align: right;">
 
        <span style="margin-right:10px;float:right;margin-top:-27px;">
          <setBhicon></setBhicon> 
          <el-switch
            v-model="lineshu"
        ...
        <!-- <span style="position: fixed;margin: -62px -170px;">
          <setBhicon></setBhicon> 
          <el-switch
            v-model="lineshu"
            inactive-color="#009900"
    ...
    <div v-if="!lineshu">
      <div
        style="padding-bottom:8px;border:1px solid #e4e7ed;height:32px;line-height:30px;margin-bottom:12px;border-left:2px solid #00cc00;background:#f8f8f8;padding-left:16px;"
      >
        <div style="float:left;margin-right:16px;letter-spacing:0.1em">
          <span style="color:#00cc00;font-weight:bold;">总应收:</span>
          <span
            v-for="(item,index) in currencyCost.charge"
            :style="{'color':'#00CC00','margin-left':index>0?'15px':'','font-weight':'bold'}"
          >
        ...
        <span
          style="float:right;color:orange;font-weight:bold;letter-spacing:0.1em;font-size:14px;margin-left:12px;margin-right:8px;"
        >平均每公斤毛利:￥{{Number(mawbInformation['accountweightin'])=='0'?'0.00':Number(Number(zdtotalCharge-zdtotalPayment)/Number(mawbInformation['accountweightin'])).toFixed(2)}}</span>
        <span
          v-if="!isFromFob"
          style="float:right;color:orange;font-weight:bold;letter-spacing:0.1em;font-size:14px;"
        >折合人民币盈利:￥{{Number(zdtotalCharge-zdtotalPayment).toFixed(2)}}</span>
      </div>
      ...
      <div class="detail mawbCharge detailPaneYoung">
        <el-row class="detail-title">
          ...
          <el-col
            :span="yingshouStr.length>0?21:21"
            style="font-size:12px;font-weight:bold;letter-spacing:0.1em;text-indent:0"
          >
            <div
              style="width:2%;display:inline-block;vertical-align:top;font-size:18px;color:#999"
              class="arrow"
            >
              <i
                class="el-icon-caret-left"
                style="margin-right:-8px;"
                @click="changeDirectionAcn(1,1)"
              ></i>
            </div>
            <div style="width:94%;overflow:hidden;display:inline-block" class="wrapSetname">
              <div style="width:10000px" class="setnameAll">
                <span
                  style="width:160px;text-overflow:ellipsis;overflow:hidden;white-space:nowrap"
                  v-for="(item,index) in yssettname"
                  @click="handleSettname(1,item.gid,index)"
                  :class="[activeSettnameYs==item.gid&&item.status=='700'?'activeset':activeSettnameYs==item.gid?'activesetUc':'']"
                  :style="{'background':item.status=='700'?'#4c738d':'','border-radius':'32px','color':item.status=='700'?'#fff':'#999'}"
                  :title="item.settname"
                >{{item.settname}}</span>
              </div>
            </div>
        ...
        <div class="detail-c">
          ...
          <commonTable
            ...
            :table-data="mawbCharge"
            style="position:relative;margin-top:26px;"
            :tableIndex="0"
            v-if="mawbCharge"
            :colorIndex="modCostIndex"
          ...
          >
            <template slot="content">
              <div style="position:absolute;top:-28px">
                ...
                <span
                  v-for="(item,index) in currentGidCost.charge"
                  :style="{'color':'#00CC00','margin-left':index>0?'15px':'','font-weight':'bold'}"
                >
                  <span style="margin-right:8px;">{{item.currencyFh}}</span>
      ...
      <i class="el-icon-shenhebohui" v-if="(initiator==30?true:props.data.row.status!=-1)" :style="setBhiconColor(props.data.row.status)" :title="setBhiconColor(props.data.row.status,2)" @click="getBohuiWageDetail(props.data.row)"></i>
            </template>
          </commonTable>
        </div>
      </div>
      ...
      <div class="detail mawbPayment detailPaneRed costTable1" v-if="!isFromFob">
        <div
          style="padding-bottom:8px;border:1px solid #e4e7ed;height:32px;line-height:30px;margin-bottom:12px;border-left:2px solid red;background:#f8f8f8;padding-left:16px;"
        >
          <div style="float:left;margin-right:16px;letter-spacing:0.1em">
            <span style="color:red;font-weight:bold;">总应付:</span>
            <span
              v-for="(item,index) in currencyCost.payment"
              :style="{'color':'red','margin-left':index>0?'15px':'','font-weight':'bold'}"
            >
              <span style="margin-right:8px;">{{item.currencyFh}}</span>
              <span :title="item.currency">{{item.money}}</span>
            </span>
          ...
          >
            ...
            <div style="width:94%;overflow:hidden;display:inline-block" class="wrapSetname">
              <div style="width:10000px" class="setnameAllf">
                <span
                  style="width:160px;text-overflow:ellipsis;overflow:hidden;white-space:nowrap"
                  v-for="(item,index) in yfsettname"
                  @click="handleSettname(2,item.gid,index)"
                  :class="[activeSettnameYf==item.gid&&item.status=='700'?'activeset':activeSettnameYf==item.gid?'activesetUc':'']"
                  :style="{'background':item.status=='700'?'#4c738d':'','border-radius':'32px','color':item.status=='700'?'#fff':'#999'}"
                  :title="item.settname"
                >{{item.settname}}</span>
              </div>
        ...
        <div class="detail-c">
          <commonTable
            class="commonTable"
            :head="mawb_table_model"
            :table-data="!applyModel?mawbPayment:otherWageoutList"
            style="margin-top:26px;position:relative"
            :tableIndex="1"
            v-if="mawbPayment"
            :colorIndex="modCostIndex"
            :wagetype="wagetype"
          >
            <template slot="content">
              <div style="position:absolute;top:-28px;">
                ...
                <span
                  v-for="(item,index) in currentGidCost.payment"
                  :style="{'color':'red','margin-left':index>0?'15px':'','font-weight':'bold'}"
                >
                  <span style="margin-right:8px;">{{item.currencyFh}}</span>
                  <span :title="item.currency">{{Number(item.money).toFixed(2)}}</span>
    ...
    <div v-if="lineshu&&!isFromFob" class="detailCost2">
      <div class="lineYinFu">
        ...
        <div class="tableContent">
          ...
          >
            ...
            <table
              :style="{'border-collapse':'collapse','table-layout':'fixed','width':'100%','margin-top':'12px'}"
            >
              ...
              <tr
                v-for="(item,index) in mawbPayment"
                :style="{'backgroundColor':index==modCostIndex&&wagetype=='2'?'#F56C6C':item.isfree=='是'?'#e2fdde':'','height':'32px','color':index==modCostIndex&&wagetype=='2'?'#fff':''}"
              >
        ...
        <div class="tableContent">
          ...
          >
            ...
            >
              ...
              <tr
                v-for="(item,index) in mawbChargeLine"
                :style="{'backgroundColor':index==modCostIndex&&wagetype=='1'?'#F56C6C':item.isfree=='是'?'#e2fdde':'','color':index==modCostIndex&&wagetype=='1'?'#fff':''}"
                style="height:32px;"
              >
                <td width="50">{{item.isfree}}</td>
      ...
      <div class="lineAllin">
        ...
        <div class="tableContent">
          <div
            :style="{'width':'188px','overflow':'auto','height':tableHeight,'position':'relative'}"
          >
            <p
              style="font-weight:bold;font-size:12px;color:#fff;background:orange;height:42px;line-height:42px;width:100%;"
            >盈利(折合人民币):￥{{Number(zdtotalCharge-zdtotalPayment).toFixed(2)}}</p>
            ...
            >
              ...
              <tr v-for="(item,index) in toolTip" style="height:32px;">
                <td
                  v-if="item!='抄录应付'"
                  :style="{'color':Number(item)<0?'red':Number(item)>0?'#00CC00':''}"
                >{{item}}</td>
                <td v-else></td>
              </tr>
            </table>
      ...
      >
        <template slot="firstTd" slot-scope="props">
          ...
          <td style="display:inline-flex;border:none;width:100%">
            <page-select
              :modelval.sync="curObjectKyYs.gid"
              :pagetype="2"
              @change="getYsSettname"
              :area="mawbInformation.area"
              v-if="mawbInformation.area"
            ></page-select>
          </td>

          <td>
            <page-select
              :modelval.sync="curObjectKyYs.items"
              :pagetype="5"
              :area="mawbInformation.area"
              v-if="mawbInformation.area"
            ></page-select>
    ...
    >
      ...
      >
        <template slot="firstTd" slot-scope="props">
          ...
          <td style="display:inline-flex;border:none;width:100%;">
            <page-select
              :modelval.sync="curObjectKyYf.gid"
              :pagetype="3"
              @change="getYfSettname"
              :area="mawbInformation.area"
            ></page-select>
          </td>
          <td>
            <page-select
              :modelval.sync="curObjectKyYf.items"
              :pagetype="5"
              :area="mawbInformation.area"
            ></page-select>
          </td>
          ...
          <td>
            <el-input
              v-model="curObjectKyYf.num"
              style="width:100px;"
              @blur="yanzhenzzF('2','1','num')"
            ></el-input>
          </td>
          <td>
            <el-input
              v-model="curObjectKyYf.price"
              style="width:100px;"
              @blur="yanzhenzzF('2','1','price')"
            ></el-input>
          </td>
          <td>
    ...
    >
      ...
      <el-input
        ...
        :placeholder="methods=='PUT'?'修改理由':methods=='POST'?'新增理由':'删除理由'"
      ></el-input>
      ...
      width="1360px"
      :title="`驳回详情(${wageBohuiData[0].wagecommitlist[0].wageinout==1?'应收':'应付'})`"
      top="4%"
      v-if="bohuiVisible&&allmaindata.boguid"
    ...
    <template v-for="(item,index) in wageBohuiData">
      <myCommonTable v-show="childBohuiChecked==index" :ifNeedFrashWage.sync="ifNeedFrashWage" v-if="childBohuiCheckedList.includes(index)&&allmaindata.boguid" :mawbinfo="allmaindata" :wageBohuiData.sync="wageBohuiData[index]" :key="index" :initiator="initiator"></myCommonTable>
    </template>
    </div>
</el-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      translate_text: {
        ...
        ybtjz: "总单预报体/重/件：",
        zdtjz: "总单制单件重体：",
        accountweightin: "应收结算重量：",
        accountweightout: "应付结算重量：",
        realtjz: "实际件/重/体：",
        zdjfweight: "制单计费重量：",
        realjfweight: "实际计费重量：",
        isinwageallin: "应收运价类型：",
        inwageallinprice: "应收运费价格：",
        fpbl: "应收运费分泡比例："
      },
      mawb_table_model: [
      ...
      if (results.data) {
        ...
        Object.keys(this.mawbInformation).forEach(key => {
          if (results.data[key]) {
            ...
            this.mawbInformation[key] =
              (results.data["ybpiece"] ? results.data["ybpiece"] : "0") +
              "/" +
              (results.data["ybweight"]
                ? Number(results.data["ybweight"]).toFixed(2)
                : "0") +
              "/" +
              (results.data["ybvolume"]
                ? Number(results.data["ybvolume"]).toFixed(3)
                : "0");
          } else if (key == "realtjz") {
            this.mawbInformation[key] =
              (results.data["realpiece"] ? results.data["realpiece"] : "0") +
              "/" +
              (results.data["realweight"]
                ? Number(results.data["realweight"]).toFixed(2)
                : "0") +
              "/" +
              (results.data["realvolume"]
                ? Number(results.data["realvolume"]).toFixed(3)
                : "0");
          } else if (key == "zdtjz") {
            this.mawbInformation[key] =
              (results.data["zdpiece"] ? results.data["zdpiece"] : "0") +
              "/" +
              (results.data["zdweight"]
                ? Number(results.data["zdweight"]).toFixed(2)
                : "0") +
              "/" +
              (results.data["zdvolume"]
                ? Number(results.data["zdvolume"]).toFixed(3)
                : "0");
          }
        });
        ...
        this.mawbInformation["accountweightin"] = Number(
          results.data.accountweightin
        ).toFixed(2);
        this.mawbInformation["hawb"] = results.data.hawb;
        this.mawbInformation["accountweightout"] = Number(
          results.data.accountweightout
        ).toFixed(2);
        this.mawbInformation["jfweight"] = Number(
          results.data.jfweight
        ).toFixed(2);
        ...
        this.mawbInformation["realjfweight"] = computedWeight(
          results.data.realvolume,
          results.data.realweight
        );
        this.mawbInformation["zdjfweight"] = computedWeight(
          results.data.zdvolume,
          results.data.zdweight
        );
        this.mawbInformation["wageconfirmstatus"] =
          results.data.wageconfirmstatus;
        ...
        this.mawbInformation["chargeWeight"] =
          Number(results.data.realvolume) / 0.006 >
          Number(results.data.realweight)
            ? Number(results.data.realvolume / 0.006).toFixed(2)
            : Number(results.data.realweight).toFixed(2);
        ...
        if (this.inputModelData) {
          this.curObjectKyYf["gid"] =
            this.inputModelData.mawb && this.inputModelData.mawbguid > 0
              ? this.inputModelData.mawbguid
              : "";
        ...
        this.mawbCost();
        if(this.allmaindata.pono.indexOf('TZ')>=0){
          this.mawb_table_model.filter(i=>i.field=='otherno')['title']='调账号'
        }
  ...
  computed: {
    ...
    zdysWageAll() {
      if (this.curObjectKyYs.leastwageall && this.curObjectKyYs.changerate) {
        this.curObjectKyYs.wageall = Number(
    ...
    zdyfWageAll() {
      if (this.curObjectKyYf.leastwageall && this.curObjectKyYf.changerate) {
        this.curObjectKyYf.wageall = Number(
    ...
    zdysChangeRate() {
      ...
      if (this.curObjectKyYs.num && this.curObjectKyYs.price) {
        this.curObjectKyYs.leastwageall = Number(
          Number(this.curObjectKyYs.num) * Number(this.curObjectKyYs.price)
        ).toFixed(2);
        this.curObjectKyYs.wageall = Number(
          Number(this.curObjectKyYs.num) *
            Number(this.curObjectKyYs.price) *
            Number(getChangeRate(this.curObjectKyYs.currency))
        ).toFixed(2);
      }
      this.$forceUpdate();
    ...
    zdyfChangeRate() {
      ...
      if (this.curObjectKyYf.num && this.curObjectKyYf.price) {
        this.curObjectKyYf.leastwageall = Number(
          Number(this.curObjectKyYf.num) * Number(this.curObjectKyYf.price)
        ).toFixed(2);
        this.curObjectKyYf.wageall = Number(
          Number(this.curObjectKyYf.num) *
            Number(this.curObjectKyYf.price) *
            Number(getChangeRate(this.curObjectKyYf.currency))
        ).toFixed(2);
      }
      this.$forceUpdate();
    ...
    tableHeight() {
      ...
      if (this.mawbChargeLine.length > this.mawbPayment.length) {
        height =
          (this.mawbChargeLine.length + 1) * 50 +
          this.currencyCost.profit.length * 50 +
          "px";
      } else if (this.mawbPayment.length > this.mawbChargeLine.length) {
        height =
          (this.mawbPayment.length + 1) * 50 +
          this.currencyCost.profit.length * 50 +
          "px";
  ...
  watch: {
    ...
    "curObjectKyYs.wagetype"(val, oldval) {
      ...
      if (!oldval) {
        switch (val) {
          case "票":
            this.curObjectKyYs.num = this.curObjectKyYs.num
          ...
          case "计重":
            this.curObjectKyYs.num = this.curObjectKyYs.num
              ? this.curObjectKyYs.num
              : this.mawbInformation.realjfweight;
            break;
          case "结算重量":
            this.curObjectKyYs.num = this.curObjectKyYs.num
              ? this.curObjectKyYs.num
              : this.mawbInformation.accountweightin;
            break;
          case "毛重":
            this.curObjectKyYs.num = this.curObjectKyYs.num
              ? this.curObjectKyYs.num
              : this.mawbInformation.realtjz.split("/")[1];
            break;
          case "箱":
            this.curObjectKyYs.num = this.curObjectKyYs.num
              ? this.curObjectKyYs.num
              : this.mawbInformation.realtjz.split("/")[0];
            break;
        }
      } else {
        switch (val) {
    ...
    "curObjectKyYf.wagetype"(val, oldval) {
      ...
      if (!oldval) {
        switch (val) {
          case "票":
            this.curObjectKyYf.num = this.curObjectKyYf.num
          ...
          case "结算重量":
            this.curObjectKyYf.num = this.curObjectKyYf.num
              ? this.curObjectKyYf.num
              : this.mawbInformation.accountweightout;
            break;
          case "毛重":
            this.curObjectKyYf.num = this.curObjectKyYf.num
              ? this.curObjectKyYf.num
    ...
    "curObjectFdYf.wagetype"(val, oldval) {
      if (!oldval) {
        switch (val) {
          case "票":
            this.curObjectFdYf.num = this.curObjectFdYf.num
          ...
          case "计重":
            this.curObjectFdYf.num = this.curObjectFdYf.num
              ? this.curObjectFdYf.num
              : this.hawb_data_list[this.cur_hawb].hawbInformation.jfweight;
            break;
          case "结算重量":
            this.curObjectFdYf.num = this.curObjectFdYf.num
              ? this.curObjectFdYf.num
              : this.hawb_data_list[this.cur_hawb].hawbInformation
                  .accountweightin;
            break;
          case "毛重":
            this.curObjectFdYf.num = this.curObjectFdYf.num
              ? this.curObjectFdYf.num
              : this.hawb_data_list[this.cur_hawb].hawbInformation.ybweight;
            break;
        }
      } else {
        switch (val) {
  ...
  methods: {
    ...
    finishApply() {
      var data1 = this.otherWageinList.filter(i => {
        return i.ischanged == "999";
      });
      var data2 = this.otherWageoutList.filter(i => {
        return i.ischanged == "999";
      });

      var originData = JSON.parse(JSON.stringify(this.mawbCharge)).concat(
        JSON.parse(JSON.stringify(this.mawbPayment))
      );
      ...
      originData = originData.filter(i => {
        return !i.thisListShowNone;
      });
      var data = data3.concat(originData);
      console.log(data);
      this.$axios({
        method: "POST",
        url: this.$store.state.webApi + "api/ExBoBaseWageCommit",
        data: { boguid: this.boguid, wagelist: data, guid: this.mawbguid },
        loading: true,
        tip: false,
        noarea: 1
      })
        .then(results => {
          if (results.data.resultstatus == 0) {
            ...
            this.otherWageinList = JSON.parse(JSON.stringify(this.mawbCharge));
            this.otherWageoutList = JSON.parse(
              JSON.stringify(this.mawbPayment)
            );
    ...
    togglehawb(hawb_obj) {
      ...
      if (!this.hawb_data_list[this.cur_hawb]) {
        this.$set(this.hawb_data_list, this.cur_hawb, {
          hawbInformation: {},
          hawbCharge: [],
          hawbPayment: []
        });
    ...
    ysfConfirmCheck(type, val, status) {
      if (type == "1") {
        ...
        if (this.wageguids.includes(val)) {
          var template = this.mawbCharge.filter(i => {
            //筛选出当前勾选的list,以便获取相同结算对象
            return val == i.guid;
          });
          ...
          this.mawbCharge.forEach(item => {
            ...
            if (item.confirmstatus == "--") {
              //记录有多少未确认，当wageguids长度和num相同时勾上
              num += 1;
            ...
            if (status == "--") {
              ...
              if (template[0].gid == item.gid && item.confirmstatus == "--") {
                //当勾选状态为未确认遍历未确认和相同结算对象的

                if (!this.wageguids.includes(item.guid)) {
                  this.wageguids.push(item.guid);
                }
              } else if (
                template[0].gid == item.gid &&
                item.confirmstatus != "--"
              ) {
                //将不同费用状态list无法勾上
          ...
          if (wagestatus.indexOf("--") != -1 && num == this.wageguids.length) {
            //未确认数量与勾选数量相同时默认勾选
        ...
        if (this.wageguidf.includes(val)) {
          ...
          console.log(num2);
          if (wagestatus.indexOf("--") != -1 && num == this.wageguidf.length) {
            //未确认数量与勾选数量相同时默认勾选
    ...
    submit(status) {
      ...
      var json = {
        ...
        guid: this.boguid,
        area: this.mawbInformation.area,
        status: status
      };
      ////console.log(json)
      this.$axios({
    ...
    modCost(index, type, dom, show) {
      if (!this.applyModel) {
        if (type == "1" && dom == "1" && show != "1") {
          ...
          this.curObjectKyYs = JSON.parse(
            JSON.stringify(this.mawbCharge[index])
          );
          this.curObjectKyYs.gid = Number(this.curObjectKyYs.gid);
          this.curObjectKyYs.isfree =
            this.curObjectKyYs.isfree == "否" ? false : true;
          ...
          this.modCostIndex = index;
          this.wagetype = type;
          this.curObjectKyYf = JSON.parse(
            JSON.stringify(this.mawbPayment[index])
          );

          this.curObjectKyYf.isfree =
            this.curObjectKyYf.isfree == "否" ? false : true;
          if (show == 1) {
            this.yingfuMod = true;
          }
          ...
          this.curObjectKyYs = JSON.parse(
            JSON.stringify(this.mawbChargeLine[index])
          );
          this.curObjectKyYs.gid = Number(this.curObjectKyYs.gid);
          this.curObjectKyYs.isfree =
            this.curObjectKyYs.isfree == "否" ? false : true;
        ...
        if (type == "1" && dom == "1") {
          ...
          this.curObjectKyYs = JSON.parse(
            JSON.stringify(this.otherWageinList[index])
          );
          this.curObjectKyYs.gid = Number(this.curObjectKyYs.gid);
          this.curObjectKyYs.isfree =
            this.curObjectKyYs.isfree == "否" ? false : true;
          ...
          this.curObjectKyYf = JSON.parse(
            JSON.stringify(this.otherWageoutList[index])
          );
      ...
      if ($.trim(this.contact) == "" || $.trim(this.modifyReason) == "") {
        ...
        if (this.methods != "DELETE") {
          ...
          if (this.wagetype == "1") {
            ...
            this.curObjectKyYs.price = Number(this.curObjectKyYs.price).toFixed(
              2
            );
            this.curObjectKyYs.wageall = Number(
              this.curObjectKyYs.wageall
            ).toFixed(2);
            this.curObjectKyYs.changerate = Number(
              this.curObjectKyYs.changerate
            ).toFixed(4);
            this.curObjectKyYs.otherguid = this.getOtherGuid(
              this.curObjectKyYs.otherno
            );
            ...
            this.curObjectKyYf.price = Number(this.curObjectKyYf.price).toFixed(
              2
            );
            this.curObjectKyYf.wageall = Number(
              this.curObjectKyYf.wageall
            ).toFixed(2);
            this.curObjectKyYf.changerate = Number(
              this.curObjectKyYf.changerate
            ).toFixed(4);
            this.curObjectKyYf.otherguid = this.getOtherGuid(
              this.curObjectKyYf.otherno
            );
          ...
          if (!this.lineshu) {
            if (this.wagetype == "1" && this.wagedom == "1") {
              data = JSON.parse(
                JSON.stringify(this.mawbCharge[this.delCostIndex])
              );
            } else if (this.wagetype == "2" && this.wagedom == "1") {
              data = JSON.parse(
                JSON.stringify(this.mawbPayment[this.delCostIndex])
              );
            }
          } else {
            if (this.wagetype == "1" && this.lineshu) {
              data = JSON.parse(
                JSON.stringify(this.mawbChargeLine[this.delCostIndex])
              );
    ...
    delCost(index, type, dom, model) {
      ...
      if (!this.applyModel) {
        ...
        this.$confirm("是否删除此费用?", "提示", {
          ...
          .then(() => {
            if (
              this.mawbInformation.area != this.conditionalAreaForBuild ||
              (this.mawbInformation.area == this.conditionalAreaForBuild &&
                this.childguid != -1)
            ) {
              var data = {};
        ...
        if (type == "1" && dom == "1") {
          ...
          if (
            this.otherWageinList[index].ischanged == "999" &&
            this.otherWageinList[index].guid == "-1"
          ) {
            //申请新增费用的删除
          ...
          if (
            this.otherWageoutList[index].ischanged == "999" &&
            this.otherWageoutList[index].guid == "-1"
          ) {
    ...
    delAllYsCost() {
      this.$axios({
        method: "DELETE",
        url: "http://192.168.0.167/BoManagementWebApi/api/ExBoBaseWage",
        data: { boguid: this.boguid, area: this.mawbInformation["area"] },
        loading: false,
        tip: false
      })
        .then(results => {
    ...
    addCost(type, dom, data) {
      ...
      if (!this.applyModel) {
        ...
        if (type == "1" && dom == "1") {
          ...
          if (
              ...
              if (method == "PUT") {
                this.$axios({
                  ...
                  if (results.data.resultstatus == 0) {
                    ...
                    if (
                      templetadata.otherguid &&
                      templetadata.otherguid != "-1"
                    ) {
                      this.hawb_data_list[
                        templetadata.otherno
                      ].hawbPayment.push(templetadata);
                    }
                    this.modCostIndex = "-1";
                  } else {
                    this.$message.error(results.data.resultmessage);
                ...
                template = JSON.parse(JSON.stringify(this.curObjectKyYf));
                template.isfree =
                  this.curObjectKyYf.isfree == "1" ? "否" : "是";
                template.settname = this.mawbInformation.wtxmname;
                template.belong = 2;
                template.confirmstatus = "--";
            ...
            this.$axios({
              ...
              if (results.data.resultstatus == 0) {
                ...
                if (templetadata.otherguid && templetadata.otherguid != "-1") {
                  this.hawb_data_list[templetadata.otherno].hawbPayment.push(
                    templetadata
                  );
                }
                this.modCostIndex = "-1";
          ...
          //templetadata.isfree=templetadata.isfree==true?'2':'1'
          this.curObjectFdYf.price = Number(this.curObjectFdYf.price).toFixed(
            2
          );
          this.curObjectFdYf.wageall = Number(
            this.curObjectFdYf.wageall
          ).toFixed(2);
          this.curObjectFdYf.changerate = Number(
            this.curObjectFdYf.changerate
          ).toFixed(4);
          ...
          this.$axios({
            method: method,
            url: this.$store.state.webApi + "api/ExAxplineWage",
            data: templetadata,
            loading: false,
            tip: false
          }).then(results => {
        ...
        if (type == "1" && dom == "1") {
          ...
          if (!this.curObjectKyYs.settname) {
            this.curObjectKyYs.settname = this.gidChange(
              this.curObjectKyYs.gid,
              "1"
            ).split("-")[1];
          }
          ...
          if (this.curObjectKyYs.id) {
            ...
            this.curObjectKyYs.isfree =
              this.curObjectKyYs.isfree == "1" ? "否" : "是";
            this.$set(
              this.otherWageinList,
              this.modCostIndex,
              this.curObjectKyYs
            );
          ...
          if (!this.curObjectKyYf.settname) {
            this.curObjectKyYf.settname = this.gidChange(
              this.curObjectKyYf.gid,
              "2"
            ).split("-")[1];
          }
          ...
          if (this.curObjectKyYf.id) {
            ...
            //method=
            this.$set(
              this.otherWageoutList,
              this.modCostIndex,
              this.curObjectKyYf
            );
            //this.otherWageoutList[this.modCostIndex]=this.curObjectKyYf
            this.otherWageoutList[this.modCostIndex].method = "PUT";
            this.otherWageoutList[this.modCostIndex].ischanged = "999";
            this.otherWageoutList[this.modCostIndex].background = "#FFDEDE";
            ...
            data.isfree = data.isfree == "2" ? "是" : "否";
            if (data.ischanged == "999") {
              this.otherWageoutList[this.modCostIndex].background = "#D8FCD8";
    ...
    // 费用确认取消确认
    confirmButton(type, status, all) {
      //type 1应收 2应付 all应收总确认总取消
      if (!all) {
        if (
          type == "1" &&
          this.mawbCharge.length > 0 &&
          this.wageguids.length == 0
        ) {
          this.$message.error("请勾选应收结算对象");
          return false;
        } else if (
          type == "2" &&
          this.mawbPayment.length > 0 &&
          this.wageguidf.length == 0
        ) {
        ...
        var data = {
          ...
          guidList:
            type == "1" ? this.wageguids.join(",") : this.wageguidf.join(","),
          isyssys: "1"
        };
      } else {
      ...
      if (!all) {
        if (status == "700") {
          if (type == "1") {
            ...
            this.mawbCharge.forEach(item => {
              if (
                Number(item.wageall) == 0 &&
                item.isfree != "是" &&
                Number(item.isallin) == 2 &&
                this.wageguids.includes(item.guid)
              ) {
                num = 1;
              }
            });
            ...
            if (!all) {
                ...
                if (status == "700") {
                  this.mawbCharge.forEach(i => {
                    if (
                      i.yssys != 2 &&
                      i.confirmstatus == "--" &&
                      this.wageguids.includes(i.guid)
                    ) {
                      i.confirmstatus = "已确认";
                    }
                ...
                if (status == "700") {
                  this.mawbPayment.forEach(i => {
                    if (
                      i.yssys != 2 &&
                      i.confirmstatus == "--" &&
                      this.wageguidf.includes(i.guid)
                    ) {
                      i.confirmstatus = "已确认";
                    }
                    if (i.thisListHide == false) {
              ...
              if (status == "700") {
                ...
                if (type == "1") {
                  ...
                  this.allCostData.forEach(item => {
                    if (
                      (item.yssys == 2 && item.wageinout == "2") ||
                      this.wageguidf.includes(item.guid)
                    ) {
                      item.confirmstatus = "--";
                    }
                  });
    ...
    mawbCost() {
      ...
      this.$axios({
        method: "get",
        url: this.$store.state.webApi + "api/ExHpoAxplineWage",
        params: { sid: this.mawbguid, boguid: this.boguid },
        loading: true,
        tip: false
      }).then(results => {
      results.data=results.data.filter(i=>i.wagedom=='航线')
        this.allCostData = results.data.filter(i=>i.wagedom=='航线');
        ...
        results.data.forEach(i => {
          ...
          if (i.yssys != 2) {
            ...
            if (i.wageinout == 1) {
              ...
              if (
                i.isfree == "否" &&
                (i.confirmstatus == "已确认" || i.confirmstatus == "700")
              ) {
                totalCharge += Number(i.wageall);
              }
        ...
        if (ysconfirm.length > 0) {
          if (
            ...
            ysconfirm.indexOf("已提交") != -1 ||
            ysconfirm.indexOf("已审单") != -1
          ) {
            this.ysDz = false;
            this.ysUconfirm = true;
          } else {
        ...
        if (yfconfirm.length > 0) {
          if (
            ...
            yfconfirm.indexOf("已提交") != -1 ||
            yfconfirm.indexOf("已审单") != -1
          ) {
            this.yfDz = false;
            this.yfUconfirm = true;
          } else {
          ...
          this.mawbCharge.forEach(item => {
            if (!currencyAlone.includes(item.currency)) {
              ...
              this.mawbCharge.forEach((item3, index3) => {
                if (
                  item3.currency == item.currency &&
                  item3.isfree == "否" &&
                  (item3.confirmstatus == "已确认" ||
                    item3.confirmstatus == "700")
                ) {
                  numC += Number(item3.leastwageall);
                }
              });

              this.currencyCost.charge.push({
                currency: item.currency,
                money: numC.toFixed(2),
                currencyFh: this.currencyFuHao[item.currency]
              });
            }
          });
        }
        ...
        //         this.toolTip.push(Number((item.isfree=='否'?Number(item.wageall):0)-Number(this.mawbPayment[index].wageall)).toFixed(2))
    ...
    salesCost() {
      ...
      this.$axios({
        method: "get",
        url: this.$store.state.webApi + "api/ExHpoAxplineWage",
        params: { sid: this.mawbguid, boguid: this.orderguid },
        loading: true,
        tip: false
      })
        .then(results => {
    ...
    //切换结算对象
    handleSettname(type, val, gidIndex) {
      //type 1应收 2应付  val结算对象gid
      //alert('3')
      this.wageguids = [];
      this.wageguidf = [];
      ...
      if (type == "1") {
        ...
        activeysf = val;
        if (val != "") {
          //切换结算对象不为空时，默认当前结算对象无法修改
          this.curObjectKyYs.gid = val;
          this.curObjectKyYs.disabled = true;
        } else if (val == 0 || val == "") {
          //切换结算对象为空时，默认初始结算对象
          if (Number(this.orderguid) > 0) {
            this.curObjectKyYs.gid = this.curObjectKyYs.gid
              ? this.curObjectKyYs.gid
              : Number(this.orderpgid);
          } else {
            this.curObjectKyYs.gid = this.curObjectKyYs.gid
              ? this.curObjectKyYs.gid
              : this.mawbInformation.gid;
          }
        ...
        if (val != "") {
          ...
          //切换结算对象为空时，默认初始结算对象
          this.curObjectKyYf.gid = this.curObjectKyYf.gid
            ? this.curObjectKyYf.gid
            : this.gysId;
      ...
      currencyAlone.forEach(item => {
        ...
        changeData.forEach(item2 => {
          if (
            item2.isfree == "否" &&
            item == item2.currency &&
            item2.thisListHide == false
          ) {
            num += Number(item2.leastwageall);
          }
        });
        if (type == "1") {
          this.currentGidCost.charge.push({
            currency: item,
            money: Number(num).toFixed(2),
            currencyFh: this.currencyFuHao[item]
          });
        } else if (type == "2") {
          this.currentGidCost.payment.push({
            currency: item,
            money: Number(num).toFixed(2),
            currencyFh: this.currencyFuHao[item]
          });
    ...
    changeDirectionAcn(type, wageinout) {
      if (wageinout == "1") {
        ...
        $(".setnameAll").css(
          "transform",
          "translateX(-" + this.arrowAcnIndex * 172 + "px)"
        );
      } else {
        var clientWidth = 0;
        if (type == 1) {
        ...
        $(".setnameAllf").css(
          "transform",
          "translateX(-" + this.arrowAcnIndexf * 172 + "px)"
        );
      }
    },
    //去重数组对象
    ...
    getAllHawb() {
      this.$axios({
        method: "get",
        url: this.$store.state.webApi + "api/ExAxpline",
        params: { hpoidHawbGroup: this.mawbguid },
        loading: true,
        tip: false
      }).then(results => {
        //console.log("分单号")
        ...
        if (!currencyAlone.includes(item.currency)) {
          ...
          this.mawbCharge.forEach((item3, index3) => {
            if (
              item3.currency == item.currency &&
              item3.isfree == "否" &&
              (item3.confirmstatus == "已确认" || item3.confirmstatus == "700")
            ) {
              numC += Number(item3.leastwageall);
            }
          });
          ...
          this.mawbPayment.forEach((item2, index2) => {
            if (
              ...
              item2.isfree == "否" &&
              (item2.confirmstatus == "已确认" || item2.confirmstatus == "700")
            ) {
          ...
          this.currencyCost.payment.push({
            currency: item.currency,
            money: numP.toFixed(2),
            currencyFh: this.currencyFuHao[item.currency]
          });
          this.currencyCost.profit.push({
            currency: item.currency,
            money: (numC - numP).toFixed(2),
            currencyFh: this.currencyFuHao[item.currency]
          });
        }
      });
      ...
      itemsArrPayment.forEach((item, index) => {
        ...
        if (arrIndex >= 0) {
          if (totalIndex.indexOf(arrIndex) == -1) {
            ...
            arrIndex = itemsArrCharge.indexOf(
              item,
              itemIndex[this.itemsReocrder[item]]
            );
          ...
          this.toolTip.push(
            Number(
              (item.isfree == "否" ? Number(item.wageall) : 0) -
                (this.mawbPayment[index].isfree == "否"
