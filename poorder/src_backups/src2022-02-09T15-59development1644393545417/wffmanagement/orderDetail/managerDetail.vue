<template>
  <div class="orderDetail">
    ...
    <div class="content">
      <infoList :inputModelData="inputModelDataInfoList" v-if="showListInfo"  name="详细">
            <template slot="wageAlert">
                ...
                <span style="color:red" v-if="basicInfo.tips">{{basicInfo.tips}}</span>
            </template>
      ...
      <div class="detail-c basicInformation">
        <el-row>
          <el-col :span="12">
            <p class="red"><span>委托客户：</span>{{ basicInfo.wtkhname.split('-')[1] }}</p>
          </el-col>
          <el-col :span="12">
            <p class="red"><span>项目：</span>{{ basicInfo.wtkhname.split('-')[0] }}</p>
          </el-col>
        </el-row>

        <el-row>
          <el-col :span="6">
            <p class="red"><span>订单编号：</span>{{ basicInfo.jobno }}</p>
          </el-col>
          <el-col :span="6">
            <p class="red"><span>总运单号：</span>{{ basicInfo.mawb }}</p>
          </el-col>
          <el-col :span="6">
            <p class="red"><span>分运单号：</span>{{ basicInfo.hawb }}</p>
          </el-col>
        </el-row>

        <el-row>
          <el-col :span="6">
            <p><span>配舱操作：</span>{{ mawbInfo.pcstatus }}</p>
          </el-col>
          <el-col :span="6">
            <p><span>货物性质：</span>{{ mawbInfo.hwxz }}</p>
          </el-col>
          <el-col :span="6">
            <p><span>货物类型：</span>{{ mawbInfo.hwlx }}</p>
          </el-col>
          <el-col :span="6">
            <p><span>分运单票数：</span>{{ mawbInfo.hawbnum == -1 ? '' : mawbInfo.hawbnum }}</p>
          </el-col>
        </el-row>

        <el-row>
          <el-col :span="6">
            <p><span>始发港：</span>{{ basicInfo.sfg }}</p>
          </el-col>
          <el-col :span="6">
            <p><span>目的港：</span>{{ basicInfo.mawbmdg }}</p>
          </el-col>
          <el-col :span="6">
            <p><span>航班号：</span>{{ basicInfo.real_hbh }}</p>
          </el-col>
          <el-col :span="6">
            <p><span>航班日期：</span>{{ basicInfo.real_hbrq }}</p>
          </el-col>
        </el-row>

        <el-row>
          <el-col :span="6">
            <p><span>总单预报体/重/件：</span>{{ mawbInfo.ybjzt }}</p>
          </el-col>
          <el-col :span="6">
            <p><span>报关重量：</span>{{ mawbInfo.realweight }}</p>
          </el-col>
          <el-col :span="6">
            <p><span>应收结算重量：</span>{{ basicInfo.accountweightin_sett }}</p>
          </el-col>
          <el-col :span="6">
            <p><span>应付结算重量：</span>{{ basicInfo.accountweightout_sett }}</p>
          </el-col>
        </el-row>

        <el-row>
          <el-col :span="6">
            <p><span>实际件/重/体：</span>{{ mawbInfo.realjzt }}</p>
          </el-col>
          <el-col :span="6">
            <p><span>实际计费重量：</span>{{ mawbInfo.realjfweight }}</p>
          </el-col>
          <el-col :span="6">
            <p><span>总单制单件重体：</span>{{ mawbInfo.zdjzt }}</p>
          </el-col>
          <el-col :span="6">
            <p><span>制单计费重量：</span>{{ mawbInfo.zdjfweight }}</p>
          </el-col>
        </el-row>

        <el-row>
          <el-col :span="6">
            <p><span>应收运价类型：</span>{{ (!mawbInfo.profitmode || mawbInfo.profitmode.indexOf('利润')==-1) ? mawbInfo.isinwageallin : '--' }}</p>
          </el-col>

          <el-col :span="6">
            <p v-if="!mawbInfo.profitmode || mawbInfo.profitmode.indexOf('利润')==-1">
              <span>应收运费价格：</span>
              {{ (Number(mawbInfo.inwageallinprice).toFixed(2) == 666666.00 ? '--' : Number(mawbInfo.inwageallinprice).toFixed(2)) || 0.00 }}
            </p>
            <p v-else><span>应收运费价格：</span>--</p>
          </el-col>

          <el-col :span="6">
            <p v-if="!mawbInfo.profitmode || mawbInfo.profitmode.indexOf('利润')==-1">
              <span>应收运费分泡比例：</span>
              WFF:{{ Number(mawbInfo.self_real_bp_freight_in) }}&nbsp;&nbsp;:&nbsp;&nbsp;客户:{{ Number(mawbInfo.cus_real_bp_freight_in) }}
            </p>
            <p v-else><span>应收运费分泡比例：</span>--</p>
          </el-col>

          <el-col :span="6">
            <p><span>分配模式：</span>{{ mawbInfo.profitmode ? mawbInfo.profitmode : '--' }}</p>
          </el-col>
        ...
        <el-row>
          <el-col :span="6">
            <p><span>销售员：</span>{{ basicInfo.salesname }}</p>
          </el-col>
          <el-col :span="6">
            <p><span>单证状态：</span>{{ basicInfo.dzstatusCn }}</p>
          </el-col>
          <el-col :span="6">
            <p><span>审单人：</span>{{ basicInfo.submitman_2_sett }}</p>
          </el-col>
          <el-col :span="6">
            <p><span>审单日期：</span>{{ basicInfo.submitdate_2_sett }}</p>
          </el-col>
        </el-row>
    ...
    <div class="wageInformation" v-if="!checkMode">
      ...
      <div class="wage_content wagein">
        <div class="header">
          ...
          <div class="header-right">
            <!-- <p>不含税总计(折合人民币): {{ basicInfo.wageinNoTaxMoneyRmb }} 元</p>
            <p>含税总计(折合人民币): {{ basicInfo.wageinTaxMoneyRmb }} 元</p>
            <p>应收实际结算重量: {{ basicInfo.accountweightin_sett }} 公斤</p>-->
            <p>不含税总计(折合人民币): {{getWageTotal(1,1)}} 元</p>
            <p>含税总计(折合人民币): {{getWageTotal(1,2)}} 元</p>
            <p>应收实际结算重量: {{getWageTotal(1,3)}} 公斤</p>
          </div>
          <!-- 操作站利润分成不显示 -->
          <el-button style="float:right" type="primary" @click="bohuiAddDialog=true;wageinout=1" v-if="!(inputModelData.profitmode&&Number(inputModelData.orderguid)>0&&inputModelData.profitmode.indexOf('利润')>=0)&&showBtnfoot&&inputModelData.orderguid<0">新增并驳回</el-button>
        </div>
        <div class="content">
          <p class="wageintotal">
          ...
          <commonTable class="commonTable" :head="table_model"
            :table-data="resultData.wageinList.concat(resultData.wageinList_old||[])">
            <template slot="checkTh">
               <el-checkbox v-model="checkBh.allCheckYs" :disabled="basicInfo.dzstatus=='1200'||inputModelData.orderguid>0"></el-checkbox>
                <i class="el-icon-shenhebohui" title="批量驳回" v-if="projectIsWecan&&mawbChargeBohuiChecked.length&&isSponsor" @click="getBohuiWageDetail(mawbChargeBohuiChecked,2)" style="font-size:14px;margin-left:4px;" :icondisabled="basicInfo.dzstatus=='1200'&&!basicInfo.tzType"></i>
            </template>
            <template slot="check" slot-scope="props">
            <!-- v-if="!mawbChargeBohuiChecked.length" -->
      ...
      <div class="wage_content wageout">
        <div class="header">
          <div :class="['title', {'active': !czdWageOutVisible}]" @click="czdWageOutVisible = false">本票应付信息</div>
          ...
          <div class="header-right" v-if="!czdWageOutVisible">
            <!-- <p>不含税总计(折合人民币): {{ basicInfo.wageoutNoTaxMoneyRmb }} 元</p>
            <p>含税总计(折合人民币): {{ basicInfo.wageoutTaxMoneyRmb }} 元</p>
            <p>应付实际结算重量: {{ basicInfo.accountweightout_sett }} 公斤</p>-->
            <p>不含税总计(折合人民币): {{getWageTotal(2,1)}} 元</p>
            <p>含税总计(折合人民币): {{getWageTotal(2,2)}} 元</p>
            <p>应付实际结算重量: {{getWageTotal(2,3)}} 公斤</p>
          ...
          <el-button style="float:right" type="primary" @click="bohuiAddDialog=true;wageinout=2" v-if="showBtnfoot">新增并驳回</el-button>
        </div>
        <div class="content">
          ...
          <p class="wageouttotal" v-else>
            ...
            {{ allCurrencyPowageoutList }}
          </p>

          <commonTable class="commonTable" :head="table_model"
            :table-data="czdWageOutVisible ? resultData.powageoutList : resultData.wageoutList.concat(resultData.wageoutList_old||[])">
            <template slot="checkTh">
               <el-checkbox v-model="checkBh.allCheckYf" :disabled="basicInfo.dzstatus=='1200'"></el-checkbox>
               <i class="el-icon-shenhebohui"  title="批量驳回" v-if="projectIsWecan&&mawbPaymentBohuiChecked.length&&isSponsor" :icondisabled="basicInfo.dzstatus=='1200'&&!basicInfo.tzType" @click="getBohuiWageDetail(mawbPaymentBohuiChecked,2)" style="font-size:14px;margin-left:4px;"></i>
            </template>
            <template slot="check" slot-scope="props">
            <!-- v-if="!mawbPaymentBohuiChecked.length"  -->
              ...
              <i class="el-icon-shenhebohui"  :style="setBhiconColor(props.data.row.status)" :title="setBhiconColor(props.data.row.status,2)" @click="getBohuiWageDetail(props.data.row)"  :icondisabled="(props.data.row.system!='空出'&&props.data.row.system!='国内服务'||props.data.row.isshare>0||basicInfo.dzstatus=='1200'||props.data.row.isbill==1&&props.data.row.wagedom!='调账')||(basicInfo.tzType!='单票'&&props.data.row.wagedom=='调账')" v-if="(isSponsor?(props.data.row.isshare<0||props.data.row.status!=-1):props.data.row.status!=-1)"></i>
    ...
    <div class="feeInformation">
      ...
      <!-- <div class="right">
        <div>
          ...
          <p>
            每公斤：
            <span>{{ basicInfo.realSetOneKgPrice }}</span> 元
          </p>
        </div>
        <div>
          <p>
            系统结算重量：
            <span>{{ basicInfo.jsweight }}</span> 公斤
          </p>
          <p>
            每公斤：
            <span>{{ basicInfo.sysSetOneKgPrice }}</span> 元
          </p>
        </div>
      </div> -->
    </div>
    ...
    <div class="footer">
      <div class="btn-c">
        ...
        <div class="right-btn" v-if="showBtnfoot&&origin=='undefined'">
          <el-button type="primary" :disabled="basicInfo.dzstatus == 1200||wageRejectin||wageRejectout||basicInfo.isLockApproval" @click="checkOrder">审核完成</el-button>
          <!-- <el-button type="primary" :disabled="basicInfo.dzstatus == 1200" @click="backOrderVisible = true">驳回
          </el-button> -->
          <el-button type="primary" :disabled="basicInfo.dzstatus != 1200||wageRejectin||wageRejectout||basicInfo.isLockApproval" @click="CancelCheckOrder">取消审核</el-button>
        </div>
      </div>
    </div>
    ...
    <el-dialog width="800px" top="4%" center :visible.sync="backOrderVisible" v-if="backOrderVisible" append-to-body
      ...
      <div>
        ...
        <el-dialog :visible.sync="bohuiAddDialog" width="1360px" title="新增并驳回" append-to-body :close-on-press-escape="false" append-to-body :close-on-click-modal="false" center top="4%">
   
<!--     {{bohuiAddDialog}} -->
     <infoList :inputModelData="inputModelData" v-if="showInfoList&&inputModelData.boguid" :pono="inputModelData.pono" name="详细" :costBiaozhu="false" :ifopendetail="false":showBtnGroup="false"/>
    
     <costComtab v-if="inputModelData.boguid" :inputModelData="inputModelData" @reSearch="fetchData" :initiator="initiator" :wageinout="wageinout"></costComtab>
...
<script>
  import { formatDate, computedWeight,getWtxmname } from "../../api/localStorage.js";
  import {
    myCommonTable,
    setBhiconColorMixin
  } from "@/common/detailPagesMixin"
  import costComtab from '@/components/templates/costComtab'
  export default {
    components: {
        myCommonTable,costComtab
    },
    ...
    data() {
      return {
        ...
        table_model: [
          ...
          { field: "confirmstatus", title: "状态",  formatType: 4, format: row => row.isbill == 10?'已开票':(row.isbill==1?'已对账':( row.confirmstatus >= 700 ? '已确认' : (row.confirmstatus == 1 ? '未确认' : '--')))},
          ...
          { field: "pricewageallRmb", title: "不含税金额<br>(折合人民币)" },
          { field: "tax", title: "税金" },
          { field: "realwageallRmb", title: "含税总额<br>(折合人民币)" },
    ...
    computed: {
      ...
      wageoutTotalRealwageall() {
        return this.resultData.wageoutList
          .reduce((p, i) => (p += i.isfree == 1 ? i.realwageallRmb : 0), 0)
          .toFixed(2);
      },
      // 折合人民币总应收
      wageinTotalRealwageall() {
        return this.resultData.wageinList
          .reduce((p, i) => (p += i.isfree == 1 ? i.realwageallRmb : 0), 0)
          .toFixed(2);
      },
      allCurrencyWagein() {
        return this.getAllCurrencyStr(this.resultData.wageinList);
      },
      allCurrencyWageout() {
        return this.getAllCurrencyStr(this.resultData.wageoutList);
      },
      allCurrencyPowageoutList() {
        return this.getAllCurrencyStr(this.resultData.powageoutList);
      },
      mawbPaymentBohuiChecked() { //多选时可批量驳回的费用
            var list=this.czdWageOutVisible ? this.resultData.powageoutList :  this.resultData.wageoutList.concat( this.resultData.wageoutList_old||[])
            return list.filter(i => this.checkBh.yf.includes(i.wageguid) && (i.status == -1 || i.status == 500) && i.isshare <= 0)
      },
      mawbChargeBohuiChecked() {
            var list=this.resultData.wageinList.concat(this.resultData.wageinList_old||[])
            return list.filter(i => this.checkBh.ys.includes(i.wageguid) && (i.status == -1 || i.status == 500) && i.isshare <= 0)
      },
      wageBohuiYs() { //是否存在应收驳回未处理
            var list=this.resultData.wageinList.concat(this.resultData.wageinList_old||[])
            return list.filter(i => (i.status == 100|| i.status == 110) && i.wageinout == 1).length
      },
      wageBohuiYf() { //是否存在应付驳回未处理
          var list=this.czdWageOutVisible ? this.resultData.powageoutList :  this.resultData.wageoutList.concat( this.resultData.wageoutList_old||[])
            return list.filter(i => (i.status == 100 || i.status == 110)&& i.wageinout == 2).length
      },
    },
    created() {
      this.fetchData();
      //console.log(this.child)
      //console.log(this.child.wageRejectin)
    },
    watch: {
      ...
      'checkBh.allCheckYs'(newVal){
        ...
        if(newVal){
          ...
          this.checkBh.ys= list.filter(i => i.isshare <= 0).map(i=>i.wageguid)
        }else{
          this.checkBh.ys=[]
        }
      },
      ...
      'checkBh.ys'(val){
        var list=this.resultData.wageinList.concat(this.resultData.wageinList_old||[]).filter(i => i.isshare <= 0).map(i=>i.wageguid)
        if(list.length==val.length){
          this.checkBh.allCheckYs=true
        }
      },
      'checkBh.yf'(val){
         var listAll=this.czdWageOutVisible ? this.resultData.powageoutList :  this.resultData.wageoutList.concat( this.resultData.wageoutList_old||[])
    ...
    methods: {
      getWageTotal(wageinout, dom) {
        ...
        let wagelist =
          wageinout == 1
            ? this.resultData.wageinList
            : this.resultData.wageoutList;
        wagelist = wagelist
      ...
      getAllCurrencyStr(wageList) {
        ...
        wageList.forEach(item => {
          ...
          if (result[this.currencyFuHao[item.currency]]) {
            result[this.currencyFuHao[item.currency]] += item.wageall * 100;
          } else {
            result[this.currencyFuHao[item.currency]] = item.wageall * 100;
          }
        });
      ...
      fetchData() {
        console.log(this.id,this.wagedom,this.jobnosid)
        let data = {
          sid: this.id,
        ...
        this.$axios({
          ...
          if (result.data.wageinList_old && result.data.wageinList_old.length) {
            result.data.wageinList_old.forEach(i => {
              i.background = '#ccc'
            })
          }
          if (result.data.wageoutList_old && result.data.wageoutList_old.length) {
            result.data.wageoutList_old.forEach(i => {
              i.background = '#ccc'
            })
          }

          // this.resultData.wageinList= this.resultData.wageinList.concat(this.resultData.wageinList_old)
          // this.resultData.wageoutList = [...this.resultData.wageoutList_old,...this.resultData.wageoutList]
          ...
          if (this.basicInfo.oldid != -1) {
            this.$axios({
              ...
              if (!result.data) {
                ...
                this.$set(this.mawbInfo,'fid',this.inputModelData.fid)
                this.$set(this.mawbInfo,'wtxmname',getWtxmname(result.data.gid, 1))
                if (this.inputModelData.iscomboine == 1) {
                  this.$axios({
                    method: "get",
                    url: this.$store.state.webApi + "api/ExHpoAxpline",
                ...
                if (result.data.orderguid > 0) { //已分配结算对象操作站
                        result.data.serviceList.forEach(item => {
                          if (item.servicecode == 'OA0010') {
                          this.inputModelData.profitmode = item.profitmode ? item.profitmode : '本站'
                         }
                      })
                  }
      ...
      //核对模式
      checkModel() {
        let wageoutItemsArr = this.resultData.wageoutList.map(i => i.items),
          wageinListCopy = this.resultData.wageinList.slice(),
          wageoutListCopy = this.resultData.wageoutList.slice();
         console.log(wageinListCopy)
    ...
    editlist.forEach(i => {
        ...
        i.wagedom = this.resetJieshuanWagedom(i, this.inputModelDataInfoList);
    ...
    this.$axios({
        method: 'post',
        url: this.$store.state.wcWebApi + 'api/WageReject/Reject',
        loading: true,
        data: editlist
    }).then(res => {
        // console.log('费用驳回')
