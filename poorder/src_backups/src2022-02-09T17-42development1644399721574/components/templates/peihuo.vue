  ...
  <commonTable :head="getGoodsTablehead().slice(1)" title="货物信息" v-show="goodsinfoshow"  :table-data="sjGoodsTableData.filter(i=>!i.isSeat)" 
    ...
    <div class="content">
      <div style="width:500px" class="left">
        ...
        <commonTable :head="ybTableHead" :table-data="ybTableData" :divStyle="tableHeightStyle" :isRadioSelect="true"  @current-change="docRadioF" :defaultSelectRowIndex="defaultSelectRowIndex">
          <template slot="khjcno" slot-scope="props">
...
<!--               <button
              ...
              <button
                class="cblue el-icon-setting"
                :class="{'tdDisabled':pdfinishstatus||props.data.row.doclist.filter(i=>i.setstatus=='2').length}"
                :style="{background:'center',color:pdfinishstatus||props.data.row.doclist.filter(i=>i.setstatus=='2').length?'#ccc':''}"
...
<!--               <span
    ...
    <p style="color:#A1A1A1;font-size:16px;margin-bottom:10px;">
      ...
      <div style="margin:12px 0px;">
        ...
        <el-button type="primary" @click="addToSjtable">添加</el-button>
    ...
    <el-dialog v-if="dialogShow" center :visible.sync="dialogShow" append-to-body>
     <docUpload :dialogShow.sync="dialogShow"  v-if="dialogShow" @success="declareSearch();getPeihuoInfo()" defaultFunc ref="document" :jcnoList="khjcnoListArray.map(i=>i.khjcno)"></docUpload>
    </el-dialog>
            ...
            if(i.doclist.length){
             ...
             //let ids=i.doclist.map(j=>j.id).join(',')
              ...
              if(i.emailsendstatus=='已发送'||i.emailsendstatus=='2'){
                ...
                this.$set(i,'background','#b3d8ff')
                i.doclist.forEach(i2=>this.$set(i2,'background','#b3d8ff'))
              }
  ...
  computed: {
    ...
    ponoDisabledList(){
      ...
      this.ybTableData.forEach(row=>{          
        if (row.doclist&& row.doclist.filter(i => i.setstatus == '2').length){
            arr.push(row.pono)
        }
      })
      return arr
