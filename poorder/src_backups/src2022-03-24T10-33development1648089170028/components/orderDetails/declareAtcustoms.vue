        ...
        <div v-for="(item,index) in hawbform" :class="{disabled:ponoDisabledList.includes(item.pono)}">
          <el-row style="width:100%">
            ...
            <el-form-item label="报关状态：">
              <my-select :options="options"  itemstyle="width:100px;min-width:120px;" v-model="item.status"
                class="input-required"></my-select>
              <!-- <el-input v-model.number="item.lds"></el-input> -->
            </el-form-item>
...
<script>
  ...
  export default {
    ...
    , methods: {
      ...
      getListInfo(callback) {
        ...
        this.$axios({ method: "get", url: this.$store.state.webApi + "api/ExAxplineDecl", params: { sid: this.mawbguid, serviceguid: this.serviceguid, servicecode: this.servicecode }, loading: true, tip: false }).then(results => {
          ...
          //results.data.forEach(item => item.status=String(item.status));
    ...
    , computed: {
      ...
      options(){
        //报关状态
        return JSON.parse(localStorage.statusData).filter(i=>i.dom=='报关状态'&&i.system==this.mawbinfo.system)
        .map(i=>{
          return {
            label:i.nodename,
            value:i.nodeindex
