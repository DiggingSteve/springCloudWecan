<template>
  <div class="hawbTabs">
    <div class="tabs-header">
      <!-- <div class="left-title" @click="Info=!Info"> -->
    ...
    <div ref="hide_tabs" @blur="tabs_blur" :class="['hide-tabs', {'show': show_all_hawb}]" :style="{'height': `${Math.ceil(hawb_list.length / 8) * 50 + 14}px`}" tabindex="0" >
      <p
        v-for="(hawb, index) in hawb_list"
        :title="hawb"
        :key="hawb"
...
export default {
  ...
  created () {
    ...
    if (this.method == 'detail') {
      ...
      this.$axios({method:'get',url:this.$store.state.webApi+'api/ExAxpline',params:{hpoidHawbGroup:this.mawbguid},loading:true,tip:false}).then(results=>{
        ...
        this.cur_index = 0
        this.cur_hawb = this.hawb_list[this.cur_index]
  ...
  watch: {
    'cur_index' (newVal) {
      this.cur_hawb = this.hawb_list[this.cur_index]
      if(this.guidHawbGroup[this.cur_index]){
        this.$emit('togglehawb', this.guidHawbGroup[this.cur_index])
      }
  ...
  methods: {
    ...
    addhawb () {
      // add hawb
      // this.show_all_hawb = true
      // this.$refs.hide_tabs.focus()
      // this.hawb_list.push(`NEW${String(Math.random()).slice(-8)}`)
      // this.cur_index = this.hawb_list.length - 1
    },
    ...
    removehawb (index) {
      if (index== 0) {
        this.cur_index = 1
      } else if (this.cur_hawb == this.hawb_list[index]) {
