...
<div class="tableWraper">
 <tableCompt :name="name" :table-data-res="tableDataRes" :nestedTableType="2" v-on:transmitMsg="getZimessage" @transmitZiMsg="transmitZiMsg" :ziTableData="ziTableData"  :chinese-where="chineseWhere" showZiConfig :showTotal="showTotal" :overIndex.sync="overIndex">


 <template slot="mawb" slot-scope="props">
  <el-tooltip class="item" effect="light" :content="$t('tips.mawbdownload')" placement="bottom-start" v-if="props.data.row.ismawbdoc=='1'">
 <!-- <i class="download" @click.stop="urlF('1',props.data.row)" v-if="props.data.row.ismawbdoc=='1'"></i> -->
 </el-tooltip>
 </template>

  <template slot="hawb" slot-scope="props">
 <el-tooltip class="item" effect="light" :content="$t('tips.hawbdownload')" placement="bottom-start" v-if="props.data.row.ishawbdoc=='1'">
 ...
 <template slot="detail" slot-scope="props">
   <el-tooltip class="item" effect="light" :content="$t('tips.detail')" placement="bottom-start">
    <i class="el-icon-arrow-right detail" @click="dialogOpen(props.data.row)" :style="{'color':overIndex==props.data.index?'#fff':'','background':overIndex==props.data.index?'#116cf3':''}" style="float:right;margin-right:30px;margin-top:6px;">
    </i>
...
<!-- 订单详细 -->
  <el-dialog  center :visible.sync="dialogShow"  class="dialogPage" :close-on-click-modal="false" :close-on-press-escape="false" width="100%" top="0px" :modal="false"  >
        <goods-info :modelData="modelData" :hawbinfoList="hawbinfoList" :visible.sync="dialogShow" effect="2" :serviceList="serviceList" v-if="dialogShow&&modelData.boguid"></goods-info>
 </el-dialog>
...
//console.log(this.inputModelData.hbrq.begin.getMonth())
   ...
   search2(val){
    //alert('1')
    if(val!=this.inputModelData.status||val=='70'){
       var jsonArr2={
      where:{"status":{"in":val}},
      order:"hbrq desc"
    }
