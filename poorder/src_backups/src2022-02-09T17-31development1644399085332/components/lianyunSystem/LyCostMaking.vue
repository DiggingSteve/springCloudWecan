<template>
  <div class="LyCostDetail">
    <div class="detail mawbInformation" v-if="pagetype==1">
      <div class="detail-title">
        <p>
          <span style="float:left">联运基本信息</span>
      ...
      <div class="detail-c">
        ...
        <el-row>
          ...
          <el-col :span="6">
            <p>
              <span class="label">{{ translate_text['yqpickupdate'] }}</span>
              <span class="value">{{ mawbInformation['yqpickupdate'] }}</span>
            </p>
          </el-col>
          <el-col :span="6">
            <p>
              <span class="label">{{ translate_text['yqfinishdate'] }}</span>
              <span class="value">{{ mawbInformation['yqfinishdate'] }}</span>
            </p>
          </el-col>
        </el-row>
        ...
        <el-row>
          ...
          <el-col :span="6">
            <p>
              <span class="label">{{ translate_text['jzt'] }}</span>
              <span class="value">{{ mawbInformation['jzt'] }}</span>
            </p>
          </el-col>
        </el-row>
      ...
      <div class="tableContent">
       ...
       <table :style="{'border-collapse':'collapse','table-layout':'fixed','width':'100%'}" >
         ...
         <tr v-for="(item,index) in mawbPayment" :key="index" :style="{background:item.isfree=='是'?'#e2fdde':''}">
           ...
           <td width='150' :title="item.custom">
             {{item.custom}}
           </td>
           <td width='100'>
             {{item.items}}
...
export default {
  ...
  created() {
    ...
    this.$axios({
      method: "get",
      url: this.$store.state.webApi + "api/extransport",
      params: { id: this.id },
      loading: true,
      tip: false
    }).then(results => {
      ...
      if (results.data) {
        this.mawbInformation = results.data.transPortInfo;
        this.mawbInformation.wtkhname = results.data.fcustomTrans
        this.mawbInformation.wtxmname = results.data.customTrans.split('-')[0]
        this.mawbInformation.jzt = `${results.data.transPortInfo.piece}/${results.data.transPortInfo.weight}/${results.data.transPortInfo.volume}`

        this.mawbCost(results.data.wageList);

        if (results.data.transPortInfo.orderguid > 0) {
          //已分配结算对象
          this.curObjectKyYs["gid"] = Number(this.mawbCharge[0].gid);
          this.orderpgid = this.mawbCharge[0].gid;
          this.profitmode = results.data.transPortInfo.profitmode;
        } else {
          //未分配结算对象
          this.curObjectKyYs["custom"] = results.data.transPortInfo.wtxmname;
          this.curObjectKyYs["gid"] = Number(results.data.transPortInfo.gid);
        }
        this.orderguid = results.data.transPortInfo.orderguid;
      }
    });
  ...
  methods: {
    ...
    delCost(index, type, dom) {
      ...
      .then(() => {
        ...
        this.$axios({
          method: "DELETE",
          url: this.$store.state.webApi + "api/exTransportWage",
          data: {
            guid: data.guid,
    ...
    addCost(type, dom, data) {
      ...
      if (type == "1" && dom == "1") {
        ...
        if (method == 'POST') {
          templetadata = {
            transGuid: this.mawbInformation.guid,
            wageList: [templetadata]
          }
        }

        this.$axios({
          method: method,
          url: this.$store.state.webApi + (method == 'PUT' ? "api/exTransportWage/update" : "api/exTransportWage"),
          data: templetadata,
          loading: true,
          tip: false
        }).then(results => {
          if (results.data.resultstatus == 0) {
            ...
            if (Number(this.orderguid) > 0) {
              ...
              this.curObjectKyYs = {
                currency: "人民币",
                custom: this.mawbInformation.wtxmname,
        ...
        if (method == "POST") {
          ...
          templetadata.wageList.push({...templetadata.wageList[0], wageinout: 1, gid: this.orderguid > 0 ? this.mawbCharge[0].gid : this.mawbInformation.gid, isOther: 2, isfree: 1})
        }
    ...
    // ***********应收应付结算对象************
    getYfSettname(data) {
      this.curObjectKyYf.custom = data.usr_name;
    },
    getYsSettname(data) {
      this.curObjectKyYs.custom = data.usr_name;
    },
    //
    //清空当前费用
    ...
    confirmButton (wageinout, status) {
      ...
      if (wageinout == 1) {
        confirmData = [...this.mawbCharge, this.charge_yssys]
      } else if (wageinout == 2) {
        confirmData = [...this.mawbPayment, this.payment_yssys]
      }

      if (status == 1) {
        confirmData = confirmData.map(i => {
          let item = {...i}
          item.isfree == '否' ? (item.isfree = 1) : (item.isfree = 2);
          [item.confirmstatus, item.confirmstatus_backup] = [item.confirmstatus_backup || item.confirmstatus, item.confirmstatus]
          // 已确实状态的费用才能修改为未确认状态
      ...
      let data = {
        ...
        transId: this.id,
        wageinout: wageinout,
        yqpickupdate: this.mawbInformation.yqpickupdate,
        status: status
      }
      ...
      .then(result=>{
        if (result.data.resultstatus == 0) {
          let data = confirmData.filter(i => i.yssys != 2).map(i => {
          ...
          if (wageinout == 1) {
            this.mawbCharge = data
            this.charge_yssys = confirmData.filter(i => i.yssys == 2)[0]
          } else if (wageinout == 2) {
            this.mawbPayment = data
            this.payment_yssys = confirmData.filter(i => i.yssys == 2)[0]
          }
    ...
    async mawbCost(data) {
      ...
      if (!results) {
        let param = {
          where: {
            sid: {in: this.mawbInformation.guid}
          }
        }
        await this.$axios({method: 'get',url:this.$store.state.webApi+'api/exTransportWage',params:{json: JSON.stringify(param)},loading:false,tip:false})
        .then(result => {
