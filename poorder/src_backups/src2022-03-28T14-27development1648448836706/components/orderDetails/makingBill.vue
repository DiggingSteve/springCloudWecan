            ...
            <div class="topCon">
              ...
              <div class="right">
                ...
                >
                  <commonTable
                    :head="CBMtableHead"
                    :table-data="comAllkhjcnoData"
                    style="max-height: 450px; overflow: auto"
                  >
                    <!-- <template slot="longs" slot-scope="props">
                      <el-input v-model="props.data.row.longs" 
                        placeholder="请填写长度" 
                        style="width: 100px;"
                        clearable
                        >
                    ...
                    <template slot="widths" slot-scope="props">
                      <el-input v-model="props.data.row.widths" 
                        placeholder="请填写宽度" 
                        style="width: 100px;"
                        clearable
                        >
                    ...
                    <template slot="heights" slot-scope="props">
                      <el-input v-model="props.data.row.heights" 
                        placeholder="请填写高度" 
                        style="width: 100px;"
                        clearable
                        >
                    ...
                    <template slot="com" slot-scope="props">
                      <el-input v-model="props.data.row.longs" 
                        placeholder="请填写长度" 
                        style="width: 100px;"
                        clearable
                    ...
                    <template slot="volume" slot-scope="props">
                      {{
                         (props.data.row.longs * props.data.row.widths * props.data.row.heights * props.data.row.piece / 1000000).toFixed(3)
                      }}
                    </template>
                  </commonTable>
                  <p style="display: flex; justify-content: end;margin-top: 10px;"
                    v-show="comAllkhjcnoData.length">
                    VOL:{{totalVolume}} CBM
                  </p>
                </div>
                <!-- 长 宽 高 件数 计算 -->
...
export default {
  ...
  data() {
    return {
      ...
      CBMtableHead: [
        {
          title: '序号', field: 'index', width: '20px',
        },
        {
            title: '长/宽/高(CM)',  field: 'com', width: '360px',
        },
        // {
        //    title: '宽', field: 'widths', width: '120px',
        // },
        // {
        //    title: '高', field: 'heights', width: '120px',
        // },
        {
           title: '件数', field: 'piece', width: '120px',
        },
        {
          title: '体积', field: 'volume', width: '120px',
        }
 ...
 */
  ...
  computed: {
    comAllkhjcnoData () {
      ...
      let data = this.backupkhjcnoData;
      let firstData = [];
      let secondData = [];
      data.forEach((i, index) => {
         firstData = firstData.concat(i.realList).concat(i.autoMapRealList)        
      })

      firstData.forEach((j, index) => {
        secondData = secondData.concat(j.vollist)
      })

      secondData.forEach((k, index) => {
        k.index = index + 1;
        // i.volume = (i.longs * i.widths * i.heights * i.piece / 1000000).toFixed(3);
      })
    ...
    totalVolume() {
      let total = 0;
      this.comAllkhjcnoData.forEach((i, index) => {
        let num = Number((i.longs * i.widths * i.heights * i.piece / 1000000).toFixed(3))
        console.log({total, num})
        total = Number(total) + num;
      })

      if(!this.pmBackup) {
        this.pmBackup = this.formdata.pm
      }
       if (this.showVolume) {
        let data = this.pmBackup;
        this.comAllkhjcnoData.forEach((i ,index) => {
          data += `\n${i.longs}X${i.widths}X${i.heights}X${i.piece}`
        })
        this.formdata.pm = data;
      } else {
        this.formdata.pm = this.pmBackup;
      }
  ...
  watch: {
    ...
    showVolume(val) {
      ...
      if (val) {  // 如果备份的话
        let data = this.formdata.pm;
        this.comAllkhjcnoData.forEach((i ,index) => {
          data += `\n${i.longs}*${i.widths}*${i.heights}*${i.piece}=${(i.longs * i.widths * i.heights * i.piece / 1000000).toFixed(3)}`
        })
        this.formdata.pm = data;
        this.backupkhjcnoData = JSON.parse(JSON.stringify(this.allkhjcnoData))
      } else { // 不备份的话，还原pm
        this.formdata.pm = this.pmBackup;
      }
    },
