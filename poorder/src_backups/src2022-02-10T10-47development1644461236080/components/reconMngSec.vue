  ...
  <div>
    ...
    <tableCompt :name="name" :table-data-res="tableDataRes" :showTotal="showTotal" :chinese-where.sync="chineseWhere">
      ...
      <!--   <template slot="canDzFun" slot-scope="props"><i class="el-icon-delete" @click="cancelDz('single',props.data.index)" title="取消对账"></i></template>
       ...
       <template slot="changeState" slot-scope="props" style="display:block!important"><i class="el-icon-sort" @click="changeDzState(props.data.index)" :title="props.data.row.billstatus=='对账中'?'改为已对账':'改为对账中'"></i></template> -->
      <template slot="print" slot-scope="props" style="display:block!important">
        ...
        <!-- <excelOut :tableData="tableDataRes" :title='name' :outindex="props.data.index" :headData="dataTable" :chinese-where="chineseWhere" pagetype='2'></excelOut> -->
      ...
      <template slot="billwageall" slot-scope="props">
        <span>{{props.data.value}}</span>
        <!--        <i
          :class="(props.data.row.wagerejectstatusin==100||props.data.row.wagerejectstatusin==200||props.data.row.wagerejectstatusin==50)?'el-icon-shenhebohui':'el-icon-document'"
          :title="wagerejectstatusTitle(props.data.row.wagerejectstatusin,1)"
          @click="singleDzDataInfo(props.data.index)"
          style="font-size:16px;"
          :style="wagerejectstatusTitle(row.wagerejectstatusin,1,2)"
      ></i> -->
      </template>
    ...
    <!-- 单条对账记录信息弹框 -->
    <el-dialog :title="wageinout == '1' ? '应收对账中' : '应付对账中'" width="100%" top="0" center :visible.sync="singleDzInfo"
      v-if="singleDzInfo" @close="handleClose" class="dialogPage" :modal="false">

      <singleDz :singleDzData="singleDzData" :wageinout="wageinout" :showbtn='1' @closeDialog="closeDialog"
        ref="singledz" :system="system" :area="area" :addman="addman"></singleDz>
    ...
    <el-dialog width="600px" top="4%" center :visible.sync="reconOpen" v-if="reconOpen" append-to-body>
      <div style="width:430px;margin:0 auto" v-if="reconOpen">
        <p>
          <span style="min-width:120px;display:inline-block">{{ wageinout == '1' ? '客户对账人:' : '供应商对账人:' }}</span>
          <el-input v-model='reconInfo.confirmman' style="width:300px;"></el-input>
        </p>
        <p style="margin-top:20px;">
          <span style="min-width:120px;display:inline-block">{{ wageinout == '1' ? '客户对账时间:' : '供应商对账时间:' }}</span>
...
<script>
  import { reset, searCondition, getMomentDate, getChangeValue } from '../api/localStorage.js'
    ...
    data() {
      return {
        ...
        inputViewData: {
          ...
          wageinout: { title: '收付类型', type: 4, options: [{ label: '应收', value: '1' }, { label: '应付', value: '2' }] },
          accountcomgid: { 'title': '结算对象', type: 20, pagetype: 1 },
          confirmman: { 'title': this.wageinout == '1' ? '客户确认人' : '供应商确认人', type: 1, "whereStr": "like", titleStyle: { 'min-width': '110px' } },
          confirmdate: { 'title': this.wageinout == '1' ? '客户确认日期' : '供应商确认日期', type: 15, defaultVal: false, titleStyle: { 'minWidth': '110px' } },
          billstatus: { "title": "对账状态", type: 4, options: [{ label: '对账中', value: '900' }, { label: '已对账', value: '1000' }] },
          confirmremark: { "title": '对账备注', "whereStr": "like", type: 1 },
        ...
        inputModelData: {
          wageinout: this.wageinout, billstatus: '900'
        },
        searchData: {},
        pageshow: true,
        tableSelectIndex: '',
    ...
    methods: {
      ...
      outExcel(index) {
        this.excelHead = [[
          ...
          { "field": "acNum", "title": this.wageinout == '1' ? "应收结算重量" : "应付结算重量" },
          { "field": "acPrice", "title": this.wageinout == '1' ? "应收空运费单价" : "应付空运费单价" }
        ], [

        ]]
        this.excelData = []
        console.log(this.tableDataRes[index])
        var data = { area: this.area, system: this.tableDataRes[index].system, billid: this.tableDataRes[index].billid, system: '' }
        this.$axios({ "method": "get", url: this.$store.state.webApi + 'api/ExAiraxpCheckBillPchFinishDetail', params: data }).then(res => {
          ...
          this.excelHead[1].push({ "field": 'remark', "title": '费用备注' })
      ...
      cancelDz(type, val) {
        ...
        if (status != 0) {
          ...
          //       })
          // .then(()=>{
          //   this.$axios({"method":"DELETE",url:this.$store.state.webApi+'api/ExAiraxpCheckBillPch',data:data})
          //     .then(results=>{
