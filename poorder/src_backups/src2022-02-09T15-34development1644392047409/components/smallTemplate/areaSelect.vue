    ...
    <div class="areaSelect">
        <el-select style="width:100%" v-model="value" ref="userSettingCitiesSelect" :placeholder="placeholder"
            ...
            <div class="title">
                ...
                <div class="title-right" v-if="multiple">
                    <p @click="checkAll = !checkAll" :class="['checkAll', {'active': checkAll}]">全选</p>
                    <p @click="$refs.userSettingCitiesSelect.blur()" class="confirm">确定</p>
                </div>
            </div>
            <table>
                <tr>
            ...
            checkAll: {
                get() {
                    if(!this.value){
                        ...
                        return this.value.length==this.cities.map(i=>i.options).flat().filter(i=>!i.disabled).map(i=>i.value).length
                    }
                    
                },
                set(val) {
                    if (val) {
                        this.value=this.cities.map(i=>i.options).flat().filter(i=>!i.disabled).map(i=>i.value)
                    } else {
                        this.value = [];
                    }
                }
