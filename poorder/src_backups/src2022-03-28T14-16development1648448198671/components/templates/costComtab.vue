      ...
      <template slot="firstTd" slot-scope="props">
        ...
        <td style="display:inline-flex;border:none;width:100%;">
          <page-select
            ...
            v-if="inputModelData.area"
            :wtxmname="inputModelData.wtxmname"
        ...
        <td>
          <page-select
            ...
            :servicecode="
              inputModelData.area == conditionalAreaForBuild
        ...
        <td>
          <el-input
            v-model="curentObject.leastwageall"
            style="width:100px;"
            :disabled="true"
          ></el-input>
        </td>
...
import { setBhiconColorMixin } from "@/common/detailPagesMixin";
  ...
  methods: {
    ...
    modCost2() {
      if ($.trim(this.contact) == "" || $.trim(this.modifyReason) == "") {
        ...
        this.templetadata.wagedom = this.resetJieshuanWagedom(
          this.templetadata,
          this.inputModelData
        );
    ...
    addConfirmWage() {
      ...
      templetadata.wagedom = this.resetJieshuanWagedom(
        templetadata,
        this.inputModelData
      );
  ...
  computed: {
    zdChangeRate() {
      ...
      if (this.curentObject.num && this.curentObject.price) {
        this.curentObject.leastwageall = Number(
          Number(this.curentObject.num) * Number(this.curentObject.price)
        ).toFixed(2);
        this.curentObject.wageall = Number(
          Number(this.curentObject.num) *
            Number(this.curentObject.price) *
            Number(getChangeRate(this.curentObject.currency))
        ).toFixed(2);
      }
      this.$forceUpdate();
  ...
  watch: {
    ...
    "curentObject.num"(val) {
      if (this.curentObject.price) {
        this.curentObject.leastwageall = (
          Number(val) * Number(this.curentObject.price)
        ).toFixed(2);
        this.curentObject.wageall = (
          Number(this.curentObject.leastwageall) *
          Number(this.curentObject.changerate)
        ).toFixed(2);
      }
    },
    "curentObject.price"(val) {
      if (this.curentObject.num) {
        this.curentObject.leastwageall = (
          Number(this.curentObject.num) * Number(val)
        ).toFixed(2);
        this.curentObject.wageall = (
          Number(this.curentObject.leastwageall) *
          Number(this.curentObject.changerate)
