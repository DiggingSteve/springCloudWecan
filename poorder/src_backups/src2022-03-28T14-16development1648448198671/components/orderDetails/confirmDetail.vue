<template>
  <div>
    ...
    <h3
      style="color:red"
      v-if="pagetype==2&&!disabledStatus"
    >*背景色为黄色的输入框为海关联系单发送项
    </h3>
    <!-- 海关联系单发送项 -->
    ...
    >
      ...
      <h3
        ...
        v-if="showBasicInfo&&!disabledStatus"
      >
        订舱编号：
      ...
      <el-button
        ...
        style="height:28px;"
        v-hide="baoguanFinished||disabledStatus"
        @click="sendCustomsContact"
        :disabled="!getSendStatus"
      >
        发送海关联系单
      </el-button>
      <!-- 发送海关联系单按钮 -->
    ...
    <div class="hawbTabs">
      ...
      <div
        ...
        <div
          v-show="cur_index==0"
          :class="{paneDisabled:pagetype==1&&dom==2&&projectIsWecan, 'disabled': disabledStatus}"
        >
          <!-- 汇总信息 -->
            <div :class="{detail: pagetype == 2 && disabledStatus}">
              <div class="detail-title" v-if="pagetype == 2 && disabledStatus">
                海关联系单信息

                  <div class="detailPaneMidinfo" style="position:relative; margin-left: 20px; padding: 0 10px;">
                    <div>
                      <span>状态：</span>
                      <span>{{$parent.$parent.$parent.isServiceFinish('AA0160')?'已完成':'未完成'}}</span>
                    </div>
                    <div>
                      <span>操作人：</span>
                      <span>{{$parent.$parent.$parent.isServiceFinish('AA0160','nodeman')}}</span>
                    </div>
                    <div>
                      <span>完成时间：</span>
                      <span>{{$parent.$parent.$parent.isServiceFinish('AA0160','nodedate') | formatDate('yyyy-MM-dd hh:mm')}}</span>
                    </div>
                    <div style="width: 50%;">
                    ...
                    >
                      ...
                      <div>
                        ...
                        <el-button
                          ...
                          v-hide="(hawbDanhaoAllConfirm&&inputModelData.commbillmodifystatus!=2)||(pagetype==1&&dom==2)||pagetype==3||addHawbDialog||disabledStatus"
                          style="height:28px;"
        ...
        <div v-show="cur_index==1">
          ...
          <div
            ...
            <div class="detail-title">
              ...
              <!-- 海关联系单确认按钮 -->
              <el-button type="primary" @click="checkMawb" v-if="!disabledStatus&&pagetype==2">{{confirmStatus?'取消确认': '确认'}}</el-button>
            </div>
            <!-- 标题 -->
            ...
            <div
              class="detail-c"
              style="width: 1200px;"
              :class="{paneDisabled:(pagetype==1&&inputModelData.mawbConfirmOverStatus==1&&inputModelData.dzmodifystatus!=2)||(pagetype==1&&dom==2)||(pagetype==1&&dom==1&&inputModelData.czlx=='自货'), 'disabled': confirmStatus}"
            >
              <newFormCmpt
        ...
        <div v-show="cur_index>(initialTabLength-1)">
          ...
          >
            <customInfoConfirm
              ...
              :ifFromMawbaddput="!showBasicInfo"
              :isOrderConfirm="isOrderConfirm"
              @changeStatus="(status) => changeHawbStatus({status, index: index1})"
...
<script>
  ...
  export default {
      ...
      return {
        ...
        inputViewData: {
          czlx: {
            ...
            itemStyle: { width: "100%" },
            disabled: true,
            hidden: this.pagetype==2,
          },
          zzg: { title: "中转港：", type: 20, verify: "uppercase", pagetype: 6, hidden: this.pagetype==2, },
          deliverydate: {
            title: "货好日期：",
            type: 12,
          ...
          yjqfsj: { title: "预计起飞时间：", hidden: this.pagetype==2, },
          yqqcts: {
            title: "要求全程天数：",
            type: 1,
            verify: "integer",
          ...
          hbh: { title: "实际航班号：", verify: "uppercase", type: 18, hidden: this.pagetype==2, },
          hbrq: { title: "实际航班日期：", format: "yyyy-MM-dd HH:mm", type: 18, hidden: this.pagetype==2, },
          qfsj: { title: "实际起飞时间：", format: "HH:mm", type: 18, hidden: this.pagetype==2, },
          qcts: { title: "实际全程天数：", verify: "integer", type: 18, hidden: this.pagetype==2, },
          hbzl: {
            title: "航班种类：",
            type: 3,
            groupid: "6",
        ...
        confirmStatus: this.disabledStatus, // 海关联系单-总运单信息确认按钮状态
      };
    },
    ...
    methods: {
      ...
      recoverData(){
        ...
        this.$axios({
          ...
          if (data != null) {
            ...
            data[i] = data[i] || this.inputModelData[i]
            }
            this.inputModelData = data;
            this.allDeclInfo = results.data.hawblist;
          }
        })
      },
       ...
       * 改变状态
       */
      changeHawbStatus({index, status}) {
        this.$set(this.allDeclInfo[index], 'confirmStatus', status)
      },
      ...
      checkMawb() {
        ...
        for (let i in arr) {
          ...
          // 报关重量清空，重新填写0为字符串零
          if(field == 'bgweight') {
            mawbinfo[field] = Number(mawbinfo[field])
          }
          if (!mawbinfo[field]) {
           ...
           if(mawbinfo.address_fhr_mawb.length>70||mawbinfo.address_shr_mawb.length>70||mawbinfo.companytitle_fhr_mawb.length>70||mawbinfo.companytitle_shr_mawb.length>70){
            ...
            if (
              ...
              this.confirmStatus = false;
              return this.$message.error( "总单收发货人的国家二字码必须是两位数！");
            }
            ...
            if (
              ...
              this.confirmStatus = false;
              return this.$message.error( "总单收发货人的信用代码格式不正确,必须是'9999+'加数字字母组合！");
            }
          }
        }
      ...
      setTabClassName(index) {
        ...
        if (index == 0 || index == 2 || this.pagetype == 2) {
          if (this.pagetype == 2 && index != 0 && index != 2) { // 海关联系单
            if (index == 1) { // 总运单信息
              // 海关联系单确认完成
              return this.confirmStatus ? 'badgeGreen': 'badgeRed'
            } else { // 分单件
              let currentHawbdata = this.allDeclInfo[index - this.initialTabLength]; // 分单数据
              return this.disabledStatus || currentHawbdata.confirmStatus  ? 'badgeGreen': 'badgeRed'
            }
          } else {
            return "";
          }
        }
        let hawbdata = this.hawbTableData[index - this.initialTabLength]; // 分单数据
...
setRound(jfweight) {
    ...
    computed: {
      // 发送海关联系单按钮可用状态
      getSendStatus() {
        return this.confirmStatus && this.allDeclInfo.every(item => item.confirmStatus)
      },
