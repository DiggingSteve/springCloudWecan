<template>
  <div>
        ...
        :class="[
          !isOutAddClass
            ? pagetype == 1
              ...
              : 'contentCmpt'
            : 'contentOut',
          { 'page-container-main': !isOutAddClass },
        ]"
        v-enterfocus
        :tempname="name"
      >
        <template v-for="(k, index) in Object.keys(viewData)">
          <div
            v-show="(isShowWtkh&&k=='wtxm')||k!='wtxm'"
            :key="k"
            :id="k"
            :style="[
              { order: viewData[k].order >= 0 ? viewData[k].order : index },
              viewData[k].idStyle,
              {
                display: (
                  typeof viewData[k].hidden == 'function'
                    ? viewData[k].hidden()
                    : viewData[k].hidden
                )
                  ? 'none'
            ...
            :class="[
              { elitem: isOutAddClass, width100: viewData[k].occupyAline },
              viewData[k].idClass,
            ]"
            v-if="k"
          >
            <div
              ...
              "
              :class="{
                itemDisabled: computedAttribute(k) && pagetype == 1,
                width100: viewData[k].occupyAline,
              }"
            >
              <div
                class="input-title"
                :style="[
                  {
                    'min-width': viewData[k].type == 15 ? '78px' : '',
                    width: viewData[k].type == 15 ? '78px' : '',
                  },
                  viewData[k].titleStyle,
                ]"
              >
                <!-- {{viewData[k].title}} -->
                {{ computedAttribute(k, "title") || "" }}
                <explainTooltip
                  :text="viewData[k].explainText"
                  v-if="viewData[k].explainText"
                >
                </explainTooltip>
              ...
              <div
                class="input-content"
                :style="[viewData[k].contentStyle]"
                :class="{
                  'input-required': computedAttribute(k, 'required'),
                  contentDisabled: computedAttribute(k) && pagetype == 2,
                }"
              >
                ...
                <template v-if="viewData[k].type == 1">
                  <template v-if="pagetype == 1">
                    <div style="display: inline-flex; width: 100%">
                      <el-input
                        style="flex: 1"
                        v-model.trim="modelData[k]"
                        :placeholder="viewData[k].placeholder"
                        clearable
                        v-verify="viewData[k].verify"
                      >
                      </el-input>
                      <el-select
                        v-model="viewData[k].whereStr"
                        class="whereSelect"
                        placeholder="包含"
                      >
                  ...
                  <template v-if="pagetype == 2">
                    <div style="display: flex; align-items: center">
                      <el-input
                        v-model.trim="modelData[k]"
                        @[viewData[k].elEvent&&viewData[k].elEvent.eventName||null]="
                          handleEventFunc(k, $event)
                        "
                        :placeholder="viewData[k].placeholder"
                        clearable
                        v-verify="viewData[k].verify"
                      >
                        <template slot="prepend" v-if="viewData[k].prepend">{{
                          viewData[k].prepend
                        }}</template>
                        <template slot="append" v-if="viewData[k].append">{{
                          viewData[k].append
                        }}</template>
                      </el-input>
                ...
                <template v-else-if="viewData[k].type == 2">
                  <el-checkbox-group
                    v-model="checkboxMultiple[k]"
                    v-if="checkboxMultiple[k]"
                  >
                    <el-checkbox
                      v-for="(checkitem, checkboxindex) in viewData[k].options"
                ...
                <template v-else-if="viewData[k].type == 3">
                  ...
                  >
                    {{ analyticPrmTtpe(radioitem.label)
                    }}<span v-if="radioitem.tip" style="color: red"
                      >({{ radioitem.tip }})</span
                    ></el-radio
                  >
                  ...
                  >
                    <div
                      slot="content"
                      style="min-height: 36px; width: 518px; line-height: 18px"
                      v-if="k == 'yfstdin'"
                    >
                      ...
                      >
                      单票运价：
                      指与客户每票谈的特殊报价(单票询价),通常针对客户手上的大货/重货所指定的运价。
                      <p style="margin: 5px 0px">
                        唯凯标准运价：
                        唯凯指针对所有客户,按起运港和目的港来区分的唯凯标准运价。
                      </p>
                      客户协议运价：
                ...
                <template v-else-if="viewData[k].type == 4">
                  <el-select
                    v-model="modelData[k]"
                    :placeholder="viewData[k].placeholder || '请选择'"
                    :filterable="viewData[k].filterable"
                    clearable
                  ...
                  >
                    <el-option
                      label="全部"
                      value="全部"
                      v-if="viewData[k].allSelect"
                    ></el-option>
                    <el-option
                      v-for="(selitem, index) in viewData[k].options"
                      :key="index"
                      :label="selitem.label"
                      :value="selitem.value"
                    ></el-option>
                ...
                <template v-else-if="viewData[k].type == 5">
                  ...
                  >
                    ...
                    <el-option
                      v-for="(selMulitem, index) in viewData[k].options"
                      :key="index"
                ...
                <template v-else-if="viewData[k].type == 6">
                  <el-date-picker
                    v-model="modelData[k]"
                    :type="viewData[k].datetype || 'date'"
                    :value-format="viewData[k].format || 'yyyy-MM-dd'"
                    :placeholder="viewData[k].placeholder || '选择日期'"
                    :editable="viewData[k].editable"
                    :picker-options="
                      viewData[k].pickerOptions ||
                      setPickerOptions(viewData[k].pickerOptionsType)
                    "
                    @[viewData[k].elEvent&&viewData[k].elEvent.eventName||null]="
                      handleEventFunc(k, $event)
                    "
                ...
                <template v-else-if="viewData[k].type == 9">
                  <div
                    v-for="inputItem in viewData[k].options"
                    :style="{
                      display: 'inline-block',
                      width: !viewData[k].input
                        ? '160px'
                        : viewData[k].input == '1'
                        ? '130px'
                        : '110px',
                  ...
                  >
                    <span
                      style="
                        ...
                        text-align: right;
                      "
                      >{{ inputItem.title }}</span
                    >
                    <el-input
                      v-model="modelData[inputItem.id]"
                      type="text"
                      @change="
                        fpChange(inputItem.id, $event, inputItem.linkage)
                      "
                      :class="{ 'input-required': inputItem.required }"
                      style="width: 80px"
                      v-if="!viewData[k].input || viewData[k].input == 1"
                      :id="inputItem.id"
                    >
                    </el-input>
                    <div
                      :style="viewData[k].style"
                      style="display: inline"
                      v-if="viewData[k].input == '2'"
                    >
                      {{ modelData[inputItem.id] }}
                    </div>
                  </div>
                </template>
                ...
                <template v-else-if="viewData[k].type == 11">
                  <pageSelect
                    :pagetype="viewData[k].pagetype || 1"
                    :area="orderArea"
                    :modelval.sync="modelData[k]"
                    @change="getWtkhMsg($event, 1, viewData[k].linkage)"
                    :disabled="computedAttribute(k)"
                    :sytype="viewData[k].sytype"
                  ></pageSelect>
                  <i
                    title="新增委托客户"
                    @click="openNewCompApply"
                    v-if="
                      viewData[k].openNewCompApply || viewData[k].contentStyle
                    "
                ...
                <template v-else-if="viewData[k].type == 10">
                  ...
                  <el-select
                    :style="{
                      'max-width':
                        viewData[k].showCustomerRel && viewData[k].isnewadd
                          ? '140px'
                          : '100%',
                    }"
                    v-model="
                  ...
                  >
                    <el-option
                      v-for="selitem in wtxmOptions"
                      :key="selitem.id"
                      :label="selitem.usr_name"
                      :value="selitem.id"
                    ></el-option>
                  </el-select>
                  <customerRel
                    style="height: 28px; margin-left: 6px; pointer-events: auto"
                    ref="customerRel"
                    v-if="viewData[k].showCustomerRel && modelData.fid"
                    :fid="modelData.fid || ''"
                    :gid="modelData.gid || ''"
                    :immediate="viewData[k].isnewadd"
                    @changeRelData="handleChangeRelData"
                    :area="orderArea || ''"
                    :relList="modelData.customerRelList"
                  >
                  </customerRel>
                  <span
                    v-if="
                      viewData[k].showCustomerRel &&
                      modelData.fid &&
                      RelData.length &&
                      viewData[k].isnewadd
                    "
                  ...
                  >
                    <span style="word-break: keep-all"
                      >{{ RelData[0] }},{{ RelData[1] }}</span
                    >
                    <a v-if="RelData[2]" :href="'mailto:' + RelData[2]"
                      >,{{ RelData[2] }}</a
                    >
                  </span>
                </template>
                ...
                <template v-else-if="viewData[k].type == 15">
                  <div style="display: flex">
                    <el-date-picker
                      v-model="modelData[k].begin"
                      :type="viewData[k].format ? 'datetime' : 'date'"
                      :value-format="
                        viewData[k].format || 'yyyy-MM-dd HH:mm:ss'
                      "
                      :format="viewData[k].format || 'yyyy-MM-dd'"
                      placeholder="开始日期"
                      :editable="viewData[k].editable"
                      class="rangeDate"
                      v-if="modelData[k]"
                      clearable
                      :picker-options="viewData[k].pickerOptions"
                    ></el-date-picker>
                    <span style="color: #666; padding-top: 3px">至</span>
                    <el-date-picker
                      v-model="modelData[k].end"
                      :type="viewData[k].format ? 'datetime' : 'date'"
                      :value-format="viewData[k].format || 'yyyy-MM-dd'"
                      :format="viewData[k].format || 'yyyy-MM-dd'"
                      placeholder="结束日期"
                      :editable="viewData[k].editable"
                      :default-time="viewData[k].format ? '23:59:59' : ''"
                      class="rangeDate"
                      v-if="modelData[k]"
                      clearable
                      :picker-options="viewData[k].pickerOptions"
                  ...
                  >
                    ...
                    <span style="display: inline-block; min-width: 550px">
                      <el-radio
                        ...
                        :label="radioitem2"
                        :key="radioIndex2"
                        :style="{ 'min-width': viewData[k].minW + 'px' }"
                        >{{ radioitem2 }}</el-radio
                      >
                    </span>
                    <span
                      ...
                      v-if="
                        ...
                        modelData[k] &&
                        projectIsWecan &&
                        ['CIF', 'C&F', 'DDP', 'DAP', 'DAT'].includes(
                          modelData[k]
                        )
                      "
                      >{{ `选择${modelData[k]}不会进入海外D/N账单流程!` }}</span
                    >
                    <span
                      style="color: red; font-size: 14px"
                      v-if="
                        ...
                        modelData[k] &&
                        projectIsWecan &&
                        ['FOB', 'FCA', 'EXW'].includes(modelData[k])
                      "
                      >{{ `选择${modelData[k]}会进入海外D/N账单流程!` }}</span
                    >
                  </div>
                </template>
                ...
                <template v-else-if="viewData[k].type == 18">
                  <div
                    :style="viewData[k].style"
                    class="type18content"
                    style="text-indent: 8px; line-height: 28px"
                    :disabled="computedAttribute(k)"
                  >
                    {{ modelData[k] }}
                  </div>
                ...
                <template v-else-if="viewData[k].type == 20">
                  <pageSelect
                    :pagetype="viewData[k].pagetype"
                    :pagedata="viewData[k].pagedata"
                    :modelval.sync="modelData[k]"
                    :disabled="computedAttribute(k)"
                    :viewdata="viewData[k].viewdata"
                    :servicecode="viewData[k].servicecode"
                    :area="orderArea"
                    :wageinout="viewData[k].wageinout"
                    v-if="viewData[k].pagedata||viewData[k].pagetype"
                  ></pageSelect>
                </template>
                ...
                <template v-else-if="viewData[k].type == 21">
                  ...
                  >
                    <el-option
                      v-for="selitem in viewData[k].options"
                      :key="selitem.value"
                      :label="selitem.label"
                      :value="selitem.value"
                    ></el-option>
                ...
                <template v-else-if="viewData[k].type == 23">
                  ...
                  >
                    <el-option-group
                      v-for="group in viewData[k].options"
                      :key="group.label"
                      :label="group.label"
                    >
                      <el-option
                ...
                <template v-else-if="viewData[k].type == 25">
                  ...
                  >
                    <el-option
                      v-for="item in getStoreArr(viewData[k].linkage)"
                      :key="item"
                      :label="item"
                      :value="item"
                    ></el-option>
                ...
                <template v-else-if="viewData[k].type == 26">
                  ...
                  >
                    <el-option
                      v-for="item in getFlowAreaArr(viewData[k].linkage)"
                      :key="item"
                      :label="item"
                      :value="item"
                    ></el-option>
                ...
                <template v-else-if="viewData[k].type == 27">
                  ...
                  >
                    <el-option
                      v-for="item in getMateriaArr(viewData[k].linkage)"
                      :key="item"
                      :label="item"
                      :value="item"
                    ></el-option>
                ...
                <template v-else-if="viewData[k].type == 28">
                  ...
                  >
                    <el-option
                      v-for="item in getGyArr(viewData[k].linkage)"
                      :key="item"
                      :label="item"
                      :value="item"
                    ></el-option>
                ...
                <template v-else-if="viewData[k].type == 31">
                  ...
                  >
                    <el-option
                      v-for="item in getGbzArr(viewData[k].linkage)"
                      :key="item"
                      :label="item"
                      :value="item"
                    ></el-option>
                ...
                <template v-else-if="viewData[k].type == 33">
                  <el-autocomplete
                    ...
                    :placeholder="viewData[k].placeholder || '请输入内容'"
                    @select="handleSelect"
                    hide-loading
                    clearable
                    @focus="autocompleteFocus(k)"
                  >
                  </el-autocomplete>
                </template>
                ...
                <template v-else-if="viewData[k].type == 34">
                  <div style="height: 28px; display: flex; align-items: center">
                    <span style="width: 125px">{{
                      modelData[k] | formatDate("hh:mm")
                    }}</span>
                    <i
                      class="el-icon-edit-outline"
                      @click="qfsjDialogFunc"
                      title="航班维护"
                      style="font-size: 20px; color: #30c76c; cursor: pointer"
                    ></i>
                    <i
                      title="刷新"
                ...
                <template v-else-if="viewData[k].type == 35">
                  <div
                    v-if="modelData.isinwageallin == 4"
                    class="inwageal"
                    style="display: flex"
                  >
                    <span
                      ...
                      "
                      class="numspan"
                      @click="$emit('inwageallinpriceFunc', -2)"
                      >-2</span
                    >
                    <span
                      class="numspan"
                      @click="$emit('inwageallinpriceFunc', -1)"
                      >-1</span
                    >
                    <el-input
                      style="width: 90px"
                      v-model="modelData[viewData[k].realBindField]"
                      v-verify="'fixedTwo'"
                    ></el-input>
                    <span
                      class="numspan"
                  ...
                  >
                    <el-input
                      :style="{
                        width: viewData[k].isnewadd ? '160px' : '178px',
                      }"
                      v-model="modelData[k]"
                      v-verify="'fixedTwo'"
                    >
                      <template slot="append" v-if="viewData[k].isnewadd"
                        >/KG</template
                      >
                    </el-input>
                  </div>
                ...
                <template v-else-if="viewData[k].type == 36">
                  ...
                  <div>
                    (实际计费重量 - 制单计费重量) *我司分泡比例 + 制单计费重量
                  </div>
                </template>

                <template v-else-if="viewData[k].type == 37">
                  <area-select
                    :model.sync="modelData[k]"
                    :pagetype="viewData[k].pagetype"
                    :required="viewData[k].required"
                    :disabled="computedAttribute(k)"
                    :placeholder="viewData[k].placeholder"
                    :multiple="viewData[k].multiple"
                    :usedarea="viewData[k].usedarea"
                    :valuetype="viewData[k].valuetype"
                ...
                <template v-else-if="viewData[k].type == 38">
                  <new-form-cmpt
                    class="inputGroup"
                    :required.sync="inputGroupRequired[k]"
                    :model-data="modelData"
                    :view-data="viewData[k].inputgroups"
                    :pagetype="2"
                  >
                  </new-form-cmpt>
                </template>
                ...
                <template v-else-if="viewData[k].type == 39">
                  ...
                  >
                    ...
                    <el-input
                      v-model="modelData.address_detail"
                      :placeholder="viewData[k].placeholder || '详细地址'"
                      class="input-required"
                    >
                    </el-input>
                  </div>
          ...
          <!-- <div
            :style="[
              { order: viewData[k].order >= 0 ? viewData[k].order : index },
              { display: viewData[k].hidden ? 'none' : '' },
            ]"
            v-if="viewData[k].occupyRestSpace"
            style="width: 100%"
          ></div> -->
        </template>
    ...
    >
      ...
      <span
        class="spanSlide"
        :class="[setPageshow ? 'slideOff' : 'slideOn']"
        @click="setPageshow = !setPageshow"
        :title="[setPageshow ? '隐藏条件' : '更多条件']"
      ></span>
      <formFieldConfig
        :name="name"
        style="float: right; margin-top: 14px; margin-right: 5px"
        :formFieldArr.sync="formFieldArr"
        :allViewData="allViewData"
        :allViewData2="allViewData2"
        :allViewData3="allViewData3"
        :viewData="viewData"
        :basicField="getBasicField()"
        v-show="formFieldConfig"
      ></formFieldConfig>
    </div>
...
import { getMomentDate } from "../../api/localStorage.js";
...
export default {
  props: {
    ...
    viewDataBasicField: {
      type: Array,
      default() {
        return [[], Object.keys(this.viewData)];
      },
    },
    name: {
      //路由组件名
  ...
  methods: {
    ...
    openNewCompApply() {
      ...
      window.open(
        `${location.protocol}//${location.host}/wffcustommanagement?username=${username}&key=${key}`
      );
    },
    analyticPrmTtpe(attr) {
      //解析viewdata里面配置参数的类型
      ...
      return (restaurant) => {
        return (
          restaurant.value.toLowerCase().indexOf(queryString.toLowerCase()) ===
            0 || PinyinMatch.match(restaurant.value, queryString)
        );
      };
    },
    handleSelect(item) {
    ...
    getStoreArr(id) {
      ...
      let arr = [];
      cargoData.map((item) => {
        if (item.area == (id ? this.modelData[id] : this.orderArea)) {
    ...
    setWtxmFunc(i) {
      ...
      this.$watch(
        ...
        function (newval) {
          if (
            this.modelData[i] != undefined &&
            this.modelData[i] != -1 &&
            gid !== undefined &&
            this.modelData.hasOwnProperty("fid")
          ) {
            if (this.viewData[i].pagetype == 8 && this.system) {
              //只有在新增页面，需要根据选择的系统，重新过滤委托项目
              this.$set(this.modelData, name, gid || "");
            }
    ...
    setRadioDefault(i) {
      ...
      this.$watch(
        function () {
          return (
            this.viewData[i] && this.viewData[i].default + this.modelData[i]
          );
        },
        function (newval) {
        ...
        this.$watch(
          ...
          function (newval) {
            if (newval) {
              ...
              this.$set(this.modelData, "address_fix", [
                newval.country,
                newval.pro,
                newval.city,
                newval.areade,
    ...
    setdata(i) {
      ...
      this.$watch(
        ...
        function (newv, oldv) {
          ...
          //begintype 开始日期默认值 1加一天 2前三个月 3前一个月 4往前推5天 5往前推1天
    ...
    getBasicField() {
      ...
      if (sameTableHeadComp[0].includes(tempname)) {
      ...
      else if (tempname == "priceFreight.vue") {
        result = [[
          "areaname",
          "sfg",
           "jg",
        ...
        result = [[
          ...
          "adddate"
                       ...
                       result = [
                        ["sfg", "mdg","validityDate", "areaname","twocode",  "wtxm"], // 固定查询字段
                        [
    ...
    getSearchData() {
      ...
      //判断必填
      let inputGroupRequired = Object.values(this.inputGroupRequired).filter(
        (i) => !!i
      );
      if (inputGroupRequired.length) {
        this.$emit("update:required", inputGroupRequired[0].replace("：", ""));
      } else {
        for (let i in this.viewData) {
          if (
            this.viewData[i].required &&
            ...
            let alertTitle =
              this.viewData[i].alertTitle || this.viewData[i].title;
            this.$emit("update:required", alertTitle.replace("：", ""));
            break;
          } else {
    ...
    handleEventFunc(k, e) {
      ...
      if (elevent) {
        ...
        if (elevent.eventFunc) {
          if (elevent.params) {
            if (typeof elevent.params == "object") {
              //多个参数用数组，调用的方法中需要几个参数就写几个参数 funcName(x1,x2,...)
