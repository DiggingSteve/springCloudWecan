<template>
  <div>
    <div class="main" v-show="!outsidePage" :class="{isoutside:system=='outside'}">
      <div class="main-left" :class="[mainLeftShow?'aniShow':'aniHide']">
        <div>
          <div class="logo" :class="system+'logo'">
            <i :class="recorderNumberLoading?'el-icon-loading':'el-icon-refresh-right'" v-show="projectIsWecan"
              style="color:#fff;font-size:16px" @click="getRecorderNumber()" title="刷新左侧菜单待处理的票数"   v-if="system!='priceSystem'"></i>
          </div>
          <!-- 站点系统服务选择框 -->
          <navTopSel :cities="cities" v-if="cities.length" :serviceBasic="serviceBasic">
          </navTopSel>
          <!-- 展开按钮 -->
          <div v-if="cities.length||system=='outside'"
            style="width:100%;display:flex;justify-content: center;align-items:center;cursor:pointer;margin-top: 20px;"
            :class="[toggleSystemShow?'animate__animated animate__flipInY':'animate__animated animate__flipInX']">
            <span @click="toggleSystem" class="toggleSystem">{{toggleSystemShow?'全部展开':'全部收起'}}</span>
            <i :class="[toggleSystemShow?'el-icon-caret-bottom':'el-icon-caret-top']" class="toggleSystem"></i>
          </div>
        </div>
        <!-- 导航树 -->
        <navlist :list="newnavdata" :recorderNumberNew="recorderNumberNew" style="height: auto;margin-top:12px">
        </navlist>

      </div>
      <!-- 侧边栏 -->
      <div class="displayArrow" :style="{left:!mainLeftShow?0:260+'px'}">
        <a class="pngf" :class="[mainLeftShow?'pngfix':'pngfixshow']" href="javascript:void(0);"
          @click="ArrowToggle"></a>
      </div>

      <div class="main-right">
        <!-- 各个设置等功能按钮 -->
        <navSetting @success="userSettingSuccess" @backindex="outsidePage=true" :cities="cities"
          :serviceBasic="serviceBasic" :userSetting.sync="userSetting"
          v-if="cities.length&&serviceBasic.length>0||system=='outside'" :system="system"></navSetting>
        <!-- 头部导航 -->
        ...
        <div class="main-right-content">
          <keep-alive :include="tabRelKeep">
            <!-- <router-view ref="search" :class="$route.name" v-if="$store.state.isLocalhost"></router-view> -->
            <!-- 本地不加key,方便开发；线上加key,需要匹配整个路由以缓存页面 -->
            <router-view ref="search" :class="$route.name" :key="$route.fullPath"></router-view>
          </keep-alive>
        </div>
      </div>
    ...
    <!-- 外网导引页 -->
    <!-- <outside v-show="outsidePage" @linkto="linkto" :autList="outsideNav"></outside> -->
  </div>
</template>
...
<script>
  ...
  import {
    cassifyObj,basicDataPages
  } from "@/common/fields"
  import { getStorage, getWtdata, getAllTemplates, getTzxm,loadCache } from '@/api/getBasicStorageData'
  ...
  export default {
    ...
    data() {
      ...
      let createPageArr = ["asignSpaceSearch", "gnUnfinish", "orderTakingOverDate", "listConfirm", "listConfirmOut", "orderTaking", "cabinUnconfirmed", "listConfirmOverDate", "asignSearchMonitor","dataAnaly","outMawbAdd","portData","priceFreight","frmSystemWageAI","frmSystemWageAE"];//通过mixin注入的页面

      createPageArr = [...createPageArr, ...Object.keys(cassifyObj), ...Object.keys(basicDataPages)]//合并多tab页面的路由和基础数据层页面
    ...
    created: function () {
      ...
      sessionStorage.setItem('buildVersionNumber', process.env.buildVersionNumber)
      ...
      // this.$watch(() => {
      //   return this.userSetting.showSystemCount + this.userSetting.refreshInterval
      // }, () => {
      //   if (recorderNumberInterval) {
      //     clearInterval(recorderNumberInterval);
      //     recorderNumberInterval = '';
      //   }
      //   if (this.userSetting.showSystemCount && this.userSetting.refreshInterval) {
    ...
    methods: {
      ...
      setNavDisplay() {
        if (this.newnavdata.length > 0) {//选择系统，隐藏导航模块

          let flage = (this.$store.state.systemCheck.length == 0 || this.$store.state.systemCheck.includes("空出")) && this.$store.state.areaState.includes(this.conditionalAreaForBuild);
          ...
          this.$nextTick(() => {
            ...
            // this.controlDn.forEach(i => {
            //   if (this.navDataGroup[i]) {
            //     this.navDataGroup[i][0].hidden = !this.$store.state.areaState.includes("市场部") && !this.$store.state.areaState.includes("海外部")
            //   }
            // })
            ...
            this.newnavdata[0].children.forEach(i => {
              if (i.autname && i.autname.includes('操作层')) {
                if (this.$store.state.systemCheck.includes(i.autname.substr(0, 2))) {
                  i.hidden = false
                } else {
                  i.hidden = true;
                  if (this.$refs.navtabs) {
                    let name = i.autname.replace('层', '');
                    if (this.$refs.navtabs.classification[name] && this.$refs.navtabs.classification[name].length > 0) {
      ...
      setWorkUnFinishSearchData(temp) {//权限控制，赋值查询条件
        ...
        let intervalID = setInterval(() => {
          if (temp == "costConfirmationkf" || temp == "costConfirmationhx" || temp == "gnCostConfirm") {

            if (resultid == 11 || resultid == 12 || resultid == 16) {             //航线驳回费用  11       客服驳回费用12  国内服务驳回费用16 需要打开第二个tab
            ...
            for (let key of Object.keys(resultdic)) {
              ...
              if (arr.length) {
                ...
                vmsearch.inputModelData[key] = {
                  begin:
                    resultdic[key].begin && new Date(resultdic[key].begin).Format("yyyy-MM-dd") || "",
                  end: resultdic[key].end && new Date(resultdic[key].end).Format("yyyy-MM-dd") || ""
                };
                ...
                if (
                  ...
                  vm.$set(
                    ...
                    "whereStr",
                    //"equal"
                    key != 'rollbackcust' && key != 'rollbackroute' ? "equal" : "like"
                  );
                  if (key == 'rollbackcust' || key == 'rollbackroute') {
      ...
      assignFunc(data) {//处理导航数据
        ...
        if (this.system == 'bo') {
          ...
          this.createNav(dataArr, { resultguid: '0', resultmessage: '', resultstatus: 0 });
        }

      },
      assignArea(areadata) {//生成权限地域选项
        var areaArr = [];
        let cityCode = [];
      ...
      setRoutes(nav) {//生成导航ui的树结构
        ...
        nav.forEach(i => {
          ...
          if (['客服操作', '航线操作', '单证操作', '运控操作'].includes(i.autname)) {
      ...
      getRecorderNumber(item) {//获取导航待处理数字
        if (!this.projectIsWecan || this.recorderNumberLoading) {
          return;
        }
        this.recorderNumberLoading = true
        let data = { area: JSON.parse(localStorage.cityCode).map(i => i.label).toString() };
        if (this.system == 'bo') {
        ...
        if (item && item.module) {
          ...
          data.module = [...new Set(JSON.parse(sessionStorage.nav).map(i => i.module))].toString()
        }
        this.$axios({
          method: 'get',
          url: (this.system == 'bo' ? this.$store.state.webApi : this.$store.state.wcWebApi) + 'api/SystemCount',
          params: data,
          loading: false,
        }).then(res => {
      ...
      setRecorderNumberNew(type = 1) {//计算待处理数字
        //type1 全部模板 2单独模块
        let obj = {}
        let moduleArr = []
        ...
        this.recorderNumberArrayNew.forEach(i => {
          ...
          if (this.$store.state.areaState.includes(i.area) && this.$store.state.systemCheck.includes(i.system) || this.system == 'outside') {
            if (!obj[name]) {
              obj[name] = i.countnum
            } else {
              obj[name] += i.countnum
...
<style lang="less" scoped>
  ...
  /deep/ .isoutside {
    .main-left {
      background: transparent linear-gradient(180deg, #FEAE2D 0%, #EF8609 100%) 0% 0% no-repeat padding-box;
