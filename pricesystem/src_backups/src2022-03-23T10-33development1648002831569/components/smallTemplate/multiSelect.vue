...
//class 是输入框占父级的宽度
    ...
    export default {
        ...
        mounted(){
          ...
          if(this.pagetype=='6'){
             let groupData = JSON.parse(localStorage.getItem('hbinfo'))
             //console.log(groupData)
                  for (let i in groupData) {
                      let item=groupData[i]
                         
                               this.options.push({ 'value':  item.ThreeCode, 'label':  item.ThreeCode, ready02: item.Ready01, id:  item.Ready01, key:  item.ThreeCode +  item.Ready01  })
