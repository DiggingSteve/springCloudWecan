  ...
  <el-dialog title="用户设置" width="550px" top="30px" center v-if="userSettingVisible" custom-class="user-setting-dialog"
    ...
    <div class="setting-c">
      <div class="setting-group">
            ...
            <table>
              <tr>
                <td v-for="(group) in systemArrList" :key="group.label">
                  <el-option-group :label="group.label">
                    <el-option v-for="item in group.options" :key="item.value" :label="item.label" :value="item.value"
                      style="width:204px;" :disabled="item.disabled"></el-option>
                  </el-option-group>
                </td>
              </tr>
            </table>
        ...
        <div class="setting-item serviceSelect" v-show="paneShow" style="display:none">
          <div class="setting-info">
            <p class="item-text">
              左侧菜单栏默认匹配的服务
...
<script>
  export default {
    ...
    computed: {
      ...
      systemCheckedAll: {
        get() {
          let length = this.systemArrList.map(i => i.options).flat().filter(i=>!i.disabled).length
          // this.systemArrList.forEach(i => {

          //   length += i.options.filter(item=>!item.disabled).length
          // })
        ...
        set(val) {
          if (val) {
            this.systemArrList.forEach(item => {
              item.options.forEach(item2 => {
                if (!this.systemCheckedBackup.includes(item2.value)&&!item2.disabled) {
