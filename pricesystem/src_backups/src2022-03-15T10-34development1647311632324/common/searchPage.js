...
import frmSystemWage from '@/priceSystem/frmSystemWage.vue'
...
export const createPage = function (name) {
  ...
  if (["listConfirm", "listConfirmOut", "orderTaking", "orderTakingOverDate", 'cabinUnconfirmed', 'listConfirmOverDate'].includes(name)) {
    return {
      name: name,
      mixins: [acceptPage],
      data() {
  ...
  if(["frmSystemWageAI","frmSystemWageAE"].includes(name)){
    return {
      name: name,
      mixins: [frmSystemWage],
      data() {
        return {
  ...
  if (Object.keys(cassifyObj).includes(name)) {
    ...
    return {
      ...
      <el-button-group  class="buttonTabs" >
          <el-button @click.native="mng=true" :class="[mng?'active':'']" >${first.title}</el-button>
          <el-button @click.native="mng=false" :class="[!mng?'active':'']" >${second.title}</el-button>
      </el-button-group>
      <keep-alive>
        <pagefirst class="cre" pagetype="1" :monitor="${!first.nomonitor}?'1':''" ${first.propsStr} v-if="mng" ref="fir"></pagefirst>
        <pagesecond class="cre" pagetype="2" :monitor="${!second.nomonitor}?'2':''" ${second.propsStr}  v-if="!mng" ref="sec"></pagesecond>
      </keep-alive>
      </div>
      `
    }
