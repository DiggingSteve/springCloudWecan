  ...
  <div>
    ...
    <tableCompt :name="name" :table-data-res="tableDataRes">
      ...
      <template v-slot:detailTitle="pdata">
        ...
        >
          <span>{{
            inputModelData.scale || inputModelData.scope
              ? "运价"
              : pdata.data.value.title
          }}</span>
            <div
            class="input-item"
            style="
...
export default {
  ...
  data() {
    return {
      ...
      booleanArr:[{code:true,name:"是"},{code:false,name:"否"}],
  ...
  methods: {
    ...
    filterDataByWeight(dataArr, calWeight, grossWeight, volType) {
      ...
      dataArrCopy.forEach((item) => {
        ...
        var exactObj = { weight: exactWeight, vol: volType };
    ...
    /**获取卡车费 列表展示运价需要加上卡车段的费用 当不显示卡车段时 */
    getTruckFee(weight) {},
    /**
     * @vol 当前对应的比例
     * @weight 当前对应重量
 ...
 // 查找当前格子一口价
      ...
      if (!!matchFixedPrice) {
        return !!matchFixedPrice.diff >= 0
          ? (matchFixedPrice.diff / this.currentCurrency).toLocaleString()
          : "--";
      }
      ...
      var isVolNoSelect = currentRow.volArr.length == 0;
      var isWeightNoSelect = currentRow.weightArr.length == 0;
      var isPackageNoSelect = currentRow.packageTypeArr.length == 0;
      var isCusNoSelect = currentRow.cusArr.length == 0;
      ...
      var packageCusMap = currentRow["packageCusDiffMap"];
      var packageDiff = packageCusMap[this.selectedPackageType] * 1;
      var cusDiff = packageCusMap[this.selectedCusType] * 1;
      var isVol = !!matchVol; //是否有匹配体积
      var isWeight = !!matchWeight;
      var isCus = Number.isFinite(cusDiff);
      var isPackage = Number.isFinite(packageDiff);

      if (weight.title == "MIN") {
        return currentRow.minPrice == 0
          ? "--"
          : (currentRow.minPrice / this.currentCurrency).toLocaleString();
      }
      var val =
        (isVol || isVolNoSelect) &&
        (isWeight || isWeightNoSelect) &&
        (isCus || isCusNoSelect) &&
        (isPackage || isPackageNoSelect)
          ? (
              ((isVolNoSelect ? 0 : matchVol.diff * 1) +
                (isWeightNoSelect ? 0 : matchWeight.diff * 1) +
                (isPackageNoSelect ? 0 : packageDiff) +
                (isCusNoSelect ? 0 : cusDiff) +
                currentRow.standardPrice * 1) /
      ...
      if (this.isExactSearch && currentRow.exactObj.weight != "+0kg") {
        ...
        else minPrice = currentRow.minPrice;
        if (Number.isFinite(total) && total < minPrice) {
          currentRow.exactObj.weight = "+0kg";
          this.setCellValue(vol, weight, currentRow);
        }
      }
      return val;
    },
    ...
    /**精确筛选价格 */
    filterExactTable() {
      var vol = this.typeStatus == "1" ? this.inputModelData.vol : "";
