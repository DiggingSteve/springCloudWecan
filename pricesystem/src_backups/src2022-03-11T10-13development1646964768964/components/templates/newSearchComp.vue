<template>
  <div class="new-search-comp">
    ...
    <el-collapse-transition>
      <div
        v-show="setPageshow"
        class="page-container-main searchCmpt"
        :pagename="name"
      >
        <div
          v-for="item of fieldTypeArr"
          :key="item.fieldtype"
          :class="['fieldtype-c']"
        ...
        >
          ...
          <p class="fieldtype-title">
            ...
            <span
              class="add-btn"
              @click="
                templateConfig.visible = true;
                templateConfig.activeFieldtype = item.fieldtype;
              "
            >
          ...
          <template
            v-for="(field, index) in getSelectedFieldByFieldtype(
              item.fieldtype,
              templateField
            )"
          >
            <div
              v-if="viewData[field] && !viewData[field].hidden"
              :key="field"
              :id="field"
              ...
              :style="[
                {
                  order:
                    viewData[field].order >= 0 ? viewData[field].order : index,
                },
                viewData[field].idStyle,
              ]"
            >
              <div
                :class="[
                  'input-item',
                  { itemDisabled: viewData[field].disabled },
                ]"
                :style="viewData[field].itemStyle"
              >
                <div class="input-title" :style="[viewData[field].titleStyle]">
                  {{ viewData[field].title }}
                </div>

                <div
                  class="input-content"
                  :style="[viewData[field].contentStyle]"
                  :class="{
                    'input-required': viewData[field].required,
                    contentDisabled: viewData[field].disabled,
                  }"
                >
                  ...
                  <template v-if="viewData[field].type == 1">
                    <div style="display: inline-flex">
                      <el-input
                        style="flex: 1"
                        v-model.trim="modelData[field]"
                        :placeholder="viewData[field].placeholder"
                        clearable
                        v-verify="viewData[field].verify"
                      >
                      </el-input>
                      <el-select
                        v-model="viewData[field].whereStr"
                        class="whereSelect"
                        placeholder="包含"
                      >
                  ...
                  <template v-else-if="viewData[field].type == 4">
                    <el-select
                      v-model="modelData[field]"
                      placeholder="请选择"
                      :filterable="viewData[field].filterable"
                      clearable
                      @change="formatField(viewData[field].initialField)"
                      default-first-option
                    >
                      <el-option
                        label="全部"
                        value="全部"
                        v-if="viewData[field].allSelect"
                      ></el-option>
                      <el-option
                        v-for="(selitem, index) in viewData[field].options"
                        :key="index"
                        :label="selitem.label"
                        :value="selitem.value"
                      ></el-option>
                  ...
                  <template v-else-if="viewData[field].type == 5">
                    ...
                    >
                      <li
                        ...
                        @click="selmultiAll(field)"
                      >
                        <span>全部</span>
                      </li>
                      <el-option
                        v-for="(selMulitem, index) in viewData[field].options"
                        :key="index"
                  ...
                  <template v-else-if="viewData[field].type == 13">
                    <pageSelect
                      :pagetype="-1"
                      :modelval.sync="modelData[field]"
                      :pagedata="wtxmData"
                      :disabled="viewData[field].disabled"
                    ></pageSelect>
                  </template>
                  ...
                  <template v-else-if="viewData[field].type == 14">
                    <div style="display: inline-flex">
                      <pageSelect
                        :pagetype="1"
                        :modelval.sync="modelData[field]"
                        @change="getWtkhMsg($event, 2, viewData[field].linkage)"
                        :disabled="viewData[field].disabled"
                      >
                      </pageSelect>
                      <el-select
                  ...
                  <template v-else-if="viewData[field].type == 15">
                    <div style="display: flex">
                      <el-date-picker
                        v-model="modelData[field].begin"
                        :type="viewData[field].format ? 'datetime' : 'date'"
                        :value-format="
                          viewData[field].format || 'yyyy-MM-dd HH:mm:ss'
                        "
                        :format="viewData[field].format || 'yyyy-MM-dd'"
                        placeholder="开始日期"
                        :editable="viewData[field].editable"
                        class="rangeDate"
                        v-if="modelData[field]"
                        clearable
                        :picker-options="viewData[field].pickerOptions"
                      ></el-date-picker>
                      <span style="color: #666; padding-top: 3px">至</span>
                      <el-date-picker
                        v-model="modelData[field].end"
                        :type="viewData[field].format ? 'datetime' : 'date'"
                        :value-format="viewData[field].format || 'yyyy-MM-dd'"
                        :format="viewData[field].format || 'yyyy-MM-dd'"
                        placeholder="结束日期"
                        :editable="viewData[field].editable"
                        :default-time="viewData[field].format ? '23:59:59' : ''"
                        class="rangeDate"
                        v-if="modelData[field]"
                        clearable
                        :picker-options="viewData[field].pickerOptions"
                  ...
                  <template v-else-if="viewData[field].type == 20">
                    <pageSelect
                      :pagetype="viewData[field].pagetype"
                      :modelval.sync="modelData[field]"
                      :disabled="viewData[field].disabled"
                      :viewdata="viewData[field].viewdata"
                      :servicecode="viewData[field].servicecode"
                    ></pageSelect>
                  </template>
                  ...
                  <template v-else-if="viewData[field].type == 21">
                    ...
                    >
                      <el-option
                        v-for="selitem in viewData[field].options"
                        :key="selitem.value"
                        :label="selitem.label"
                        :value="selitem.value"
                      ></el-option>
                  ...
                  <template v-else-if="viewData[field].type == 23">
                    ...
                    >
                      <el-option-group
                        v-for="group in viewData[field].options"
                        :key="group.label"
                        :label="group.label"
                      >
                        <el-option
                  ...
                  <template v-else-if="viewData[field].type == 26">
                    ...
                    >
                      <el-option
                        v-for="item in getFlowAreaArr(viewData[field].linkage)"
                        :key="item"
                        :label="item"
                        :value="item"
                      ></el-option>
              ...
              <div
                :class="[
                  'remove-btn',
                  pageBasicField[0].includes(field) ? 'fixed' : 'active',
                ]"
    ...
    <el-dialog
      :visible.sync="templateConfig.visible"
      :title="
        '查询模板编辑-' + fieldTypeArr[templateConfig.activeFieldtype - 1].label
      "
      top="5%"
      width="1026px"
      :close-on-click-modal="false"
      @close="setSearchColor()"
    >
      <div class="template-config-c">
        <div class="fieldtype-title-c">
          <div
            ...
            :class="[
              ...
              { active: templateConfig.activeFieldtype === item.fieldtype },
            ]"
            :style="setBgcolor(userSerColor[item.fieldtype])"
            :key="item.fieldtype"
        ...
        <h6>
          ...
          <span v-for="item in fieldTypeArr" class="myColorPicker">
            <el-color-picker
              size="mini"
              v-model="userSerColor[item.fieldtype]"
              color-format="rgb"
              :show-alpha="false"
            >
            </el-color-picker>
          ...
          <el-button
            ...
            >恢复默认颜色</el-button
          >
          <el-button type="primary" @click="saveTemporaryField(2)"
            >保存颜色</el-button
          >
        </h6>
        ...
        <div class="select-area">
          ...
          <div
            class="field-item-c"
            ref="selectDropArea"
            @drop="onDrop($event, 'select-area')"
            @dragenter="dragEnter($event, 'select-area')"
            @dragleave="dragLeave($event, 'select-area')"
            @dragover="$event.preventDefault()"
          >
            <p
              v-for="field of getSelectedFieldByFieldtype(
                templateConfig.activeFieldtype,
                templateConfig.temporaryField
              )"
              :key="field"
              :data-field="field"
            ...
            >
              <span
                :draggable="pageBasicField[0].includes(field) ? false : true"
                @dragstart="dragStart(field, 'select-area')"
                @dragend="dragEnd"
                :title="allViewData[field].title"
              >
                {{ allViewData[field].title }}
                <i
                  class="el-icon-close"
                  v-show="pageBasicField[1].includes(field)"
                  @click="moveTags(2, field)"
                ></i>
              </span>
            </p>
          </div>
        ...
        <div class="not-select-area">
          ...
          <div
            class="field-item-c"
            ref="notSelectDropArea"
            @drop="onDrop($event, 'not-select-area')"
            @dragenter="dragEnter($event, 'not-select-area')"
            @dragleave="dragLeave($event, 'not-select-area')"
            @dragover="$event.preventDefault()"
          >
            <span
              v-for="field of getNotSelectFieldByFieldtype(
                templateConfig.activeFieldtype
              )"
              :key="field"
              :class="[
              ...
              draggable="true"
              @dragstart="dragStart(field, 'not-select-area')"
              @dragend="dragEnd"
              :title="allViewData[field].title"
            >
              {{ allViewData[field].title }}
              <i
                class="el-icon-plus"
                @click="moveTags(1, field)"
                v-show="
                  getSelectedFieldByFieldtype(
                    templateConfig.activeFieldtype,
                    templateConfig.temporaryField
                  ).length < 7
                "
        ...
        <div class="btn-c">
          <el-button
            type="primary"
            @click="templateConfig.temporaryField = getDefaultTemplate()"
            >恢复默认字段</el-button
          >
          <el-button type="primary" @click="saveTemporaryField"
            >保存字段</el-button
          >
        </div>
      </div>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      allViewData: {
        ...
        jobno: {
          title:
            this.name == "tzOperation.vue" || this.name == "tzMonitor.vue"
              ? "工作/调账号"
              : this.name == "managerExamine.vue"
              ? "订单编号"
              : "订舱编号",
          titleStyle: {
            width:
        ...
        zzg: { title: "中转港", type: 1, whereStr: "like", fieldtype: 4 },
        twocode: { title: "二字码", type: 1, whereStr: "like", fieldtype: 4 },
        ...
        hwxz: {
          title:
            this.name == "mawbManageNew.vue" || this.name == "alertManage.vue"
              ? "运单性质"
              : "货物性质",
          type: 5,
          groupid: 115,
        ...
        jyr: { title: "运单转移人", type: 1, fieldtype: 5 },
  ...
  created() {
    ...
    if (
      this.name == "costConfirmation.vue" ||
      this.name == "costConfirmationMonitor.vue" ||
      this.name == "gnCostConfirmMonitor.vue"
    ) {
  ...
  methods: {
    ...
    moveTags(type, field) {
      ...
      if (type == 1) {
        ...
        this.templateConfig.temporaryField =
          this.templateConfig.temporaryField.filter((i) => i != field);
      }
    },
    updateViewData() {
    ...
    getPageBasicField() {
      ...
      let outsideType = [
        "orderAccepting.vue",
        "offertConfirm.vue",
        "outBusinessCancel.vue",
        "outOrderSearch.vue",
        "outOrderImproved.vue",
        "cargoSearch.vue",
        "outerAccount.vue",
        "outInvoice.vue",
        "dataAnaly.vue",
        "invoiceGet.vue",
        "reconciliationMng.vue",
      ];
      let fields = [[], []];
      ...
      if (dzType.includes(tempname)) {
        ...
        fields = [
          ["pono", "status", "mawb", "hawb", "hbh", "hbrq", "sfg", "mdg"],
      ...
      if (tempname == "customerSearch.vue") {
        fields[1].push(
          ...["releasestatus", "pdfinishstatus", "pdfinishdate", "pdfinishman"]
        );
      }
      ...
      if (
        tempname == "listConfirm.vue" &&
        (this.realPageName == "listConfirmOut" ||
          this.realPageName == "listConfirmOverDate")
      ) {
      ...
      if (
        tempname == "costConfirmationMonitor.vue" ||
        tempname == "gnCostConfirmMonitor.vue"
      ) {
        fields[0].push(...["confirmstatus"]);
      }
      if (
        tempname.includes("Monitor") &&
        tempname != "orderSearchMonitor.vue"
      ) {
      ...
      if (tempname == "mawbManageNew.vue" || tempname == "alertManage.vue") {
        fields[0] = ["mawb", "jcpchno", "status", "bookingstatus", "djr"];
        fields[1] = [
          "djdate",
          "jydate",
          "bookingdate",
      ...
      if (tempname == "priceFreight.vue" || tempname == "customerFee.vue") {
        fields[0] = [];
        fields[0].push(
          ...[
            "packageType",
      ...
      if (tempname == "truckFee.vue") {
        fields[0] = [];
        fields[0].push(...["zzg", "mdg", "adddate", "twocode", "addman"]);

        fields[1] = [];
      }
      if (tempname == "priceSearch.vue") {
        fields[0] = [];
        fields[0].push(...["areaname", "sfg", "mdg", "wtkh", "validityDate"]);
        fields[1] = [];
      }
      ...
      if (navInfo && navInfo.othername == "订单层") {
        //订单层不显示订单编号
    ...
    getSearchData() {
      ...
      Object.keys(this.modelData).forEach((k) => {
        ...
        if (!this.viewData[k] || this.viewData[k].nosearch) {
          //nosearch，为true时 过滤掉此查询条件
          return;
        }
    ...
    getNotSelectFieldByFieldtype(fieldtype) {
      ...
      if (this.pageBasicField && Array.isArray(this.pageBasicField)) {
        ...
        Object.keys(this.allViewData).forEach((i) => {
          if (
            this.allViewData[i].fieldtype == fieldtype &&
            !basicArr.includes(i)
          ) {
            if (
              this.name == "managerExamine.vue" &&
              !["pono", "hbh", "hbrq", "mdg"].includes(i)
            ) {
              arr.push(i);
            }
            if (
              this.name != "managerExamine.vue" &&
              !["jobno", "real_hbh", "real_hbrq", "mawbmdg"].includes(i)
            ) {
              arr.push(i);
            }
          }
