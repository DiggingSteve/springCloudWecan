...
axios.interceptors.request.use(function (config) {
  ...
  if (config.method != "get") {
    ...
    config.data.area=config.data.area||localStorage.getItem('realarea');
    config.data.cztime=new Date().Format("yyyy-MM-dd hh:mm:ss");
    if (!config.data.dom) {
      config.data.dom = '出口部'
    }
    ...
    if (!config.noarea&&sessionStorage.system!='outside') {
      if (!config.data.area) {
       ...
       if (config.params.json && !area&&sessionStorage.system!='outside'&&!config.noarea) {
          Message({
            type: 'error',
            message: '请选择指定区域！',
            duration: 1800
    ...
    if (config.params.json) {
      ...
      if (newconfig.where && !config.nofield) {
        ...
        if (showAllField.indexOf(curRoute) != '-1' || showField.length == 0) {
          ...
          arr = arr.concat(['iscomboine', 'pono', 'inwageallinprice', 'inwageallinprice_trans', 'self_real_bp_freight_in', 'cus_real_bp_freight_in', 'self_real_bp_trans_in', 'cus_real_bp_trans_in', 'orderguid', 'profitmode', "creditisbackstatus", 'outsidedom', 'operguid', 'customprice', 'hbzl', 'ddrq', 'priceabout'])
        ...
        if (!newconfig.select || curRoute == 'frmSystemWage'||config.noselect) { //价格管理不传select
