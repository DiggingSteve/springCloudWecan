<template>
  <div :style="{'min-height':name=='cwSearch.vue'?'500px':''}"
    style="width:100%;height:inherit;text-align:left;display:flex;flex-direction:column" :pagename="name">
    <!-- {{ziTableData}} -->
          ...
          {{dataGrouping[row.guid]['hbh']}}<br>{{dataGrouping[row.guid]['hbrq'].substring(0,10)}}<br>{{dataGrouping[row.guid]['qfsj']}}
            ...
            <tr class="trRow" @contextmenu.prevent="rightFun($event,'2',row)">
              ...
              <td v-for="item2 in dataTable[1]" :style="item2.style">
                ...
                <template
                  v-if="item2.field.indexOf('status')!=-1&&!['dzstatus','usr_status','usr_status_cw','creditisbackstatus','billstatus_in','billstatus_out','confirmstatus_wageout','confirmstatus_wagein','mawbconfirmstatus','hawbconfirmstatus','approverstatus','pdfinishstatus','hwstatus'].includes(item2.field)">
                ...
                <template v-else>
                  ...
                  <template v-else>
                    <slot :name="item2.field" :data="{value:row[item2.field],index:row.index,row:row}">
                     <span> {{row[item2.field]}}</span></slot>
                  </template>
                </template>
              </td>
            </tr>
            ...
            <tr v-if="ziTableIndex==row.index&&ziTableData.length>0">
              <td colspan="60" :class="'ziTable'+row.index" :height="getHeight('ziTable'+row.index)">
                <nested-tables v-if="showZiConfig" :style="getLeft" style="position:relative;background:#fffaf2"
                 ...
                 <template slot="operate" slot-scope="props"  v-if="depotModule">
                    <i class="el-icon-setting" @click="modifyCost(props.data)" ></i>
          ...
          <tr class="trRow thfont">
            ...
            <th v-for="(relation,index) in dataTable[1]"
              ...
              :key="index">
               ...
               <slot :name="relation.field+'Title'" :data="{value:relation}">
                 <span v-html="setzhHtml(relation.title,relation)"></span>
                  <span class="sortWrap" v-if="isSort&&!relation.ifsort">
                    <i>
        ...
        <table>
          <tr class="trRow thfont hebingTrFixed" :style="styleObj"
            ...
            <th v-for="(relation,index) in dataTable[0]"
              ...
              <template v-else>
                <span v-html="setzhHtml(relation.title,relation)"></span>
                 <slot :name="relation.field+'Title'" >
                  </slot>
              </template>
              <span class="sortWrap"
          ...
          <template v-for="(row,index) in dataContent">
            ...
            <!--常规表格行开始处-->
            <tr class="trRow" @contextmenu.prevent="rightFun($event,'2',row)">
              <td v-for="(item,i) in dataTable[0]" 
              :class="[item.field]" 
              v-if="item.field!='merge'&& isCreateTd(item.field,index)" 
              :rowspan="calRowSpan(item.field,index)"
              :key="i">
                <template v-if="(expansionSubcolumn=='pono'?(item.field=='pono'||item.field=='shipperno'):(item.field === expansionSubcolumn))&&nestedTableType===2 ">
                  <span style="display:inline-block;min-width:28px">
                      <template v-if="expansionSubcolumnShow(row)">
                        ...
                        <i v-else-if="ziTableIndex!=row.index" class="el-icon-plus" @click="openNestedTables(row.index,row.id,row)"
...
<script>
  ...
  export default {
    ...
    props: {
      ...
      name: String, //父组件名称
      depotModule: Boolean,   // 查询 和 非查询页显示数据
      nestedTableType: {
        //是否有固定列 1默认没有，2有
        type: [Number, String],
        default: 1
    ...
    methods: {
        ...
        } else if(this.name == 'portDepotCostMaintain.vue'){
          return true
        }  else if(this.name == 'portAirlineCostMaintain.vue'){
          return true
      ...
      openNestedTables(index,id,row) {
        if (index == this.ziTableIndex) {
          console.log("=======11111=========")
          console.log(this.ziTableIndex)
          this.ziTableIndex = -1;
        } else {
      ...
      calRowSpan(field,index){
        if(!!!this.mergeRowFields)return true;
         if(!!!this.mergeRowFields[field])return 1;
         let insertIndex=this.searchInsertIndex(this.mergeRowFields[field]["rowspanArr"],index);
        return this.mergeRowFields[field]["rowspanArr"][insertIndex+1]-this.mergeRowFields[field]["rowspanArr"][insertIndex]; // 
      },
      isCreateTd(field,index){
        if(!!!this.mergeRowFields)return true;
        if(!!!this.mergeRowFields[field])return true; // 不是分组条件的 
        let arr=this.mergeRowFields[field]["rowspanArr"];
        ...
        //查找插入数组的位置 数组为顺序输入: [1,3,5,6], 5 输出: 2
      searchInsertIndex(nums,target){
      ...
      ziHeadFixed() {
        ...
        for (let i in nestedTableFixed) {
          if (
            ...
            console.log(JSON.parse(nestedTableFixed[i].jsondata)[0])
    ...
    watch: {
      ...
      dataContent(val){
        var self=this;
          for(var field in self.mergeRowFields){
            let keyArr=self.mergeRowFields[field]["key"] ;
            let rowspanArr=[];
            let flag="";
            ...
            rowspanArr.push(val.length);
            self.mergeRowFields[field]["rowspanArr"]=rowspanArr;
