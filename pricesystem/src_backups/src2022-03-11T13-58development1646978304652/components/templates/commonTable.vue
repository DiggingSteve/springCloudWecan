<template>
  <div>
    ...
    <div v-scroll="onScroll" class="tablebox" :style="divStyle">
      ...
      <div v-if="showHead&&position.scrollTop>0&&isMultiSelect" class="fixedCheckbox MultiSelectCheckbox">
      ...
      <table style="width:100%;text-align:center;box-sizing:border-box">
        <tr v-show="showHead" :style="transform">
          ...
          <th v-for="(item,k) in head" :style="[{'width':item.field=='dwageall'?'120px':(item.width||'')},item.style]"
            style="width:80px!important;font-weight:normal" :class="item.field" v-if="!item.hidden" :key="k">
            <template v-if="item.field!='checkbox'">
              <slot :name="item.field+'Th'" :data="{value:item.title,row:item}">
                <span v-html="item.title" class="title"></span>

                <span class="sortWrap" v-if="item.sort">
                  <i>
                    <i class="sort-ascend" @click.stop="tableSort(item.field,1,$event,'.sort-descend')"></i>
                    <i class="sort-descend" @click.stop="tableSort(item.field,2,$event,'.sort-ascend')"></i>
                  </i>
                </span>
            ...
            <template v-else style="position:relative">
              <el-checkbox @change="allChoice" v-model="checkorno" :disabled="item.disabled" style="width:30px;">
              </el-checkbox>
            </template>
          </th>
        </tr>
        ...
        <template v-for="(row,index) in tableNData">
          <tr :key="index" v-if="!row.hidden" :draggable="draggable" v-mydrag :id="row.id"
            v-show="(putIndex!=index&&!row.thisListHide&&!name)||(row.isdel!='2'&&name)"
            :class="[{ bgColor: colorIndex==index && wagetype==row.wageinout }, rowClassName ? rowClassName({row, index}) : {}]"
            :title="row.adddate&&row.addman?'创建人:'+row.addman+',创建时间:'+row.adddate:''"
            :style="{'background':row.background?row.background:'',fontWeight:row.fontWeight?row.fontWeight:'',height:row.height?row.height:''}">
            <td v-if="isMultiSelect" class="MultiSelectCheckbox" :style="row.style">
              <el-checkbox v-model="row['checked']" @change="$emit('multiSelectChange', { row, index:(paginationShow?((currentPage2-1)*pageSize2+index):index)})"
                :disabled="row['checkboxdisable']"></el-checkbox>

              <slot name="bohui" :data="{index:index,row:row}"></slot>
            </td>
            <th v-if="isRadioSelect" style="padding:0 8px">
              <input v-model="row['checked']" :value="true" type="radio" name="radio" @click="radioFunc(index)"
                v-if="!row['isSeat']">
            </th>

            <td v-for="item in head" :class="[item.field,row.isSeat?'isSeatTd':'']" :key="item.id" v-if="!item.hidden"
              :style="[row.style,item.style,item.tdStyleFunc&&item.tdStyleFunc(row[item.field]),{'display':item.field=='operate'&&row.rowHide?'none':''}]"
              :rowspan="item.field=='operate'?row.rowspan:''">
              <!-- 航线综合查询签单表格 -->
              <!--   <slot name="iconplus" :data="{index:index}" v-if="row[item.field]=='实际'">
                
              </slot> -->
              <slot v-if="!row.isSeat||(row.showField&&row.showField.includes(item.field))" :name="item.field"
                :data="{value:row[item.field],index:paginationShow?((currentPage2-1)*pageSize2+index):index,disabled:row.disabled,firstParentIndex:row.firstParentIndex,indexAll:row.indexAll,row:row}">
                <span
                  v-if="item.field=='hawb' && row[item.field]">{{row[item.field].indexOf(',')!='-1'?(row[item.field].split(',')[0]+(row[item.field].split(',')[1]?',':'')+(row[item.field].split(',')[1]?row[item.field].split(',')[1]:'')+(row[item.field].split(',')[2]?',':'')+(row[item.field].split(',')[2]?row[item.field].split(',')[2]:'')):row[item.field]}}</span>
                <span v-else-if="item.field=='remark'" :title="row[item.field]">
                  {{row[item.field]||emptyCellText}}
                </span>
                <span v-else-if="item.field=='confirmBill'">
                  {{row['isbill']=='1'?'已对账':(row['isbill']=='10'?'已开票':row['confirmstatus']=='未确认'||row['confirmstatus']=='1'?'未确认':'已确认')}}
                </span>
                <span v-else-if="item.field=='iszd'">
                  {{row['iszd']=='0'?'否':'是'}}
                </span>
                <!-- 开始日期 -->
                <span v-else-if="item.field === 'begindate'">
                  {{row.begindate === '1900-01-01 00:00:00' || row.begindate === '' ? '' : row.begindate | formatDate("yyyy-MM-dd") }}
                </span>
                <!-- 结束日期 -->
                <span v-else-if="item.field === 'enddate'">
                    {{ (row.enddate === '1900-01-01 00:00:00' || row.enddate === '') ? '' :  row.enddate | formatDate("yyyy-MM-dd") }} 
                </span>     
                <!-- 修改日期 -->
                <span v-else-if="item.field === 'modifydate'">
                  {{row.modifydate === '1900-01-01 00:00:00' || row.modifydate === '' ? '' : row.modifydate | formatDate("yyyy-MM-dd") }}
                </span> 
                <!-- 创建日期 -->
              <span v-else-if="item.field === 'adddate'">
                  {{row.adddate === '1900-01-01 00:00:00' || row.adddate === '' ? '' : row.adddate| formatDate("yyyy-MM-dd") }}
                </span>                 
                <span v-else-if="item.field=='price_status'">
                  {{row['status']=='0'?'无效':'有效'}}

                </span>

                <span v-else
                  v-html="item.formatType?formatFunc(row[item.field],item,row):((row[item.field]||row[item.field]==0)?row[item.field]:emptyCellText)"></span>
              </slot>
          ...
          <tr v-if="((ziIndex==index)||showAllZiTable||showZiIndex.includes(index))&&!row.isSeat&&!row.ziTableHidden">
            <td colspan="100" style="padding:0px;border:0px" class="zitable">
              <slot name="ziTable" :data="{row:row}"></slot>
            </td>
            <td> 
              <slot name="operate" :data="{row:row}"></slot>
            </td>
      ...
      <el-pagination style="margin-top:10px;" @size-change="handleSizeChange2" @current-change="handleCurrentChange2"
        :current-page="currentPage2" :page-sizes="[10, 15, 20]" :page-size="pageSize2"
        layout="total, sizes, prev, pager, next, jumper" :total="tableData.length" v-if="paginationShow">
      </el-pagination>
    </div>
  </div>
</template>
...
import _debounce from 'lodash.debounce';
  ...
  export default {
    props: {
      ...
      firstTdShow: {//是否显示操作行,无需新增的时候隐藏编辑行
        type: Boolean,
        default: true
      },
      checkValue: Object,
    ...
    methods: {
      formatFunc(value, item, row) {
        ...
        if (item.formatType == 5) { //供应商项目取中文名
    ...
    computed: {
      transform() {//固定头部
        return this.position.scrollTop > 0 ? { transform: `translateY(${this.position.scrollTop}px)`, 'box-shadow': 'rgba(0, 0, 0, 0.9) -12px 0px 12px 0px' } : ''
      },
      multiSelect: {
        get() {
          let n = 0;
          ...
          return n == this.tableData.filter(i => !i.checkboxdisable).length && this.tableData.filter(i => !i.checkboxdisable).length > 0;
        },
        set(val) {
          console.log(val)
          this.tableData.forEach(i => {
        ...
        handler(val) {
          ...
          if (this.isMultiSelect || this.isRadioSelect) {
            val.forEach((item, index) => {
              ...
              } else if (!item.checkboxdisable && index == this.defaultSelectRowIndex || item.defaultChecked) {//defaultChecked 设置默认选中
                this.$set(item, "checked", true);
